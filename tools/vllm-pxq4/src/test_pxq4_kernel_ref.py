...
Nothing here needs a GPU, a container, or the checkpoint. Run:
...
def _load_hostsim():
    ...
    if not os.path.exists(_LIB_PATH):
        ...
        # means "no compiler here" (a host without a toolchain), not "the kernel is wrong".
        ...
        raise _HostsimUnavailable(
            ...
            f"    (no g++ on this host; build on a dev host or in a CUDA container)")
...
def test_real_artifact_panel():
    ...
    Qwen3.8-27B-PXQ4.gguf (ggml type 252, K=5120, R=6144) by
