...
Four implementations are compared pairwise:
  ...
  A     -- the converter's src/pxq4_vllm/reference.py                          [skipped if absent]
  CUDA  -- the kernel's torch.ops.pxq4.dequant_out                          [needs a GPU]
...
def test_g1_oracle_vs_agent_a(real=None):
    ...
    The converter ships one, and the kernel ships one as its numpy twin.
