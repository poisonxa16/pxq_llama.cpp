...
never needs a serving container's python and cannot perturb it.

USAGE
  # side A -- our engine, configured the way it is served
  llama-server -m $PXQ4_GGUF -ngl 99 -c 4096 \
               --host 127.0.0.1 --port 8081

  # side B -- vLLM with the plugin, on FREE cards
  PYTHONPATH=$PXQ4_ROOT/site \
  VLLM_SM70_QUANT_BACKEND=turbomind \
  python -m vllm.entrypoints.openai.api_server \
      --model $PXA_ROOT/models/Qwen3.8-27B-PXQ4-vllm \
