...
def test_dequant_smem_limit_for_wide_k():
    ...
    shapes that only appear at TP<=2 -- i.e. never on a 4-GPU host and always on the
    2-GPU host.  That is a bug that ships.
    """
    ops, torch = _require()
...
def assert_no_sm70_fastpath(model) -> None:
    ...
    ASSUMPTION: the runtime plugin names the class `PXQ4LinearMethod` (plan §6.6). This helper
