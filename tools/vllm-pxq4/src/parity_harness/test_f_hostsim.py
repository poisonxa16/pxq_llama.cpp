"""
GATES H1-H6 -- run the ACTUAL shipping kernel, on the CPU, with no GPU.
...
# [b0, b1) = [(kslabs*c)/nfix, (kslabs*(c+1))/nfix), with the kernel's EDIT 3 staging ONLY that
...
def test_h4_hostsim_shard_invariant(real=None):
    ...
    dequantizing a shard with the kernel's own code gives bit-identical values to the
