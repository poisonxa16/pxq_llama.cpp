# PXQ4-in-vLLM parity harness (plan §9)
...
`Qwen3.8-27B-PXQ4.gguf`; the converter reference, the kernel's
numpy twin, AND the real CUDA kernel (host-simulated) all bit-exact against the
production `ggml/src/pxq-cpu.c`. No container was touched. `pxq_llama` was
...
A harness that compares the converter's reference against the kernel proves they agree,
...
```
 ...
 gguf_to_vllm.reference        converter   what the converter calls
 pxq4_kernel_ref               kernel      the kernel's numpy twin
        |  H1-H7 (bit-exact)              <- CPU, runs today, via libpxq4_hostsim.so
 k_pxq4_dequant_matrix / k_pxq4_mmv        the REAL device code, host-simulated
        |  G6/G8 (bit-exact, fp16)        <- needs a GPU
 torch.ops.pxq4.*              kernel      the same code, on sm_70
```

**The hostsim leg is the reason most of this runs with no GPU at all.**
...
drives it through ctypes. So gates H1-H7 test the actual kernel source -- layout
addressing, table values, accumulation order, the fp16 store, and the whole shard
invariant -- locally, in a second.
...
# add the CUDA gates (needs a GPU + the built .so)
PXQ4_LIB=$PXQ4_ROOT/build/libpxq4_sm70.so \
...
The 14.64 GiB artifact lives on the V100 host, which has no numpy. `extract_raw.py` is
stdlib-only and self-contained — scp it anywhere:

```bash
# where the artifact lives (writes only under $FIXTURES)
python3 extract_raw.py --gguf $PXQ4_GGUF \
                       --out  $FIXTURES/raw --panels 4 --panel0 1
...
| G1b | the converter's `reference.dequant` == oracle, bitwise; and its BOOK/SUB == `ggml-pxq6-tables.h` | converter |
...
`[(kslabs*c)/nfix, (kslabs*(c+1))/nfix)`, with the kernel's EDIT 3 staging only that chunk's
...
for all 256 pairs. The kernel may fold `anchor*sub` into `eff` with no bit-exactness risk,
...
3. `test_d_ops_abi.py` — `assert_no_sm70_fastpath` assumes the runtime plugin's class is named
...
extract_raw.py       GGUF -> .bin + manifest.json (stdlib only; runs on the V100 host)
