...
never to PASS.  A gate that cannot see the thing it tests must say so.
  ...
  * `pxq4_vllm.reference` / `pxq4_vllm.layout` (converter)  -- plan §6.2, §6.3
  * `libpxq4_sm70.so` -> torch.ops.pxq4.*     (kernel)   -- plan §7.1
"""
...
# The plan (§4) names the runtime package `pxq4_vllm`, but the components landed under
# flatter names in this directory while the repo layout was still being settled.
...
def all_ref_modules():
    ...
    More than one is normal here: the converter ships one and the kernel ships
...
def pxq4_ops():
    ...
    Resolution order:
      1. pxq4_vllm.ops.load_library()  -- the supported path once the plugin has landed
      2. $PXQ4_LIB                     -- direct .so path, for testing the kernel standalone
