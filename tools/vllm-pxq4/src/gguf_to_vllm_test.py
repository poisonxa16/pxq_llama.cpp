"""gguf_to_vllm_test.py — CPU-only test suite for the converter. No GPU and no accelerator host.
...
Fixtures live in ``fixtures/`` and are REAL BYTES pulled from the real artifact:
    ...
    hdr.bin            first 10,997,184 B of a Qwen3.8-27B-PXQ4 artifact — the complete
                       header, KV table and 866-entry tensor directory, with no data
                       section. NOT shipped (10.5 MB); regenerate it from any PXQ4 GGUF
                       with:  head -c 10997184 <model>-PXQ4.gguf > fixtures/hdr.bin
                       and set GGUF_TRUE_SIZE below to that file's real size. The header
...
def header() -> G.GGUFHeaderOnly:
    ...
    ``fixtures/hdr.bin`` is a 10.5 MB header slice of a real artifact and is not shipped with
    the package. When it is absent these tests SKIP rather than fail -- see the module
    docstring for the one-line command that regenerates it."""
    if not os.path.exists(os.path.join(FX, "hdr.bin")):
        raise unittest.SkipTest(
            "fixtures/hdr.bin absent: regenerate with "
            "`head -c 10997184 <model>-PXQ4.gguf > fixtures/hdr.bin`")
    if not _HDR_CACHE:
...
class TestDenseDecoders(unittest.TestCase):
    ...
    def test_mxfp4_bit_exact(self):
        # ssm_out on all 48 GDN layers is MXFP4, not PXQ4 — the type the initial project spec did not expect.
