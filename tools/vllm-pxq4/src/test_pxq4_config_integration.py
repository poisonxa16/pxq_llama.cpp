...
container started from the serving image (never a container serving traffic):

  docker run --rm --network none -e CUDA_VISIBLE_DEVICES= \
    -v $PXQ4_ROOT/impl:/work -w /work $VLLM_IMAGE \
...
Every check here is one that a stub cannot honestly make:
  ...
  4. real ``importlib.metadata`` entry-point discovery from a hand-written
     ``.dist-info``, which is how this ships (the image is treated as read-only).
"""
...
def test_entry_point_discovery_from_a_handwritten_dist_info():
    ...
    hand, because nothing can be installed into the image (it is read-only,
    0 bytes available)."""
