"""pxq4_validate.py -- prove a built libpxq4_encode.so agrees with the numpy reference.

Run this before letting the native encoder touch a real artifact: an encoder that produces
plausible-but-wrong bytes poisons every checkpoint downstream.

    python3 pxq4_validate.py --so ./libpxq4_encode.so --gguf /path/to/model-PXQ4.gguf
"""
import argparse, os, sys, ctypes, time
import numpy as np

_here = os.path.dirname(os.path.abspath(__file__))
ap = argparse.ArgumentParser(description=__doc__.splitlines()[0])
ap.add_argument("--so", default=os.environ.get("PXQ4_ENCODE_SO", "libpxq4_encode.so"),
                help="libpxq4_encode.so built by tools/build_encoder.sh")
ap.add_argument("--gguf", default=os.environ.get("PXQ4_GGUF"),
                help="a real PXQ4 .gguf artifact to validate against")
ap.add_argument("--src", default=os.environ.get("PXQ4_SRC", _here),
                help="directory containing the gguf_to_vllm package (default: this file's dir)")
args = ap.parse_args()
if not args.gguf:
    ap.error("--gguf is required (or set PXQ4_GGUF)")
