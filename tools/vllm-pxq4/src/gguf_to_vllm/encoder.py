...
The backbone table pins k/v and the LM head away from 4 bits deliberately (recorded in the file as ``pxa.pxq.backbone_map``), so each of these is a quality decision, not
...
``pxq_llama/src/pxq6-quantize.inc.cpp`` read-only so the file-static
...
The extension is not built by this component (plan §9: the kernel component owns ``csrc/``). If
