# vLLM quantization plugin surface — the sm_70 vLLM fork, as installed in the target container

Source read in the community sm_70 vLLM fork, git HEAD `2ceb15066518e7241cf3c57a71ddd39168cbc675` (2026-08-15) — matches the reported `v0.1.dev1+g2ceb15066`. All line numbers below are from that tree. This is a read-only source study: nothing was built, run or measured for this document.
...
class PXQ8LinearMethod(LinearMethodBase):        # q8_0 tier: attn_k / attn_v / geometry failures
    ...same four members...
```
Plus, from the mixed-checkpoint reality (`PXA_PXQ_BACKBONE` rev 2), `PXQ4Config.get_quant_method` must dispatch **per tensor class**:
...
- Exact `envs.py` line numbers for `VLLM_SM70_QUANT_BACKEND` were taken from the initial project spec, not re-read here.
