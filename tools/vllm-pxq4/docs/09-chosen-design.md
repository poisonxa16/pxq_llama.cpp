...
`08-design-least-code.md`. Implementations are built from this file.

LAYOUT NOTE: this plan specifies a `pxq4_vllm/` package and a `tools/pxq4_gguf/` converter.
The code as shipped is FLAT under `tools/vllm-pxq4/src/` — `pxq4_config.py`, `pxq4_linear.py`,
`pxq4_ops.py`, `pxq4_kernel*.{cu,cuh,cpp}` and the `gguf_to_vllm/` package — because the flat
layout is what the build and the test entry points resolve against (see the README). Module
names map one-for-one; paths in this document describe the design, not the tree.
This is a design document: no GPU was run and no number in it is a measurement unless it is
tagged as one.
...
this rig is ≈ +9% over the incumbent's measured 92.8 tok/s peak, and reaching even that
...
This was re-derived independently from both checkpoints, and it confirms
...
Measured inputs (FACT, both read directly):

| | source | bytes |
|---|---|---:|
| AWQ body (layers 0–63, excl. lm_head/embed/visual/mtp) | safetensors headers, `awq.py` | 12,691,100,928 (11.820 GiB) |
...
= **4.156 bpw** (group_size 128, 4-bit, asymmetric — `config.json quantization_config`). Our PXQ4 `ffn_gate` of the same shape is **47,384,576 B = 4.2540 bpw**
...
1. **Quality-per-bit at ≈ equal bytes** — PXQ4 is imatrix-calibrated with a learned 16-entry
   ...
   beats AWQ on our own eval at parity bytes, that is the product.
...
**GATE G0 (product decision, no code, do it first): does PXQ4 beat the AWQ twin on quality at
...
CPU-only bit-exactness gates retire >90% of format risk before any GPU is needed.
...
Everything below was read in source **directly** unless marked otherwise.
...
- `register_quantization_config(name)` — `quantization/__init__.py:57-102`. Appends to the
  ...
  — per `03-vllm-plugin-surface.md`). **Zero patches to the community sm_70 vLLM fork.**
...
### 2.7 Toolchain (as shipped in the serving image)
...
the community sm_70 vLLM fork. **Treat the image as read-only.**
Nothing may be installed into it. Everything we ship lives on a mounted host volume and is
reached by `PYTHONPATH` + a hand-written `.dist-info` (see §7.4).

### 2.8 PXQ4 device format (re-read from source)
...
One git repo, checked out anywhere, and deployed at `$PXQ4_ROOT/`
(never to `/`, never to container `/`).
...
pxq4-vllm/
  ...
  tools/pxq4_encode/                 # P2 only
    encode_shim.cpp                  # #includes pxq_llama's pxq6-quantize.inc.cpp (read-only)
    CMakeLists.txt
  tests/
...
Pure Python + numpy. No torch, no CUDA, no vLLM, no GPU. **Fully testable on a CPU-only host.**
...
python -m pxq4_gguf.convert \
  --gguf     $PXQ4_GGUF \
  --ref-hf   $AWQ_REF \
  --out      $PXA_ROOT/models/Qwen3.8-27B-PXQ4-vllm \
  --policy   {p1,p2a,p2b,p2c} \
  [--encoder $PXQ4_ROOT/build/pxq4_encode.so]      # P2 only
  [--shard-size-gb 4]
```
...
Copy **verbatim** from `pxq_llama` (read-only source; do not edit that tree):
...
the community sm_70 vLLM fork.**
```
nvcc -O3 -std=c++17 -gencode arch=compute_70,code=sm_70 \
     --expt-relaxed-constexpr -Xcompiler -fPIC -shared
```
Build in a **throwaway container from the same image** with a host volume mounted,
writing only there. A serving container's `/` must not be
written to, and a container that is serving traffic must not be restarted.
...
```
launched with `PYTHONPATH=$PXQ4_ROOT/site`. `importlib.metadata` discovers
...
CPU-only gates run anywhere, with no GPU and no container. GPU gates need real hardware.

| # | gate | needs | what it retires |
|---|---|---|---|
| **G0** | PXQ4 beats AWQ on quality at parity bytes | product decision, no code | the project's reason to exist |
...
| **G9** | first throughput measurement. **Nothing before G9 may quote tok/s.** | GPU | — |
| **G10** | P2 only: 4-bit `lm_head` and 4-bit `attn_k/v` each clear the serving quality bar *before* they ship | GPU | that P2 buys speed without buying a quality regression |
...
## 9. Component split (four components, four contracts)

The work divides into four components that can be built in parallel because the
interfaces between them are frozen in this document. Each column says what a
component owns, what it must leave alone, and what it waits on.

| component | owns | must not touch | blocked on |
|---|---|---|---|
...
semantics), **§7.1** (the op ABI). Nothing else crosses a boundary. The converter can reach G1–G4
with no input from the plugin or the kernel. The kernel can reach G6 against the converter's `reference.dequant` alone.
...
Files modified in the community sm_70 vLLM fork or `pxq_llama`: 0.**
...
## 11. Corrections to the initial project spec, carried forward
...
- **PXQ4 is 2.33% larger per tensor than the incumbent's AWQ** (4.254 vs 4.156 bpw). The
  initial project spec's "3.66 GiB/GPU vs their 4.64" compared different scopes; the like-for-like decode-read
...
- **MoE: void, and measured.** 866 tensors, zero `*_exps`, zero expert KVs. Zero effort, zero
  fallback, zero VRAM. The 35B model is the MoE one.
