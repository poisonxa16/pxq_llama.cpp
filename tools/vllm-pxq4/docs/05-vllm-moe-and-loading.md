...
All paths below are relative to a checkout of the community sm_70 vLLM fork
(build `0.1.dev1+g2ceb15066.cu128`), as installed in its serving image. This is a
read-only source study: nothing was built, run or measured for this document.

---

## 0. Headline

**MoE effort delta = ZERO.** FACT, from the artifact itself. The GGUF header of
`Qwen3.8-27B-PXQ4.gguf` was parsed directly (raw struct parser, not the `gguf`
...
**Two corrections to the initial project spec, both load-bearing:**
...
1. `sm70_turbomind.py` is **not** a `QuantizationConfig` and does not register anything. It is
   ...
   which is the scheme in production for this model.

2. The PXQ4 artifact is **five** types, not the four the initial project spec listed, and it contains **no f16
   tensor at all**. Measured histogram (ggml type id → count):
   `{0 (F32): 360, 8 (Q8_0): 132, 14 (Q6_K): 1, 39 (MXFP4): 48, 252 (PXQ4): 325}`.
   `GGML_TYPE_MXFP4 = 39` confirmed at `pxq_llama/ggml/include/ggml.h:424`.
...
* **Gating predicates** — `is_exact_sm70_cuda(tensor, enabled)` (`:44-47`) checks
  ...
  to `vllm.envs` — this is the `VLLM_SM70_QUANT_BACKEND` routing knob, and the initial project spec is right
...
* **Operational blockers, both real:**
  - the container's overlay is effectively read-only in practice (`df -h /` shows it full).
    **You cannot build in this container.** Build in a fresh container from the same base image with a
    writable host volume, then install the resulting wheel.
  - `site-packages/vllm` is a **copied install**, not an editable link to the community sm_70 vLLM fork
    (`ls -la` shows a real directory, and `vllm.__file__` →
    `/opt/vllm-venv/lib/python3.12/site-packages/vllm/__init__.py`). Editing `vllm/*.py` in that checkout
    changes nothing at runtime. Another reason to go out-of-tree.
* **Caveat (ASSUMPTION):** two `TORCH_LIBRARY` namespaces coexisting is standard PyTorch, but no
  such `.so` was built or loaded against this image — this note predates any GPU run. The one
...
* **`in_proj_ba` must be split out, and the switch is our config's ignore list.**
  ...
  production AWQ config does exactly this — its `config.json` lists
...
And the failure point is unrecoverable at *file* granularity, not tensor granularity. From
`gguf.gguf_reader.GGUFReader._build_tensors` in the installed package:
...
* Independently, the initial project spec's warning stands: `GGUFUninitializedParameter` + `_create_padded_weight_param`
...
standalone script; the runtime side is an out-of-tree pip package (§A2). the fork's tree stays clean,
...
1. Parse the GGUF header with **our own** reader (the raw struct parser already written and run —
   the raw GGUF scanner used for the census), *not* the `gguf` package. 30 lines: magic/version/counts,
...
3. Rename ggml names → HF/vLLM names (`blk.N.ffn_gate` → `model.language_model.layers.N.mlp.gate_proj`,
   ...
   the AWQ W4A16 twin's key set and match one-for-one.
...
1. **Converter output size.** Dequantizing `token_embd` (Q6_K) and `output.weight` (Q8_0) to fp16
   ...
   space **on a mounted host volume** — never the container `/`.
...
3. Build it in a **fresh** container from the same image, writing only to a mounted host
   volume — a serving container's overlay must not be written to.
