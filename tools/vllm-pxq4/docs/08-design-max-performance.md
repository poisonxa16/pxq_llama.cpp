# PXQ4 as a first-class quantization backend in the sm_70 vLLM fork — maximum-performance design

Target: the community sm_70 vLLM fork @ `2ceb15066`, Qwen3.8-27B (gguf arch `qwen35`), 4x V100-32GB (TP=4)
and 2x V100-16GB (TP=2). Artifact: `Qwen3.8-27B-PXQ4.gguf`.
Every line citation below is to a file that was read (either `pxq_llama` @ `acf8f245`, or
the community sm_70 vLLM fork). **No GPU was run for this document. Every throughput
...
**But the headline premise in the initial project spec is wrong, and the design turns on fixing it.** The artifact is
...
At TP=2 (2x16 GB): Policy MAX = 7.815 GiB/GPU resident, leaving ~7 GiB/card for KV
...
| `csrc/pxq4_vendor.cuh` | **500 (copied verbatim)** | the id-252 slice of `pxq_llama/ggml/src/ggml-cuda/pxq6.cuh` (see §4.1 for the exact symbol list and line ranges) + the 32 table constants from `ggml/include/ggml-pxq6-tables.h:21-44`. Carries the provenance canary comment (`pxq6.cuh:1-3`) and a `PXQ4_VENDOR_SRC_COMMIT "acf8f245"` macro |
...
| `tools/enc/pxq4_enc.c` + `Makefile` | 60 | 40-line exported shim over `pxq6_quantize_expert` compiled from a **copy** of the pxq_llama tree (§5.4) |
...
### 2.2 Files to patch in the community sm_70 vLLM fork

**None. Zero. The design requires no fork patch.** This is deliberate and is what makes the work
shippable as a package rather than a merge.
...
From `pxq_llama/ggml/src/ggml-cuda/pxq6.cuh` — **`pxq4.cuh` is a decoy**, it documents the retired
...
| v2 experiment | `gemm_out` (fused tile) and `gemm_wmma_out` | both compiled in, both default-off behind `PXQ4_PREFILL=fused|wmma|dequant`. Promotion requires a measured win on a real card |
...
Operational constraints (FACT, surveyed on the build host): nvcc 12.8, gcc, cmake, ninja and torch 2.10.0+cu128 are
present in the container image; but **a serving container's overlay must be treated as
read-only**, and so must every system root. Therefore: build in a **fresh** container from the
same image with a mounted host volume, never inside a container that is serving traffic, never writing to
`/` or host `/tmp`. Also note `site-packages/vllm` is a *copied* install, not
editable-linked to the community sm_70 vLLM fork — edits to that tree are inert at runtime, which is another
...
Own ~200-line GGUF parser (already prototyped as `ggufscan.py`/`tc.py`).
...
by diffing against the AWQ twin's `model.safetensors.index.json`
...
* **Encoder:** build `libpxq4enc.so` from a **copy** of the pxq_llama tree at the pinned commit, adding a
...
* Quality: this creates a *new artifact* and therefore needs the standard PXQ quality gate before
  deployment (same class of work as the open 35B MoE gate). That gate is CPU-side perplexity/
  same-top-token on a GPU — it is real work and it is outside this design's scope.
...
all 65 blocks. The initial project spec's "40×256 routed experts" belongs to **the 35B MoE model**, a different model. No
...
uses. If PXQ4 is ever pointed at that 35B MoE model, the additional work is a `FusedMoEMethodBase` subclass plus
...
(`n_active_experts × expert_shard_bytes`), never resident fp16 experts; sizing that requires the 35B MoE model's
per-expert shapes, which this document did not measure.
...
initial project spec. **This design's projections reuse that same 462 GB/s constant**, which makes them a like-for-
like scaling of a measured point on this exact hardware rather than a fresh guess.
...
| Policy MAX, TP=2 (16 GB) | 7.815 GiB | 6.631 GiB | 64.9 | n/a |
...
in S2/S3; those two are the only pre-model GPU touches and must be scheduled on real hardware
(this document performs none).
...
| **S7** | whole model, TP=4, greedy | same-top-token vs the llama.cpp PXQ4 engine on a fixed 500-prompt set — the two engines share the *same weight bytes*, so disagreement is a port bug, not a quantization artifact. This is the strongest end-to-end gate available and it needs no reference fp16 model | GPU window |
| **S8** | perf: `mmv_max` crossover sweep, v2 kernels, autotune cache | throughput vs the incumbent on the same harness (790-sample methodology) | GPU window |
...
2. **The VRAM premise depends on work outside vLLM.** Policy A (no re-encode) is a *regression* vs the
   ...
   i.e. a new artifact plus a quality gate on a GPU. *Detect:* immediately — it is an input
...
7. **Container/build friction.** The production overlay is effectively read-only;
   `site-packages/vllm` is a copy, not editable. *Mitigate:* fresh container + host volume,
   never touch a serving container beyond `docker exec`-to-read.
...
**What would retire the project:** risk 1 landing badly *and* risk 2's quality gate failing
together — that combination leaves a single-stream-only win on a model that no longer fits the VRAM
story. Nothing in the survey suggests either is likely; both are measurable early (S8 and the
...
risk retirement. S4–S6 is the vLLM integration. S7–S8 need GPU windows and are where every
