...
Source of truth: `pxq_llama` @ HEAD `acf8f245` (READ ONLY).
...
`ne[1] = R` (number of weight output rows). Below, **R = rows, K = columns**.
...
`Qwen3.8-27B-PXQ4.gguf`, assume the frozen tables unless the KVs say
otherwise (ASSUMPTION — that file's KVs have not been read; verify
...
initial project spec plus the formula above.)
...
| `output.weight` | via `pxa_pxq_head_type()` (q8_0 per the initial project spec) | 1323 |
...
3. **What you may NOT do:** slice at a row boundary that is not a multiple of 64, slice K
   ...
   The last one is the specific reason the initial project spec's warning about vLLM's generic GGUF
...
- `Qwen3.8-27B-PXQ4.gguf` has **not** been opened here. Its
...
- The HQ (`sub8`) table values are in `ggml-pxq6-tables.h:47-51`. They are not transcribed
