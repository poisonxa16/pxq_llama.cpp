...
Source of truth: the struct-parsed GGUF header of `Qwen3.8-27B-PXQ4.gguf` (a raw struct
parser, not the `gguf` package). Only the header and tensor directory were read
...
backend must handle, and it was not in the initial project spec's expected set. Everything else
...
`general.file_type = 38` (INFERENCE: a PXA-local PXQ4 ftype; the nearest `MOSTLY_*`
enumerator to it is `GGML_FTYPE_MOSTLY_MXFP4 = 25` at
...
`ffn_gate_inp`. Confirms the initial project spec: this model has no MoE.
...
tensor type is 252/PXQ4 (consistent with the initial project spec's "id-252 kernels live in
...
2. **MXFP4 is the newly-discovered work item.** 48 `ssm_out` GEMMs (6144→5120,
   ...
   table simply does not claim the `ssm` class (`ssm_*` is only reachable via the `PXA_PXQ_NATIVE=ssm` research override, and the
   backbone policy states that an unclaimed/failing tensor "is demoted to MXFP4 by the
...
5. **`attn_q` and `attn_qkv` are fused-multi-output.** Column shards must be taken
   ...
   `MergedColumnParallelLinear` (per the initial project spec; not re-verified here).
...
sharded row-wise at 248320/4. The initial project spec's 3.66 GiB/GPU figure assumes uniform PXQ4
