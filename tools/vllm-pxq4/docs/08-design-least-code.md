...
Target: the community sm_70 vLLM fork @ `2ceb15066` (`v0.1.dev1+g2ceb15066`), 4x V100-SXM2 (sm_70), TP=4,
...
The port is feasible with ~1,600 hand-written LOC, zero patches to the fork's tree, and two CUDA kernels.
**But the throughput premise it started from is arithmetically wrong, and that follows from the two checkpoints
on disk.**

The initial project spec says "PXQ4 sharded 4 ways = 3.66 GiB/GPU vs their AWQ 4.64 GiB → 110-120 tok/s". Those two
...
Decode throughput on both engines is bytes-read-per-GPU-per-token (established fact in the initial project spec). So:
...
HBM peak, and that KV/activation traffic is unchanged. No GPU was run for this document.)
...
The first question was whether a new quantization method could be avoided entirely. Four candidates, three rejected.
...
It fails on the codebook, and the artifact itself proves it. The file stores its book
(`pxa.pxq6.book`, read out of the GGUF):
...
**One more seam, which removes a whole class of converter work.** The fork's
...
| file | LOC | contents |
|---|---:|---|
| `gguf_raw.py` | 180 | Raw GGUF v3 header parser. **Reuse the scanner already written this design pass** (the raw GGUF scanner used for the census above) — it already parses 866 tensors / 57 KVs of this exact file. Must NOT use the `gguf` PyPI package: `gguf.GGMLQuantizationType(252)` raises inside `GGUFReader._build_tensors`, killing the whole file open. |
...
**None in the community sm_70 vLLM fork. None in `pxq_llama`.** Both trees are read-only in this design.
...
- **Merged column-parallel with a tuple shard id** — the GDN `in_proj_qkv` case. Read at
...
would need the S-split path — irrelevant at TP=4, **relevant for the 2x V100-16GB host only if
...
Build environment, as measured in the target container: `nvcc` 12.8, gcc, cmake, ninja and `torch 2.10.0+cu128`
are present in the container, and `site-packages/vllm` is a **copied** install (not editable-linked to
the community sm_70 vLLM fork, so edits there are inert at runtime — another reason the plugin is the right seam).
Two operational constraints: treat a serving container's overlay as **read-only**, so
build in a *fresh* container with a mounted host volume; and never write to `/` or host `/tmp`.
...
### 6.3 Name map (authoritative — taken from the incumbent checkpoint's own keys)
...
header parse: 866 tensors, `general.architecture = qwen35`, **zero `*_exps` tensors and zero
expert KVs**; `ffn_gate`/`ffn_up`/`ffn_down` are dense on all 65 blocks. The initial project spec's "40x256 routed
experts" belongs to **the 35B MoE model**, a different model. No `FusedMoEMethodBase`, no `RoutedExperts` path, no
dequant fallback, **0 GiB of fallback VRAM cost** — the line item does not exist.

If that 35B MoE model is targeted later, the seam is `FusedMoEMethodBase` (not `LinearMethodBase`), and the ggml
...
A scheduling reality first: **stages S2+ need a GPU for unit tests.** The 4x V100 host is booked by other
jobs and the 2x V100 host is serving production traffic. S0/S1 gates are CPU-only and can start immediately;
...
10. **Volta smem ceiling at TP=1** (§5.1): 69.6 KB needed vs 46 KB cap for `ffn_down` — TP=1 is
    unsupported without the S-split path. Document it; the 2x V100-16GB host must use TP=2.
...
|---|---:|---:|
| offline converter | 810 | 0 (but ~180 reused from the existing scanner) |
...
Patches to the community sm_70 vLLM fork: **0 files.** Patches to `pxq_llama`: **0 files.**
