...
directions. All three analyses reached that conclusion independently, and every load-bearing
claim was re-verified against source. The genuine risk in this project is *not* sharding — it is
...
Unanimous, and each point independently re-verified below or in the format and
loading notes (01-06):
...
FACT, `pxq_llama/src/llama-build-context.cpp:1993-2007`
...
- v1: `RowParallelLinear.weight_loader`, `vllm/model_executor/layers/linear.py:1728-1761`.
...
### 2.4 The "3.66 GiB/GPU" premise in the initial project spec
**moe-and-mixed is RIGHT that the earlier figure is wrong, and its arithmetic reproduces
...
the MoE analysis's Policy A (everything non-PXQ4 → fp16) gives **5222.2 MB = 4.864 GiB/GPU at TP=4**,
...
### 2.6 46 KB smem cap (kernel survey) — the row-parallel analysis could not verify it
Still **NOT VERIFIED** here: `ggml-cuda.cu:4262` was not re-checked. Immaterial to sharding: at TP=4
...
### TP = 4 (4×V100-32GB, NVLink): **SHARDS. No re-quantization.**
...
### TP = 2 (2×V100-16GB, PHB/no NVLink): **SHARDS. No re-quantization.**
...
`tp_size == 2 and layer._awq_sm70_prepared`, so we forgo it on the host where the all-reduce is most
...
- The dequant contract has **zero cross-K and zero cross-row coupling**:
  ...
  `pxq_llama/ggml/src/pxq-cpu.c:141` (`const float anchor = GGML_COMPUTE_FP16_TO_FP32(((const uint16_t*)panel)[r]);`),
...
FACT, `pxq_llama/src/pxq6-quantize.inc.cpp:287-289`:
```
...
## 5. Corrections to the initial project spec's own framing
...
- **The incumbent AWQ ignore list is more generous than assumed** (read from
  the AWQ twin's `config.json`, 311 entries):
...
- The same list confirms `in_proj_b`/`in_proj_a` are ignored in production, which is exactly what
  ...
  (`vllm/model_executor/models/qwen3_5.py:127-157`: scans
  ...
  `rows % 64`. **Geometrically mandatory, as the MoE analysis found.**
...
no GPU runs were performed for this document.
