...
`Qwen3.8-27B-PXQ4-vllm-p1f`, greedy, 512-token streams, `--max-num-seqs 4`,
...
* **A checkout can differ from what is actually running.** One fork checkout we
  worked from carried an uncommitted, never-executed edit swapping the pure-torch
  KV write for `torch.ops._C_cache_ops.reshape_and_cache_flash` — an op that build
  does not have — while the serving container predated the edit entirely. Run
  `git diff` in the checkout before trusting that the source explains the binary.
