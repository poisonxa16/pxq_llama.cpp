...
Target: the sm_70 vLLM fork @ `2ceb15066` (`v0.1.dev1+g2ceb15066.cu128`), 4x V100-SXM2-32GB (sm_70),
TP=4, `dtype=float16`. Artifact: `Qwen3.8-27B-PXQ4.gguf`.
...
**No blocker.** Every hard question is already answered by the format and loading notes: the format is pinned
(01), the kernels carry no ggml types and can be vendored (02), `register_quantization_config`
is a real out-of-tree hook (03), sm_70 is a first-class capability in this fork, the GGUF
...
initial project spec's headline ("PXQ4 sharded 4 ways = 3.66 GiB/GPU vs their AWQ 4.64 → 110-120 tok/s") is
...
Nothing in the community sm_70 vLLM fork is patched. Nothing in `pxq_llama` is modified.
...
| file | LOC | contents |
|---|---|---|
| `tools/pxq4_gguf/gguf_raw.py` | 130 | Raw GGUF v3 struct parser: magic/version/`n_tensors`/`n_kv`, KV table (all 13 GGUF value types incl. arrays), tensor table (name, n_dims, dims, **raw type id as an int — never an enum**), aligned data base. Derived from the working scanner already run against the artifact (the raw GGUF scanner used for the census). **This is the whole reason option (b) is cheap** — `gguf.GGMLQuantizationType(252)` raises `ValueError` inside `GGUFReader._build_tensors`, killing the entire file open (FACT, 05 §B1). |
...
| `tools/pxq4_gguf/namemap.py` | 220 | ggml→HF/vLLM name mapping + the fusion/split policy (§5.3). Ships a `--check-against <awq_twin_dir>` mode that diffs the produced key set against the AWQ twin's key set and **fails the build on any mismatch**. This is the cheapest possible gate on the fiddliest part of the job. |
...
| `pxq4_vllm/csrc/pxq4_pxa.cuh` | 560 | **Vendored verbatim** from `pxq_llama/ggml/src/ggml-cuda/pxq6.cuh` (~500 lines) + ~60 lines of edits. Slice list in §4.2. |
...
**In the community sm_70 vLLM fork: NONE.** This is a design requirement, not a preference —
`site-packages/vllm` is a *copied* install, not an editable link to the community sm_70 vLLM fork, so edits
there are inert at runtime anyway (FACT, 05 §A2).

**In `pxq_llama` (our llama.cpp tree): NONE.** Read-only by rule. The vendored kernel
slice is a copy, with the provenance canary comment (`pxq6.cuh:1-3`) preserved and a
`// VENDORED FROM pxq_llama@acf8f245 ggml/src/ggml-cuda/pxq6.cuh:<range>` banner per block so a
...
`vllm/config/vllm.py:611-621` (FACT, read in source): `capability < get_min_capability()` raises, then
...
Copy verbatim from `pxq_llama/ggml/src/ggml-cuda/pxq6.cuh` (**not** `pxq4.cuh` — that
...
1. A serving container's overlay must be treated as read-only. **You cannot build in
   a container that is serving traffic,** and you must not try.
   Build in a *fresh* container from the same image with a writable host volume,
   then `pip install` the resulting wheel into the deployment.
2. Nothing is written to a system root. Everything — build tree, wheel, converted checkpoint —
   lives on a mounted host volume. Never `/` and never host `/tmp`.

No GPU time is needed for any of the build steps or for stages S0-S2's offline gates.
...
FACT (05 §0, 06 §1): the GGUF header of `Qwen3.8-27B-PXQ4.gguf` was parsed
...
(6 distinct PXQ4 shapes, none expert-stacked). This is consistent with the initial project spec's own statement
and with the AWQ twin's HF config. The 35B model is the MoE one; this is not it.
...
the MoE case is *less* work for us than the dense case, not more. That is a note for the 35B MoE
...
Every gate below **C0-C5** is verifiable without a full model run. Gates C0-C3 need no GPU time
...
- **C2 (GPU, load only):** the fp16 checkpoint loads at TP=4 and generates. Compare greedy
  ...
  VRAM: 13.5 GiB/GPU at TP=4 (fits 32 GB with KV). **Does not fit 2×16 GB cards** — S0 is
  4x32 GB only.
...
- Only now is a throughput measurement meaningful — and it needs GPU time this document does
  not have.
...
a serving container cannot be built in and must not be touched. Mitigated by
building in a fresh container with a host volume.

**R7 — vendoring forks the kernel.** Future fixes in `pxq_llama`'s `pxq6.cuh` will not propagate.
...
**R9 — the 2×16 GB deployment (TP=2).** S0/S1/S2 need 27 GiB/GPU and simply do not fit;
even S3 at 10.4 GiB/GPU leaves little KV headroom on 16 GB cards. The 2x16 GB host is a phase-2 target
(V2 policy = 6.8 GiB/GPU), and its cards are typically already committed to production traffic.
...
included. Script: the VRAM accounting sheet in §7.
...
900 GB/s (= 450 GB/s effective), which is the initial project spec's established measured fact; no MTP, no
