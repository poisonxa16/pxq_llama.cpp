...
Date: 2026-08-18. Hardware: Tesla V100-SXM2-32GB, cc 7.0, one card only.
Container: the sm_70 vLLM image, CUDA 12.8 (nvcc V12.8.93), torch 2.10.0+cu128,
...
V100 host were md5-identical to branch `pxa/vllm-pxq4-sidecar`.
...
The host is shared. The cards reserved for other work were never touched; no container
was stopped, killed or removed; only `docker run --rm` pinned to a single free card with
`NVIDIA_VISIBLE_DEVICES`; free memory was checked before and after every run
(minimum seen 2601 MiB against a 1200 MiB abort threshold); nothing under
the model or checkpoint directories was deleted.
