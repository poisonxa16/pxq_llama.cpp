...
MTP speculative decoding was retested on a two-card Tesla V100 pair, TP=2,
against `Qwen3.8-27B-PXQ4-vllm-p2cf`. **It does not work, and cannot work with
any artifact produced so far**, because the converter deliberately drops
the MTP block. This note records the evidence, the two independent blockers hit
along the way, and exactly what a fix costs.

## Measured

`Qwen3.8-27B-PXQ4-vllm-p2cf` (18 GB, gemma-norm fixed), 2xV100, TP=2,
...
which is why the MTP measurement harness reports it and why it must be quoted
...
(4 here) and is not comparable to anything. Cross-check against the 4x V100 host
...
|---|---|---|---|
| `Qwen3.8-27B-PXQ4-vllm-p2cf` | 18 GB | **0** | 0..63 |
| `Qwen3.8-27B-PXQ4-vllm-p1f` | 22 GB | **0** | 0..63 |
| `Qwen3.8-27B-PXQ4-vllm` (broken norms) | 18 GB | **0** | 0..63 |
...
Neither defaults to those values once the fork's automatic sm_70 MTP defaults are
disabled, so both must be passed explicitly. Working speculative-config:
...
`config.json` still exist somewhere, this is minutes rather than
...
```
# non-MTP baseline
deploy/launch_v100.sh pxa-vllm-2v100 8420

# MTP4
EXTRA_ENV="-e VLLM_SM70_GEMMA_LONG_PREFILL_FUSED=0" MNS=1 \
  deploy/launch_v100.sh pxa-vllm-2v100-mtp 8421 \
  --speculative-config '{"method": "qwen3_5_mtp", "num_speculative_tokens": 4,
    "rejection_sample_method": "standard", "draft_sample_method": "probabilistic"}'

# then drive both ports with an OpenAI-compatible load harness that records
# acceptance rate and mean accept length from /metrics
```

The measurement harness gates on the 391 check either side of every timed run
and reports acceptance rate and mean accept length from `/metrics`.
