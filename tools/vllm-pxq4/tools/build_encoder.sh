...
#
# usage: build_encoder.sh <pxq_llama-src-dir> [OUT]
#   <pxq_llama-src-dir>  the src/ directory of a pxq_llama engine checkout; it must contain
#                        pxq6-quantize.inc.cpp and ggml-pxq6-tables.h. May also be given as
#                        $PXQ_LLAMA_SRC.
set -euo pipefail
MGV="${1:-${PXQ_LLAMA_SRC:-}}"
OUT="${2:-libpxq4_encode.so}"
[ -n "$MGV" ] || { echo "usage: build_encoder.sh <pxq_llama-src-dir> [OUT]" >&2; exit 2; }
HERE="$(cd "$(dirname "$0")" && pwd)"

for f in pxq6-quantize.inc.cpp ggml-pxq6-tables.h; do
  [ -f "$MGV/$f" ] || { echo "missing $MGV/$f — pass the pxq_llama src dir as \$1" >&2; exit 2; }
done
