...
WHY
  Two runtimes serve one quant family. pxq_llama runs on every card the PXQ
  family supports. vllm-pxq4 runs wherever its IMAGE has kernels for the card
  (see "ELIGIBILITY IS AN IMAGE PROPERTY", below) and brings real data
  parallelism that llama.cpp's `-sm layer` does not have. Choosing by hand means
  remembering which card is which, whether the model is dense or MoE, which PXQ
  tier is actually inside the file, and which of the two engines wins at the
  concurrency you actually serve at.
...
HOW TO READ EVERY CLAIM IN THIS FILE
  ...
  leaving the file:
    ...
    MEASURED     a number from a correctness-gated boot on the PXA reference
                 bench, carrying the id of the bench row that produced it.
  ...
  BENCH ROW IDS are stable labels for individual gated boots. They are quoted
  inline beside every number they back, so a claim can be traced to one run:

    D1 / D3           dense 27B PXQ4, 2x P100 sm_60 - D1 llama.cpp, D3 vLLM
    M1 / M4 / M7-M9   MoE 35B PXQ4, 2x P100 sm_60
    np1, np4..np8     the MoE crossover sweep - 11 gated boots, one 2x P100 pair

  Every boot behind a row was gated on short-prompt correctness BEFORE its number
  was kept. An ungated boot has no row and appears nowhere in this file.

  WHAT SHIPS ALONGSIDE, and what these claims can be checked against:
    docs/PXA-SM60-SERVING.md   reproduced sm_60 seat numbers + the shipping recipe
    docs/PXA-SM70-SERVING.md   the same for sm_70
    scripts/pxa-serve-sm60.sh  the recipes themselves
    scripts/pxa-serve-sm70.sh
    ...
    ggml/include/ggml.h        the ggml type ids this file dispatches on
  Line references into those source files are exact and re-checkable in this tree.

THE DECISION IS NOT JUST ABOUT THE HARDWARE

    DENSE 27B PXQ4, 2x P100 sm_60 -- vLLM wins everything measured
      single decode   vLLM 24.01  vs llama.cpp 13.7   (1.75x)   bench D3/D1
      agg decode @8   vLLM ~70    vs llama.cpp 12.4   (5.6x)    bench D3/D1
      prefill         vLLM ~225   vs llama.cpp 156.5  (1.44x)   bench D3/D1
      agg decode @4   vLLM UNMEASURED vs llama.cpp 12.0         bench D3
      ^ the llama.cpp side (D1) is ONE boot, below this bench's own 2-boot bar,
    ...
    MoE 35B PXQ4, 2x P100 sm_60 -- SPLIT SEAT, this is the important branch
      np1  llama.cpp 95.6  vs vLLM 30.4   llama.cpp 3.14x   bench M1/M7
      np4  llama.cpp 75.93 vs vLLM 64.82  llama.cpp +17.1%  crossover sweep
...
DESIGN NOTES - five places where the obvious implementation is wrong, and why
  (each was verified against this tree and against real model files first)

  N1. THE TIER IS READ FROM THE TENSOR DIRECTORY, NOT FROM A `pxa.pxq.*` KV.
      There is no generic `pxa.pxq.tier` key to read. `grep -n "gguf_set_val"
      src/llama-quantize.cpp` shows the only tier-bearing provenance KVs written
      are pxa.pxq6.{version,tier} (:1760-1767), pxa.pxq2.version (:1775),
      pxa.pxq3.version (:1780) and pxa.pxqu.version (:1790) - and PXQ1 writes NONE
      of them (":1506 comment: Fixed {-1,+1} book + the shared SUB16 -> no
      provenance KVs needed"). So a KV-only reader would leave PXQ1 - the one tier
      this launcher exists to refuse - invisible. The engine does not use KV
      either: it detects PXQ1 by TENSOR TYPE (src/llama-model-loader.cpp:527).
      This file does the same. Ground truth = the per-tensor ggml type histogram;
      the provenance KV is read too and any CONFLICT between the two is printed,
      never resolved silently. Verified against real artifacts: 5/5 PXQ GGUFs
      yield a tier this way, 0/5 through general.file_type (all report 38).

  N2. THE "22.3 -> 24.0" FAP-vs-FDO PAIR IS A MoE NUMBER, NOT A DENSE ONE.
      Rows M8/M9 sit in the MoE 35B PXQ4 table, not the dense one. Dense TP=2
      single-stream is D3 = 24.01, a different measurement, and there is NO dense
      FAP-vs-FDO pair anywhere - the graphs-ON dense arm (D2) was never launched.
      An earlier version of this file carried the same mislabel in its
      FULL_DECODE_ONLY comment. Fixed in both places.

  N3. "EVERY vLLM NUMBER IS sm_60" IS OVERSTATED. One sm_70 vLLM figure exists: a
      3.76x shared-prefix win measured on V100 (sm_70) hardware. What is true, and
      what the eligibility rule actually rests on, is narrower: NO vLLM
      DECODE-OR-PREFILL THROUGHPUT NUMBER on sm_70 exists at all. The comment at
      vllm_eligibility() states the narrow version.

  N4. THE FIT CHECK CANNOT BLOCK ON A KV ESTIMATE - so it does not pretend to.
      The KV-per-token figure is arithmetic over header fields and is unvalidated
      against a real allocation on every arch but one, so it may not block
      anything. R-17 blocks ONLY on facts that need no formula: ctx > n_ctx_train
      (a KV field), and GPU-resident weights alone > total VRAM of the selection
      under full offload (arithmetic on file bytes). Everything that needs the
      per-token estimate warns and says [INFERRED]. A real 42.93 GB PXQ4 file
      (Fusion-Coder-80) is refused by the weights-only clause on 2 cards.

  N5. THE `-sm graph` GUARD IS STRUCTURAL, NOT A NAME LIST. The hazard is a
      property of the DeltaNet tensors, so this file refuses on `linear_attn.*` /
      `ssm_*` presence OR the arch allowlist, whichever fires. That closes the
      hole where a hybrid arch nobody thought to list slips through - while a
      model that is on neither trigger stays allowed on evidence rather than on a
      name.

ELIGIBILITY IS AN IMAGE PROPERTY, NOT A COMPUTE CAPABILITY
  An earlier version set MIN_VLLM_CAP=70 and routed any selection containing a
  sub-sm_70 card to llama.cpp. Every vLLM decode/prefill number in the decision
  table above was produced on 2x P100 sm_60 - bench D3, M7-M9 and the whole
  crossover sweep, on image pxa-sm60-dev with libpxq4_sm60_v10.so and
  --attention-backend PASCAL_SDPA. So that table could never reproduce a single
  one of its own vLLM cells, and the np>=6 vLLM branch was dead on the only
  hardware where the crossover was ever measured. Eligibility is a property of
  the resolved IMAGE, probed. See vllm_eligibility().
"""
...
# HARDWARE FACTS
# ---------------------------------------------------------------------------
# cc -> human card class. sm_61 is NOT folded into "Pascal" here: the bench has
...
# (docs/LEVERS.md §3). Folding it into a Pascal set is how it stops being visible.
CARD_CLASS = {60: "P100-class sm_60", 61: "GTX 1080 Ti-class sm_61", 70: "V100-class sm_70"}

# MEASURED, hardware-verified (docs/LEVERS.md §4, ADAPTIVE-UB fallback table):
...
# ---------------------------------------------------------------------------
# MEASURED 2026-08-24, 11 gated boots on one 2x P100 sm_60 pair, PXA-Coder-35B-v2
# PXQ4, -c np*4096, raw /completion (NOT chat-templated), every boot gated on
# short-prompt correctness BEFORE its number was kept.
# Bench rows: the np4..np8 crossover sweep.
...
# The crossover sweep measured 95.81 at np8 on a newer engine revision, where bench
# M7 has 88.7 on an older one (+8.0%) for the same cell. The intervening change is a
# HYPOTHESIS, not a measurement, so every vLLM MoE number here may now be stale in
# vLLM's favour. If it is stale the crossover could sit BELOW np=6 - i.e. this can
# change an engine decision, not just a number.
MOE_CURRENCY_NOTE = ("vLLM MoE currency: 95.81 (newer engine revision) vs 88.7 (older) = +8.0% "
                     "on the same cell, cause hypothesised and NOT tested. If the newer number "
                     "is real the crossover may sit BELOW np=6. [crossover sweep]")

# MEASURED dense, 27B PXQ4, one 2x P100 sm_60 pair (bench rows D1/D3).
DENSE_NUMBERS = {
    "single":  ("24.01", "13.7",  "1.75x"),
    "agg8":    ("~70",   "12.4",  "5.6x"),
    "prefill": ("~225",  "156.5", "1.44x"),
}
DENSE_AGG4_NOTE = ("dense agg@4 on vLLM is UNMEASURED (bench D3); llama.cpp side is 12.0. "
                   "No ratio is printed for np=4 because none was measured.")
DENSE_WEAKNESS = ("dense envelope: the llama.cpp side (bench D1) is ONE boot, below this "
                  "bench's own 2-boot bar, and the graphs-ON dense arm (D2) was never launched. "
...
MOE_LONGDOC_NOTE = ("long-doc prefill: llama.cpp 1136 / ~1058 / ~1000 vs vLLM 567.6 / 595.8 / "
                    ...
                    "(2059 vs ~6.4k tok) [bench: long-doc prefill arm]. Directionally trusted, not controlled.")

# The anchor models. The crossover is a property of a model x hardware PAIR, not
# of the engines - the other MoE arches this engine serves (qwen3next MoE-512,
# qwen3moe MoE-128, deepseek4 MoE-6) have NO engine-vs-engine data at any np.
ANCHOR_MOE_HINTS   = ("coder-35b", "coder35", "pxa-coder-35b")
ANCHOR_DENSE_HINTS = ("27b", "qwen38-27b", "qwen3.8-27b")
ANCHOR_CTX_PER_SLOT = 4096      # MEASURED envelope: --ctx-size np*4096 (crossover sweep)

# ---------------------------------------------------------------------------
# PXQ TIERS - ggml TENSOR TYPE IDS (ground truth; see DESIGN NOTE N1)
# ---------------------------------------------------------------------------
...
# 248, and PXQ1C/PXQ2C appear nowhere in the tree. VERIFIED on a real artifact: a
# file, Fusion-Coder-80-PXQU.gguf, carries 106 tensors of type 247 and 38 of type
# 246 plus pxa.pxq1c.* / pxa.pxq2c.*
...
# writing it at :1658. VERIFIED over a 138-file library: 138/138 GGUFs report 38 or a K-quant
...
# vLLM implements exactly one PXQ tier. Per the tier-support matrix in
# docs/VLLM.md: on the vLLM backend only PXQ4 is supported; every other tier is
# refused cleanly at the conversion gate, so no silent wrong output exists on
# the vLLM path.
VLLM_SUPPORTED_PXQ = {"PXQ4"}
# Tier-support matrix (docs/VLLM.md) - these run on llama.cpp only.
LLAMA_ONLY_PXQ = {"PXQ4-HQ", "PXQ6", "PXQ3", "PXQ2", "PXQ_UNIVERSAL"}
# No CPU codec is implemented for these tiers yet: GPU-only (docs/VLLM.md).
...
# DESIGN NOTE N5: the arch names are one of TWO triggers; the structural one
...
#
# NOT EVERY IMAGE IS SELF-CONTAINED. An image can be a bare CUDA runtime whose
# python, torch and vllm all live on the HOST and are bind-mounted at run time -
# `pip list` inside it shows pip and nothing else. Every number attributed to
# such a tag was really produced by the image PLUS those host paths, and
# recording that is not pedantry: without it the launcher declares the image
# eligible on a machine where the venv has moved, and emits `vllm serve`, which
# is not even on PATH in that container.
#
# Those paths are SITE-LOCAL, so none are hardcoded here. Declare them in a JSON
...
#
#   mounts           host dir -> container dir. The model path is translated
#                    through the longest matching prefix before the command is
#                    emitted; a model outside every mount is called out, because
#                    a path that is correct on paper and absent in the container
#                    surfaces as a load failure minutes later instead of as a
#                    sentence here.
#   requires         host entries that MUST exist or the image is declared
#                    ineligible. Checked with lexists, NOT exists: venv/bin/python
#                    is typically a symlink to an interpreter that exists only
#                    INSIDE the container, so exists() calls a perfectly good venv
#                    missing and refuses a seat that would have served.
#   python           the interpreter to invoke inside the container. `vllm serve`
#                    is WRONG for this class of image - vllm is not on PATH there.
#   env              env the host runtime needs to be importable (PYTHONPATH, the
#                    PXQ4 kernel library). Merged with setdefault, so a value from
#                    a measured arm always wins over the image's default.
#   editable_source  if vllm is an editable install, the working tree it resolves
#                    to. The seat then imports whatever is checked out there right
#                    now: edit that tree, or switch its branch, and the running
#                    engine changes with no redeploy and no version to notice it
#                    by. The launcher prints that tree's branch, sha and
#                    dirtiness so a measurement can be attributed to something.
#
# Only tags listed below are ever eligible. An image this launcher does not know
# has an UNMEASURED arch set and gets no card, whoever built it.
VLLM_IMAGES = {
    "pxa-sm60-dev": {
        "caps": {60}, "status": "MEASURED",
        # NOT SELF-CONTAINED - see the PXA_VLLM_HOST_ENV note above. Without a
        # declared host environment this tag yields NO eligible card, because the
        # measurements it carries were produced by the image plus a host runtime
        # that is not described by the tag.
        "needs_host_env": True,
        "why": "produced every MoE-crossover vLLM number (libpxq4_sm60_v10.so, "
               "--attention-backend PASCAL_SDPA). v10, NOT v8 (stale: both shipping "
               "launchers moved to v10) and NOT v11 - v11 exists and is FASTER on "
               "prefill with PXQ4_GEMM2D=1 (300.1 tok/s, +37%) but FAILS raw-prompt "
               "correctness, and a prefill win that changes what the model says is "
               "not a win"},

    # THE FAT IMAGE IS WITHDRAWN. One image spanning sm_60+sm_70 was tried and does
    # not work: 8 boot attempts on a Tesla V100, 8 failures. The cause is
    # structural, not configuration - VLLM_SKIP_C_STABLE=1 is required to build
    ...
    # lives. You cannot have sm_60 cubins and that op in the same build. Hence two
    # thin images, each pinned to the torch its cards need. Do not reintroduce a
    # fat tag.

    # THE SHIPPING SET: two thin images, each built from our own tree, each pinned
    # to the torch its cards need. No third-party image is eligible.
    "pxa-vllm:sm60": {
        ...
        "why": "Pascal variant: torch 2.7.1 (last torch shipping sm_60 cubins), "
               ...
               "until a gated boot on a P100 pair passes against THIS tag"},
    "pxa-vllm:sm70": {
...
ATTN_BACKEND = {
    ...
    70: ("FLASH_ATTN_V100", "[INFERRED] - from the shipping V100 serving recipe "
                            "(scripts/pxa-serve-sm70.sh, docs/PXA-SM70-SERVING.md); no "
...
def host_env_descriptors():
    """Site-local host-environment descriptors, read once from the JSON file named
    by PXA_VLLM_HOST_ENV. -> {image_tag: {mounts, requires, python, env,
    editable_source, why}}.

    Never raises - a launcher must not die on a config file it was merely trying to
    read - but never silently succeeds either: a missing or unparseable descriptor
    is reported and then treated as ABSENT, which REFUSES rather than proceeding on
    a half-read file."""
    if _HOST_ENV_CACHE[0] is None:
        d = {}
        p = os.environ.get("PXA_VLLM_HOST_ENV", "")
        if p:
            try:
                ...
                if isinstance(loaded, dict):
                    ...
                    print(f"pxa-launch: PXA_VLLM_HOST_ENV={p} is not a JSON object - "
                          f"no host environment is declared", file=sys.stderr)
            except Exception as e:
                print(f"pxa-launch: PXA_VLLM_HOST_ENV={p} unreadable "
                      f"({e.__class__.__name__}) - no host environment is declared",
                      file=sys.stderr)
        _HOST_ENV_CACHE[0] = d
    return _HOST_ENV_CACHE[0]


def host_env_for(tag, rec=None):
    """-> the host_env declared for this image tag, or {}. A descriptor entry
    overrides an in-table default key by key, so a site can move one path without
    restating the whole record."""
    he = dict(((rec or {}).get("host_env") or {}))
    ext = host_env_descriptors().get(tag)
...
def resident_procs(gpus):
    """-> {gpu_index: [(pid, name, mib), ...]}. This may be a SHARED, LIVE box; the
...
def peer_topology():
    """-> (has_p2p, description). The bench pair is all-PHB, no NVLink, no P2P,
    which is WHY custom all-reduce is off in every measured vLLM arm
    (--disable-custom-all-reduce). MEASURED: custom all-reduce costs ~18% vs NCCL
    on MoE without P2P, while the CAR KERNEL itself is exonerated. We READ the
    topology; we do not hardcode the answer."""
...
# ---------------------------------------------------------------------------
# MODEL INTROSPECTION - header-only reads, no tensor data, no GPU
# ---------------------------------------------------------------------------
...
def gguf_header(path, max_tensors=200000):
    ...
    is not a usable GGUF - which is a REFUSAL (R-23), not a shrug: a truncated
    5.3 GB PXQ4 file whose first 4 bytes are 00 00 00 00 is a shape that really
    occurs, and the old launcher built a full command for it."""
...
def tier_from_tensors(tensors):
    """Tier detection, done the way the ENGINE does it (src/llama-model-loader.cpp:527 detects
    ...
    failure as a uniform PXQ1 file (docs/VLLM.md, tier-support matrix)."""
...
def model_kind(path):
    """Artifact taxonomy. gguf | gguf_broken | vllm_dir | hf_dir | lora_dir | weightless_dir |
    ...
    launcher told the operator a path that plainly exists does not exist. Model
    directories are full of tempting single files next to real models (one shard of
    six, a .tiers map, an .imatrix, a LoRA adapter)."""
...
def model_profile(path, kind):
    ...
    if kind in ("gguf", "gguf_broken"):
        ...
        # MTP by TENSOR WALK, never by the KV flag. Two shipped f16 files declare
        ...
        # DESIGN NOTE N5: structural DeltaNet detection, and it needs BOTH namings.
        # Detecting the hybrid by "presence of linear_attn.* tensors" alone is dead
        # code on the only format llama.cpp - the engine -sm graph applies to -
        # actually serves. linear_attn.* is the SAFETENSORS name (a PXQ4 MoE
        # checkpoint's quantization_config.pxq4_modules carries
        # linear_attn.in_proj_qkvz). In GGUF the SAME layers are named ssm_*:
        # Fusion-Coder-80-P100-PXQ4.gguf (arch qwen3next) has blk.N.ssm_a, ssm_ba,
        # ssm_conv1d, ssm_dt, ssm_norm, ssm_out and ZERO tensors matching
        # linear_attn. Both namings are checked here.
...
def kv_bytes_per_token(kv, arch):
    """[INFERRED] - this is ARITHMETIC, not a measurement.
    ...
    model, every other MoE arch and every GQA ratio. Neither the old constant nor
    this formula has been validated against a real allocation on any arch but one,
    so NEITHER MAY BLOCK ANYTHING. It warns."""
    try:
        ...
        if arch == "qwen4exp":
            tag = ("MEASURED 2026-08-28: predicts 24576 B/token and the engine allocated "
                   ...
                   "Three exact hits - validated for this arch only.")
        elif kv_layers != n_layer:
            tag = ("[INFERRED] hybrid-attention correction applied from the qwen4exp "
                   "measurement, but NOT validated on this arch - warn only, never "
                   "blocks.")
        else:
            tag = ("[INFERRED] UNVALIDATED against a real allocation - warn only, "
                   "never blocks")
...
def kv_bytes_per_token_hf(cfg):
    """KV bytes/token for a safetensors checkpoint. Same [INFERRED] status as the
    GGUF form: arithmetic on config fields, UNVALIDATED against a real allocation,
    ...
    try:
        ...
        return b, (f"[INFERRED] arithmetic: {n_layer} layers x {n_kv} kv-heads x {d_h} head-dim "
                   ...
                   f"allocation - warn only, never blocks")
    except Exception:
...
def find_mmproj(model_path, kind):
    """A paired projector is "discoverable next to the model" - but nothing says
    how, and the ambiguous shape is real: one model directory can hold TWO F16
    projectors, and a sibling -mmproj directory an F16 AND a Q8_0 for the same
    base model. There is no rule that picks a winner, so this does not invent one:
...
def vllm_eligibility(sel, image_arg):
    ...
    true statement (DESIGN NOTE N3): NO vLLM decode-or-prefill THROUGHPUT number on
    sm_70 has ever been taken - the only sm_70 vLLM figure on record is a 3.76x
    shared-prefix win measured on V100 sm_70 hardware, which is not a seat decision.
    ...
    if img:
        ...
        if rec["status"] == "INELIGIBLE":
            ...
            return set(), img, trail
        henv = host_env_for(img, rec)
        if rec.get("needs_host_env") and not henv:
            # NOT SELF-CONTAINED and nothing declares where its runtime lives. The
            # measurements this tag carries were produced by the image PLUS a host
            # runtime, so the tag alone earns no card.
            trail.append(f"probe 1: image {img!r} is NOT SELF-CONTAINED - its python, torch "
                         f"and vllm live on the HOST - and no host-environment descriptor was "
                         f"declared for it, so no card is eligible under it. Point "
                         f"PXA_VLLM_HOST_ENV at a JSON descriptor for this tag (see "
                         f"VLLM_IMAGES for its shape).")
            return set(), img, trail
        missing = [q for q in henv.get("requires", []) if not os.path.lexists(q)]
        if missing:
    ...
    # "available" has to MEAN available here. This line previously printed every
    ...
    trail.append("gated images PRESENT on this host: " + (", ".join(present) or "NONE"))
    if absent:
...
# ---------------------------------------------------------------------------
# REFUSALS - R-01..R-21 are the original set; R-22..R-29 were added by review
# (each comment says which hole it closes).
...
R = {
 "R-01": ("REFUSING: PXQ1 content. A PXQ1 MoE file loads, clears the composition gate at 80.9% "
          ...
          "(docs/VLLM.md, tier-support matrix). PXQ1 has no dense path and no CPU codec. "
 ...
 "R-03": ("REFUSING to guess the PXQ tier. general.file_type is {ftype} for every PXQ tier by "
          ...
          "this launcher cannot enforce the PXQ1 refusal or the vLLM PXQ4-only gate. Re-quantize with a "
 ...
 "R-06": ("REFUSING: vLLM needs a converted artifact and {conv} does not exist. Convert: "
          "cd tools/vllm-pxq4/src && python -m gguf_to_vllm.convert {model} {conv}. Or "
          "--engine llama to serve the "
          "GGUF directly."),
 "R-07": ("REFUSING: forced --engine vllm but NO selected card is eligible under image {img} "
          ...
          "CUDA_VISIBLE_DEVICES is never set, and the server inherits EVERY GPU on the host. "
 ...
 "R-08": ("REFUSING: cudagraph_mode={mode}. FULL_AND_PIECEWISE captures PREFILL graphs and "
          ...
          "best aggregate (88.4, bench M8) is BELOW the correct config's (88.7, M7). There "
 ...
 "R-13L": ("REFUSING: -ctk {k} / -ctv {v} has no compiled FA vec kernel at head 128 on this "
           ...
           "here: q8_0/q6_0, q8_0/iq4_nl, q6_0/q5_0 (docs/LEVERS.md §4). NOTE: NO measurement "
 ...
 "R-14": ("REFUSING: --spec mtp with vLLM. The vLLM path has no MTP drafter, and this launcher will not "
          "substitute ngram for it - on this model class the two have OPPOSITE verdicts (ngram "
          "+23.0% code / +4.6% prose; MTP -8.6% at n_max=1 and -29.8% at n_max=2, docs/LEVERS.md §5). "
 ...
 "R-16": ("REFUSING: {tier} has no CPU codec - it is GPU-only, and no CPU codec for it is "
          "implemented yet. A CPU-only or partially-offloaded run ABORTS. You asked for "
 ...
 "R-18": ("REFUSING: {dir} is an unquantized HF checkpoint (config.json has no "
          ...
          "directory. Convert first - this launcher will not emit 'vllm serve --quantization pxq4' against "
          "fp16 weights."),
 ...
 "R-21": ("REFUSING: np={n} needs a cudagraph capture ladder above the measured [1,2,4,8]. This launcher can "
          "widen it, but nothing above np=8 has been measured on either engine and a too-short "
          "ladder has cliffed before: a hardcoded [1,2] ladder cliffed at 3+ concurrent. "
          "Re-run with --accept-unmeasured."),
 # ---- added by review, beyond the original R-01..R-21 set ----
 ...
 "R-23": ("REFUSING: {path} is not a usable GGUF - {detail}. A truncated 5.3 GB file with a "
          "zeroed header is a shape that really occurs, and the previous version of this file "
          "built a full launch command for it."),
 ...
 "R-25": ("REFUSING: --draft-model (external draft-model speculation) has ZERO coverage in this "
          "bench - no decode, prefill, acceptance or quality number exists for it on any cell. "
 ...
 "R-27": ("REFUSING: this is a multimodal/VL checkpoint (vision_config / vision tensors present) "
          ...
          "--limit-mm-per-prompt, no image-token accounting, no projector. Nothing in the bench "
 ...
 "R-28": ("REFUSING: this file carries {n} tensor(s) of ggml type id(s) {ids}, which the CURRENT "
          ...
          "in the tree). The engine cannot dispatch them. A real file is in exactly "
          "this state: Fusion-Coder-80-PXQU.gguf, 106 tensors of type 247 "
...
def envelope_notes(plan, sel, prof, np_, per_slot_ctx, model_path):
    """The whole decision table is keyed to TWO CARDS OF ONE CLASS. Outside that
    envelope the launcher labels the answer and, where this file says so, requires
    --accept-unmeasured. MEASURED evidence that even 2->2 does not transfer: 22.3 vs
    24.6 tok/s on an IDENTICAL config between two different P100 pairs."""
    ...
    if n == 2 and caps == [60]:
        ...
        plan.notes.append("[INFERRED]: 2x sm_70. NO engine-vs-engine number exists on Volta at "
                          "all, for either class. The table below is applied, not measured "
                          "here.")
        ...
        plan.notes.append("mixed-class pair: PXA_AUTO_TS fills -ts 1.4,0.6 on an exactly-2-device "
                          ...
                          "ONE cell only (docs/LEVERS.md §4, 'PXQ4-35B split V100+P100'); it does not "
                          "generalize.")
    if n not in (1, 2):
        plan.notes.append(f"UNMEASURED card count: the MoE crossover and the dense pair were both "
                          ...
                          f"printed and labelled [INFERRED]. MEASURED: 22.3 vs 24.6 on an "
                          f"identical config between two different P100 PAIRS - even 2->2 does "
                          f"not transfer cleanly.")
    ...
    if 61 in caps:
        plan.notes.append("selection contains an sm_61 card: the np5/np6 thresholds were NEVER "
                          ...
                          "explicitly excludes all of sm_61 (docs/LEVERS.md §3). UNMEASURED.")
    ...
    if prof.get("is_moe"):
        ...
        if not any(h in base for h in ANCHOR_DENSE_HINTS):
            plan.notes.append("model is not the dense anchor (Qwen3.8-27B): the dense ratios "
    ...
    if len(ubs) > 1:
        plan.notes.append(f"HETEROGENEOUS -ub expectation across this selection: the card-type "
                          f"table (docs/LEVERS.md §4) wants {sorted(ubs)} on different cards while "
...
def decide(sel, kind, model, forced, prof, np_, workload, elig_caps, image, probe_trail,
    ...
    """-> Plan. Refusals first, always: what CANNOT run is settled before what is fastest."""
    p = Plan()
    p.image = image
    prof = prof or {}
    ...
    if isinstance(ft, int) and ft in RETIRED_FTYPE:
        ...
        return p
    if tier == "PXQ1":
        # Positive detection by tensor type, so this fires on a uniform PXQ1 file
        # AND on a curated UNIVERSAL map with PXQ1-mapped experts - the HIGH-severity
        # silent case a tier-NAME condition leaves unguarded, because
        # `tier in {PXQ1}` never matches a PXQ_UNIVERSAL tag.
        p.refuse("R-01")
        return p
    ...
    if tier == "PXQ_UNIVERSAL":
        p.notes.append("PXQ_UNIVERSAL: this is a MIXED per-tensor tier map. "
                       "Tier-support matrix finding 8 records a UNIVERSAL MoE that loads PASS "
        ...
        if forced_v:
            if tier and tier not in VLLM_SUPPORTED_PXQ:
                p.blockers.append(f"FORCED vllm but this model is {tier} and the vLLM backend "
                                  f"implements PXQ4 only (docs/VLLM.md)")
            if kind == "gguf":
    ...
    if tier and tier not in VLLM_SUPPORTED_PXQ and tier != "PXQ4":
        p.engine, p.reason = "llama", (
            ...
            f"cleanly at the conversion gate (docs/VLLM.md). llama.cpp reads "
            f"PXQ2/PXQ3/PXQ4/PXQ4-HQ/PXQ6/UNIVERSAL. Cards: {names}")
        p.evidence.append("tier support: the PXQ tier-support matrix in docs/VLLM.md")
        ...
        p.engine, p.reason = "llama", (
            ...
            f"(tools/vllm-pxq4/src, python -m gguf_to_vllm.convert). Cards: {names}")
    elif not p.elig:
        p.engine, p.reason = "llama", (
        ...
        p.engine, p.reason = "llama", (
            ...
            f"overhead. This is also the only measured single-card ground in the bench "
            f"(the single-card bench arm, 1x P100). Cards: {names}")
    else:
        ...
        if not prof.get("is_moe"):
            ...
            p.evidence.append("MEASURED dense: bench rows D1 (llama.cpp) / D3 (vLLM), "
                              "27B PXQ4, 2x P100 sm_60")
            ...
            p.evidence.append("MEASURED MoE longdoc: the long-doc prefill arm - cross-harness, "
                              "see the note")
        else:
...
def moe_seat(np_, names):
    ...
    if np_ <= 1:
        l, v, w, m = MOE_NP1
        ev.append(f"MEASURED np=1: llama.cpp {l} vs vLLM {v} tok/s ({m}) [bench M1/M7]")
    ...
    if np_ in MOE_TABLE:
        ...
        ev.append(f"MEASURED np={np_}: llama.cpp {l} vs vLLM {v} tok/s, {w} by {m} "
                  f"[crossover sweep, 11 gated boots, one 2x P100 pair]")
        if np_ == 5:
...
# ---------------------------------------------------------------------------
# VRAM - see DESIGN NOTE N4. Only formula-free facts may block.
# ---------------------------------------------------------------------------
def vram_check(plan, sel, mbytes, ctx, prof, ngl_all):
    ...
    if kvb:
        ...
        if need > free:
            notes.append("TIGHT FIT by that estimate. It is arithmetic, not a measurement - KV "
                         "bytes/token has not been measured per arch family - so it WARNS and "
                         "does not block. Reduce -c or add cards if the boot OOMs.")
        ...
        # llama_decode - the split had left one card with 51 MiB. Decode allocates
...
# ---------------------------------------------------------------------------
# There is no site-specific build list here on purpose: a hardcoded absolute path
# is a machine's private detail, and a launcher that carries one is a launcher
# that works on exactly one machine. Resolution order:
#
#   1. PXA_ENGINE_DIR (alias: PXQ_ENGINE_DIR) - an explicit operator override,
#      pointing at the directory that CONTAINS bin/llama-server. It wins over
#      everything, and if the build there will not start that is REPORTED rather
#      than quietly stepped over: an override that gets ignored is how a
#      measurement ends up attributed to a build nobody selected.
#   2. build directories inside this repository, resolved relative to THIS FILE -
#      the ordinary case for someone who just built the tree they are standing in
#      (and for out-of-tree builds, the same names as siblings of the checkout).
#   3. common install prefixes, for a packaged or `cmake --install`ed engine.
#   4. llama-server on PATH.
#
# Every candidate that exists but will not START is collected and printed with the
# reason, never skipped in silence.
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# Build-directory names this tree's presets and docs produce.
BUILD_DIR_NAMES = ("build", "build-cuda", "build-unified", "build-release",
                   "build-sm60", "build-sm70", "build-all")

# Prefixes a package or a manual install would land in. Overridable wholesale by
# PXA_ENGINE_DIR; this list only decides where to LOOK, never what to run.
INSTALL_PREFIXES = ("/usr/local", "/usr", "/opt/pxa", "/opt/pxq_llama")


def engine_dir_candidates():
    """Ordered, de-duplicated build-directory candidates. Pure path arithmetic, no
    filesystem probing - so the order is auditable and testable on a host that has
    none of them."""
    out = []
    ...
    for n in BUILD_DIR_NAMES:
        add(os.path.join(REPO_ROOT, n))
    parent, base = os.path.dirname(REPO_ROOT), os.path.basename(REPO_ROOT)
    for n in BUILD_DIR_NAMES:                       # out-of-tree: ../<repo>-build*
        add(os.path.join(parent, f"{base}-{n}"))
    for p in INSTALL_PREFIXES:
        add(p)
    add(os.path.expanduser("~/.local"))
    return out
...
def engine_ld_path(E):
    ...
    AND: any inherited /stubs directory is STRIPPED. MEASURED: a 66 KB stub
    ...
    numerically CORRECT and ~50x SLOWER. A CUDA toolkit install puts that stubs
    directory on LD_LIBRARY_PATH routinely, so this is not a hypothetical.
...
def resolve_engine_dir():
    tried = []
    for var in ("PXA_ENGINE_DIR", "PXQ_ENGINE_DIR"):
        env = os.environ.get(var)
        if env:
            # An explicit override is honoured even when it is broken, and the
            # breakage is named. Falling through to auto-detection here would run a
            # build the operator did not ask for and never say so.
            ok, why = engine_runs(env)
            return env, (var if ok else f"{var} -- WILL NOT START: {why}")
...
# ---------------------------------------------------------------------------
# MEASURED 2026-08-28 on Flash-Next PXQU over a heterogeneous five-card pool.
#
# WHY THIS EXISTS: an even split is the default and it is WRONG on any pool where
# the cards are not equally free. In that pool one 16 GiB card was already carrying
# another seat (7865 of 16384 MiB free) and one was an 11 GiB 1080 Ti also in use.
# An even five-way split puts ~9.2 GiB of weights on a card with 7.8 GiB free.
#
...
# left one card with 51 MiB. Decode allocates transient buffers that the init-time
...
def build_llama_cmd(plan, a, sel, prof, ctx, ub_expect, mmproj, explain=False):
    ...
    if E is None:
        ...
        if "NO CUDA RUNTIME ON THIS HOST" in note:
            ...
            print("     A CUDA build needs a CUDA runtime: run pxa-launch inside a CUDA")
            print("     container with the build visible to it, or set PXA_ENGINE_DIR.")
        else:
    ...
    if dropped:
        print(f"  LD_LIBRARY_PATH: DROPPED {dropped} - a CUDA stubs dir on the child's library "
              ...
              f"numerically correct output. [MEASURED]")
    ...
    # table (docs/LEVERS.md §4). One global -ub across a heterogeneous pool is wrong
    ...
    if a.ub:
        ...
        print(f"  -b/-ub: NOT PASSED - adaptive-ub probes each device at startup. Card-type "
              f"table (docs/LEVERS.md §4) expects {ub_expect} on this selection. Verify against "
    ...
    # PXA_ENHANCE=1 is the anchor arm's env, verbatim (crossover sweep, arm A).
    ...
    # bench; the previous version of this file set it unconditionally and its effect
    # on the anchors is UNMEASURED. Never set a lever the anchor was not measured
    # with.
    return cmd, env


MEASURED_LADDER = [1, 2, 4, 8]      # MEASURED; the fix for the [1,2] cliff at 3+ concurrent
...
def compilation_config(np_):
    ...
    means the two correctness keys cannot be lost on some branch:
      ...
      custom_ops:["none"]  is MANDATORY wherever FULL_DECODE_ONLY is emitted on
        ...
        determine_available_memory -> profile_run, on a crossover-sweep boot that
        produced ZERO tokens. Adding the key fixed it with no other change, and every
        working recipe carries it. The previous version of this file carried it
        NOWHERE while emitting FDO - i.e. it emitted exactly the command proven twice
        not to boot.
      ...
      cudagraph_mode FULL_DECODE_ONLY is a CORRECTNESS requirement, not a knob.
        ...
        config's best aggregate 88.4 (bench M8) is BELOW the correct config's
...
def build_vllm_cmd(plan, a, prof, ctx, used, image):
    ...
    while deg * 2 <= len(used):
        deg *= 2
    if deg != len(used):
        # vLLM rejects a non-power-of-two degree at startup. Truncating is a
    ...
    # python lives on the host: vllm is not on PATH inside such an image, so the
    # emitted command could not start, and in non-explain mode this file exits 4 on
    # the shutil.which check - after printing a full, confident plan. An image that
    # declares a host python gets invoked through it, the same way the boot that
    # produced the measurements did.
    hostenv = host_env_for(image, VLLM_IMAGES.get(image))
    hpy = hostenv.get("python")
    if hpy:
        # The model path has to be translated through the same mount the interpreter
        # came through. This launcher runs INSIDE the container, where the host model
        # root does not exist - only its mount point does. Emitting the host path
        # produces a command that is correct on paper and cannot find its weights,
        # which surfaces as a load failure minutes in rather than as a sentence here.
    ...
    # No P2P -> custom all-reduce OFF, as in every measured vLLM arm. MEASURED: CAR
    # costs ~18% vs NCCL on MoE without P2P, while the CAR kernel itself is
    # exonerated. Read from topology, not hardcoded.
    if not plan_has_p2p():
        cmd += ["--disable-custom-all-reduce"]

    # MoE goes PIPELINE parallel; dense goes TENSOR parallel. PP=2 is the arm that
    # holds every MoE number in the table (crossover sweep, arm B).
    ...
    #
    # custom_ops:["none"] - the single most dangerous omission in the previous
    #   version of this file. PP=2 + FDO WITHOUT this key is a HARD BOOT FAILURE:
    #   "RuntimeError: Worker failed with error 'CUDA error: an illegal memory access
    #   was encountered'", Worker_PP1 -> determine_available_memory -> profile_run,
    #   on a crossover-sweep boot that produced ZERO tokens. Adding the key fixed it
    #   with no other change, and every working recipe carries it. `grep -n
    #   custom_ops` on the old file returned 0 hits while it emitted FDO - i.e. it
    ...
    #
    # cudagraph_mode:FULL_DECODE_ONLY - a CORRECTNESS requirement, never a
    ...
    #   MEASURED: the broken config's BEST aggregate (88.4, bench M8) is BELOW the
    #   correct config's (88.7, M7). DESIGN NOTE N2: the 22.3 -> 24.0 single-stream
    #   pair that used to be quoted here as "dense" is a MoE pair (bench M8/M9, both
    #   in the MoE table). Dense TP=2 single is D3 = 24.01 and has NO FAP arm at all.
    #
    # cudagraph_capture_sizes - powers of two covering --max-num-seqs. [1,2,4,8] is
    #   the MEASURED ladder and the fix for an earlier [1,2] ladder that cliffed at 3+
    #   concurrent. Widening past 8 is [INFERRED]: the precedent says a too-short
    ...
    # model class (ngram +23.0% code vs MTP -8.6%/-29.8%, docs/LEVERS.md §5).
    ...
    env = {"TORCHDYNAMO_DISABLE": "1",           # MEASURED crossover-sweep arm B env
           "VLLM_USE_BREAKABLE_CUDAGRAPH": "1"}  # MEASURED crossover-sweep arm B env
...
# ---------------------------------------------------------------------------
# POST-BOOT VERIFICATION CONTRACT (R-09)
# ---------------------------------------------------------------------------
def print_post_boot_contract(engine, cv):
    ...
    took effect: an image can PARSE FULL_DECODE_ONLY, boot healthy, and SILENTLY
    OVERRIDE it back to FULL_AND_PIECEWISE from its own compile policy - observed on
    a third-party sm_70 image. That is the whole failure class this keeps costing."""
    ...
    print("       completion BEFORE any number is trusted, exactly as all 11 boots of the")
    print("       crossover sweep did. Chat-templated traffic pads every prompt")
...
def selftest(gpus):
    ...
    print("    decision table: bench rows D1/D3 (dense), M1/M4/M7-M9 and np4..np8 (MoE)")
    if gpus:
        for g in gpus:
    ...
    # Run the table twice: once with eligibility as this host actually resolves it,
    # once with the MEASURED sm_60 image forced. On a host where no image resolves,
            ...
            for plabel, pkind, prof in profiles:
                for np_ in (1, 4, 5, 6, 8):
                    ...
                    print(f"  {label:18} {plabel:11} {pkind:9} np={np_:<2} -> {out:11} :: "
                          f"{rest[:48]}{ack}")
    print(f"  MoE table (MEASURED, 2x P100 sm_60): "
    ...
    #     too - the case a `tier in {PXQ1}` name check could never match.
...
def main():
    ...
    # R-20: this may be a shared, live box. Do this AFTER the decision so the operator
    ...
    if m == "mtp":
        ...
        if nmax == 0:
            # A bare 'mtp' used to expand to n_max=4,n_min=2 - a MEASURED loss
            ...
            print("  --spec mtp expanded to mtp:n_max=1. MEASURED: n_max>=2 LOSES on both arches "
                  ...
                  "docs/LEVERS.md §5). The previous version of this file expanded a bare 'mtp' to "
        ...
        if prof.get("mtp_tensors") == 0:
            ...
            print("  MTP: PXA_MTP_LAZY_WARMUP is armed by PXA_ENHANCE and is MANDATORY whenever "
                  "MTP is active - without it MTP costs -33% prefill (docs/LEVERS.md §5). "
                  "PXA_MOE_FASTTG_MAX_NY is left at its shipped 8; =1 with MTP verify measured "
                  "48.1 -> 30.3 on P100 (docs/LEVERS.md §5).")
            if prof.get("is_moe"):
                print("  MTP on a sparse MoE is a MEASURED LOSS even at n_max=1: -8.6% (n_max=1), "
                      "-29.8% (n_max=2) despite 0.800 acceptance (docs/LEVERS.md §5). You asked for "
    ...
    # ---- VRAM (only formula-free facts may block; see DESIGN NOTE N4) ---
    ...
    if plan.engine == "llama" and not mmproj and not a.no_mmproj:
        ...
        if prof.get("vision"):
            ...
            # ambiguous case: two F16 projectors in one dir, an F16 and a Q8_0 in
    ...
    if plan.engine == "llama":
        ...
        if not a.gmu:
            ...
            print(f"  --gpu-memory-utilization {a.gmu}: recipe value for sm_{cc} "
                  f"(0.90 = the MEASURED sm_60 arm; 0.85 = the sm_70 serving recipe). "
    ...
    # R-07: devices are NEVER left to the ambient environment. If we cannot
    ...
    # CUDA_VISIBLE_DEVICES entirely, so the child inherited every GPU on the host -
    # including cards mid-measurement and cards carrying someone else's seat.
    if not cv:
        print("  REFUSING to execute with an unscoped device set: no card could be named for "
              "CUDA_VISIBLE_DEVICES. That means inheriting every GPU on the host, including "
              "cards other workloads are using.")
        print("=" * 78)
        sys.exit(3)
    ...
    if plan.engine == "vllm":
        ...
        if img:
            ...
            he = host_env_for(img, VLLM_IMAGES.get(img))
            if he:
