...
static float pxq2_pick_anchor(const float * x, const float * w, int64_t K,
    ...
    for (int k = -2; k <= 2; ++k) {
        ...
        if (dup) continue;
        prev[np++] = cand;
        const double e = pxq2_quant_row(x, w, K, cand, book, mids, sub, s4_tmp, codes_tmp);
...
static void pxq2_quantize_expert(const float * src, uint8_t * dst, int64_t R, int64_t K,
    ...
    for (int64_t p = 0; p < P; ++p) {
        ...
        for (int64_t r = 0; r < 64; ++r) {
            ...
            const float anchor = pxq2_pick_anchor(x, imx, K, book, mids, sub, s4.data(), codes.data());
            anchors[r] = ggml_fp32_to_fp16(anchor);
            pxq2_quant_row(x, imx, K, anchor, book, mids, sub, s4.data(), codes.data());
            // scatter into slabs
