...
#define LU8(x) (const char*)(u8##x)
#include <algorithm>
#include <atomic> // PXA_SCOPED_CANCEL_v1
#include <array>
#include <cassert>
#include <cctype>
#include <cfloat>
...
// PXA_SCOPED_CANCEL_v1: atomic — set from HTTP reader threads via llama_decode_stop(), read in
// the decode loop; the old plain bool was a verified UB data race.
static std::atomic<bool> stop_internal_decode{false};
...
static bool llama_kv_cache_init(
    ...
    if (offload) {
        ...
        for (int64_t i = 0; i < n_layer; ++i) {
            // MTP-SPLIT: account the MTP "nextn" tail layer the SAME as every normal layer
            // (count the split buft_matrix), so the KV-cache buffer-type accounting matches the
            // allocation below (which now also puts the tail's KV in the split ctx). Previously
            // the tail was excluded (is_mtp_tail) and forced to the non-split buft -> single-device
            // island that OOM'd the last GPU under -sm attn. Inert for single-GPU/-sm layer/non-MTP
            // (split_cache/replicate_mla are false there).
    ...
    for (int i = 0; i < (int) n_layer; i++) {
        ...
        (void) is_mtp_tail_layer;
        // MTP-SPLIT: the MTP "nextn" tail layer is plain attention + MoE, so place its KV in the
        // SPLIT context (buft_matrix) exactly like every normal transformer layer. Previously the
        // tail was forced (!is_mtp_tail_layer) into the non-split buft = single-device island that
        // OOM'd the last GPU under -sm attn. The split-KV path below self-disables (reverts ctx to
        // the non-split buft) when wk/wv lack ->extra, so this is inert for single-GPU / -sm layer /
        // non-MTP models; for the now-split MTP tail, wk/wv carry ->extra so its KV splits correctly.
        ...
        struct ggml_context * ctx = (split_cache || replicate_mla) ? ctx_map.at(model.buft_layer[i].buft_matrix) : offload ? ctx_map.at(model.buft_layer[i].buft) : cache.ctxs.front();
        ...
        if (is_mla_attn && cparams.mla_attn) {
            ...
            // MTP-SPLIT: the MTP tail layer is now split like a normal layer, so treat it like one
            // here too — drop the (!is_mtp_tail_layer) exclusion and let the wo->extra check gate.
            // (qwen35moe is not an MLA model, so replicate_mla is false for the 122B target and this
            // branch does not fire; the gate stays correct for any future MLA+MTP model: if the tail
            // is not actually split, wo->extra is null and this self-disables.)
            if (replicate_mla && model.layers[i].wo && model.layers[i].wo->extra) {
            ...
            if (qnext_recurrent) {
                ...
                if (split_cache && model.layers[i].ssm_out->extra) {
                    ...
                    // PXA_REPLICATE_RECURRENT (Lever C): each device carries the COMPLETE recurrent
                    // state (all heads), identical on every device, so the replicated delta-net builder
                    // runs the full-head recurrence locally with no cross-device reduce. When unset, keep
                    // the head-split per-device state (size derived from this device's ssm_out width).
...
static bool llama_kv_cache_find_slot(
    ...
    while (true) {
        ...
        if (n_tested >= cache.size) {
            // PXA_HYBRID_CTX_SHIFT_v1 diagnostics: this is a rare terminal path (request will
            // fail); dump the allocator's view so a "no free space with free cells" state is
            // visible in the server log instead of silent.
            uint32_t free_cells = 0, max_run = 0, run = 0;
            for (uint32_t i = 0; i < cache.size; ++i) {
                if (cache.cells[i].pos < 0) { ++free_cells; if (++run > max_run) max_run = run; }
                else { run = 0; }
            }
            LLAMA_LOG_WARN("%s: no slot for %u tokens: size=%u used=%u head=%u free=%u max_contig_free=%u\n",
                __func__, n_tokens, cache.size, cache.used, cache.head, free_cells, max_run);
            return false;
        }
    }
...
// PXA_PER_SEQ_CKPT: seq_id >= 0 restricts the restore to that sequence's single state row of
// s_l (row index == absolute seq_id: the decode path fills state_row_idx / inp_s_seq_qnext with
// the raw batch seq_id - see the inp_s_seq_qnext fill in llama_set_inputs - and scatters the new
// state back to the same row via ggml_set_rows). seq_id < 0 = legacy all-rows behavior (np=1).
//
// PXA_MULTISEQ_CKPT -- SNAPSHOT LAYOUT (the SAVE side now captures MULTI-sequence batches):
...
// So BOTH snapshots share the layout [step][batch_pos][state-row] with step stride
// n_seqs_captured * row_bytes, where batch_pos is the sequence's index in the batch's
// first-seen (gathered) order -- recorded at capture time in ckpt.last_capture_seqs by
...
static void restore_recurrent_cache_tensors(int step, ggml_backend_sched_t sched,
    ...
    if (seq_id >= 0) {
        dst.ne[1] = 1; // PXA_PER_SEQ_CKPT: copy ONE state row, not all qnext_state_slots rows
    }
    dst.nb[1] = dst.nb[2] = dst.nb[3] = ssm_bytes + conv_bytes;
    dst.data  = (char *)s_l->data + row_offset + conv_bytes;
    auto src = dst;
...
bool llama_kv_cache::per_step_restore(const llama_model & model, ggml_backend_sched_t sched, int step, llama_seq_id seq_id) { // PXA_PER_SEQ_CKPT
    ...
    // PXA_PER_SEQ_CKPT: validate the requested row; FAIL rather than fall back to the all-rows
    // restore, which at np>1 clobbers every other sequence's live recurrent state.
    if (seq_id >= 0 && !llama_kv_qnext_seq_id_in_range(*this, seq_id)) {
        LLAMA_LOG_WARN("%s: seq_id %d out of qnext state-slot range; skipping per-step restore\n", __func__, (int)seq_id);
        return false;
    }

    // PXA_MULTISEQ_CKPT: translate the absolute seq_id into its batch position inside the captured
    // [step][batch_pos][row] snapshot. last_capture_* is recorded by llama_set_inputs on the capture
    // decode; n_seqs == 0 means the last decode did NOT qualify for capture (or none happened) ->
    // FAIL rather than restore from stale/garbage snapshot rows.
    const int32_t n_seqs_captured = ckpt.last_capture_n_seqs;
    if (n_seqs_captured <= 0) {
        LLAMA_LOG_WARN("%s: no per-step capture recorded for the last decode; skipping per-step restore\n", __func__);
        return false;
    }
    if (step + 1 >= ckpt.last_capture_steps) {
        // ssm snapshots exist only for steps 0..n_seq_tokens-2 (the final state lives in s_l itself)
        LLAMA_LOG_WARN("%s: step %d out of captured range (steps=%d); skipping per-step restore\n",
                __func__, step, (int)ckpt.last_capture_steps);
        return false;
    }
    int32_t batch_pos = 0;
    if (seq_id >= 0) {
        batch_pos = -1;
        for (size_t i = 0; i < ckpt.last_capture_seqs.size(); ++i) {
            if (ckpt.last_capture_seqs[i] == seq_id) { batch_pos = (int32_t) i; break; }
        }
        if (batch_pos < 0) {
            LLAMA_LOG_WARN("%s: seq_id %d was not part of the captured batch (n_seqs=%d); skipping per-step restore\n",
                    __func__, (int)seq_id, (int)n_seqs_captured);
            return false;
        }
    } else if (n_seqs_captured != 1) {
        // legacy all-rows restore is only meaningful for single-sequence snapshots
        LLAMA_LOG_WARN("%s: all-rows restore requested but the capture holds %d sequences; skipping\n",
                __func__, (int)n_seqs_captured);
        return false;
    }
    ...
    for (uint32_t il = 0; il < n_layer; ++il) {
        ...
        if (!s_l[il]->extra) {
            ...
            for (int id = 0; id < split_sl->n_device; ++id) {
                ...
                // PXA_PER_SEQ_CKPT: each device split keeps the same [state-slice, qnext_state_slots]
                // row structure (alloc: ggml_new_tensor_2d(..., size, qnext_state_slots)); the row
...
static bool llm_load_tensors(
    ...
    // device_count comes from model.splits (always >= 1), but the device_mem[] array indexed
    // inside this GPU-fit block is sized by model.devices, which is EMPTY on a CPU-only run of a
    ...
    // PXA-SHARD (M2/M3-lite): when PXA_EXPERT_SHARD="d0,d1[,...]" is set, expert-dim
    // shard the UP+GATE experts of every layer HOME to a group device across the
    // matched group (the V100 pair for M2). Down-proj stays normal so the fused MoE
    // path is preserved (split_down_exps NULL). The shard buft stores per-device
    ...
    if (const char * pes = getenv("PXA_EXPERT_SHARD")) {
        ...
        { std::string t; for (const char * p = pes; ; ++p) {
            if (*p == ',' || *p == 0) { if (!t.empty()) group.push_back(atoi(t.c_str())); t.clear(); if (*p==0) break; }
            else t.push_back(*p); } }
        if (group.size() >= 2) {
            auto shard_buft = pxa_expert_shard_buffer_type(group.data(), (int)group.size());
            const int nl = (int)hparams.n_layer;
            int n_sh = 0;
            for (int il = 0; il < nl; ++il) {
                int dev = (il < (int)model.default_layer_device.size()) ? model.default_layer_device[il] : -1;
                bool home_in_group = false;
                for (int g : group) if (g == dev) home_in_group = true;
                if (!home_in_group) continue;
                std::string pattern = "blk\\." + std::to_string(il) + "\\.ffn_(up|gate)_exps\\.(weight|scale)";
                auto & o = overrides.emplace_back();
                o.pattern = strdup(pattern.c_str());
                o.buft    = shard_buft;
                ++n_sh;
            }
            LLAMA_LOG_INFO("PXA-SHARD: expert-sharded up+gate of %d layer(s) across a %d-device group [",
                    n_sh, (int)group.size());
            for (size_t g = 0; g < group.size(); ++g) LLAMA_LOG_INFO("%s%d", g?",":"", group[g]);
            LLAMA_LOG_INFO("]\n");
        }
    }
#endif // GGML_USE_CUDA
...
static void llama_set_inputs(llama_context & lctx, const llama_batch & batch) {
    ...
    if (lctx.inp_s_seq_qnext || (lctx.inp_conv_seq_map && lctx.inp_conv_seq_map->buffer)
        ...
        // PXA_MULTISEQ_CKPT: record the capture identity of THIS decode so per_step_restore can map an
        // absolute seq_id to its batch position in the [step][batch_pos][row] per-step snapshots.
        // Mirrors delta_net's save_per_step_states gate (flag + total-token capacity) plus the
        ...
        if (lctx.kv_self.save_per_step_ssm) {
            ...
            const bool pxa_layout_ok = d.ok && d.uniform_tok;
            if (pxa_fits && pxa_layout_ok) {
                pxa_ck.last_capture_seqs.assign(d.seqs.begin(), d.seqs.end());
                pxa_ck.last_capture_n_seqs = (int32_t) d.n_seqs;
                pxa_ck.last_capture_steps  = (int32_t) d.n_seq_tokens;
            } else {
                pxa_ck.last_capture_seqs.clear();
                pxa_ck.last_capture_n_seqs = 0;
                pxa_ck.last_capture_steps  = 0;
            }
        }
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; ) {
        ...
        if (!lctx.can_reuse_graph(u_batch)) {
            ...
            if (getenv("PXA_GRAPH_DUMP") && u_batch.n_tokens == 1) {
                ...
                if (pxa_gd_count < 3) {
                    ++pxa_gd_count;
                    fprintf(stderr, "PXAGD ===== GRAPH %d =====\n", pxa_gd_count);
                    for (int gi = 0; gi < gf->n_nodes; ++gi) {
                        ggml_tensor * nd = gf->nodes[gi];
                        fprintf(stderr, "PXAGD %4d %-18s %-10s %-42s ne=[%d,%d,%d,%d] t=%s cont=%d src0=%s src1=%s\n",
                            gi, ggml_op_name(nd->op),
                            nd->op == GGML_OP_UNARY ? ggml_unary_op_name((ggml_unary_op)nd->op_params[0]) : "",
                            nd->name, (int)nd->ne[0], (int)nd->ne[1], (int)nd->ne[2], (int)nd->ne[3],
                            ggml_type_name(nd->type), ggml_is_contiguous(nd) ? 1 : 0,
                            nd->src[0] ? nd->src[0]->name : "-", nd->src[1] ? nd->src[1]->name : "-");
                    }
                    fflush(stderr);
                }
            }
                ...
                case LLAMA_POOLING_TYPE_NONE:
                    {
                        ...
                        int32_t n_outputs_new_embd = has_mtp ? embd->ne[1] : lctx.n_outputs;
                        size_t  pxa_embd_src_off   = 0;

                        // PXA_MTP_LAZY_WARMUP_v1: the reserve was clamped for prompt-sized lazy
                        // batches; keep only the LAST row(s) that fit — the final prompt token's
                        // hidden is what ensure_sequence_hidden() seeds the first draft from.
                        // Last-row-wins across ubatches: clamped ubatches write from row 0 and the
                        // cursor clamp below keeps n_outputs_prev_embd at 0, so the batch's true
                        // final row is what survives. Unclamped behavior is bit-identical.
                        const bool pxa_embd_clamped = has_mtp &&
                            (int64_t) n_outputs_prev_embd + n_outputs_new_embd > (int64_t) n_outputs_embd;
                        if (pxa_embd_clamped) {
                            const int32_t pxa_keep = std::max<int32_t>(0, (int32_t) n_outputs_embd);
                            pxa_embd_src_off   = (size_t) std::max<int32_t>(0, n_outputs_new_embd - pxa_keep) * n_embd_output * sizeof(float);
                            n_outputs_new_embd = std::min<int32_t>(pxa_keep, n_outputs_new_embd);
                            embd_out           = lctx.embd;
                        }

                        if (n_outputs_new_embd) {
                            GGML_ASSERT((pxa_embd_clamped ? (int64_t) n_outputs_new_embd : (int64_t) n_outputs_prev_embd + n_outputs_new_embd) <= (int64_t) n_outputs_embd);
                            GGML_ASSERT((int64_t) n_outputs_new_embd*n_embd_output <= (int64_t) lctx.embd_size);
                            ggml_backend_tensor_get_async(backend_embd, embd, embd_out, pxa_embd_src_off, n_outputs_new_embd*n_embd_output*sizeof(float));
                        }
                    } break;
        ...
        n_outputs_prev += lctx.n_outputs;
        {
            ...
            // must not advance the cursor; unclamped behavior is unchanged.
            int64_t pxa_adv = (has_mtp && embd) ? embd->ne[1] : lctx.n_outputs;
            if ((int64_t) n_outputs_prev_embd + pxa_adv > (int64_t) n_outputs_embd) {
                pxa_adv = 0;
            }
            n_outputs_prev_embd += pxa_adv;
        }
        cur_token += n_tokens;
        if (reset_previous) {
...
static bool get_can_shift(struct llama_context & lctx) {
    ...
    // PXA_HYBRID_CTX_SHIFT_v2: IMROPE was blanket-blocked here, but build_k_shift already
    // rotates M-RoPE/IM-RoPE K-caches as whole vectors via NEOX (ggml-org#13870 workaround):
    // for TEXT tokens every position section carries the same scalar pos, so one uniform delta
    // rotation is exact for both blocked (MROPE) and interleaved (IMROPE) layouts. Image chunks
    // never enter a shift window (the server refuses to shift through mtmd chunks). Blocking
    // here made llama_kv_cache_update return 1 MID-DECODE; the server misreported it as
    // "KV cache is full" and the slot stayed wedged forever once a hybrid (qwen35moe) chat
    // outgrew its per-slot n_ctx — the production "long conversation goes silent" failure.
    return !no_shift;
}
    ...
    if (model->split_mode == LLAMA_SPLIT_MODE_NONE || model->split_mode == LLAMA_SPLIT_MODE_GRAPH) {
        ...
        // scheduler and compute buffers
        {
            ...
            bool pipeline_parallel =
                ...
                // PXA: minimax-m2 hits the SAME shared-KV pipeline-parallel race -> exact phrase
                // repetition (endless 三季度) at -sm layer. Serialize decode like OPENAI_MOE.
...
void llama_control_vector_set_ablation(struct llama_context * lctx, bool ablation, float alpha) {
    lctx->cvec.ablation       = ablation;
    lctx->cvec.ablation_alpha = alpha;
}
...
int llama_spec_ckpt_init(struct llama_context * ctx, int mode, int max_tokens) {
    ...
    // PXA_MULTISEQ_CKPT: max_tokens is PER-CHAIN (one slot's drafted+1). At np>1 every co-decoding
    // slot verifies in ONE shared uniform batch and the per-step snapshot layout is
    // [step][batch_pos][row] with a step stride of n_seqs rows, so the buffers must hold
    // max_tokens * n_state_slots rows in total. Scaling HERE (instead of at each caller) keeps the
    // public capacity semantics per-chain (fixed_max_tokens / the per-batch gate above), while the
    // allocation and the delta_net TOTAL-token gate (batch.n_tokens <= per_step_max_allocated)
    // both see the full multi-sequence size.
    const uint32_t pxa_state_slots = llama_kv_qnext_state_slots(kv);
    const int pxa_max_tokens_total = max_tokens * (int) (pxa_state_slots > 0 ? pxa_state_slots : 1);

    if (requested == LLAMA_SPEC_CKPT_PER_STEP) {
        if (spec_ckpt_try_per_step(kv, ctx->model, pxa_max_tokens_total)) {
    ...
    if (resolved == LLAMA_SPEC_CKPT_CPU) {
        ...
        // PXA_LLAMA_MTP_NP3B_FIX: do NOT clear cpu_state_by_seq here. llama_spec_ckpt_init() runs ONCE PER
        // ACTIVE SLOT in the pre-verify save loop; clearing the whole map per-slot would wipe an earlier
        // slot's just-saved checkpoint (the very np>1 clobber this fix targets). Per-seq entries are simply
        // overwritten by that seq's next save; bounded by n_seq_max.
...
bool llama_spec_ckpt_save(struct llama_context * ctx, llama_seq_id seq_id) {
    ...
    switch (kv.ckpt.selected_spec_mode) {
        ...
        case LLAMA_SPEC_CKPT_CPU: {
            // PXA_LLAMA_MTP_NP3B_FIX: save into a PER-SEQ buffer (not the shared cpu_state_data) so a
            // later slot's save in the same pre-verify loop cannot clobber this slot's checkpoint.
            const size_t need = llama_state_seq_get_size(ctx, seq_id, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
            auto & buf = kv.ckpt.cpu_state_by_seq[seq_id];
            buf.resize(need);
            const size_t written = llama_state_seq_get_data(
                ctx, buf.data(), need, seq_id, LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY);
            buf.resize(written);
            return written > 0;
        }
...
bool llama_spec_ckpt_restore(struct llama_context * ctx, llama_seq_id seq_id,
    ...
    switch (kv.ckpt.selected_spec_mode) {
        case LLAMA_SPEC_CKPT_PER_STEP: {
            ...
            // PXA_MTP_HYBRID_ROLLBACK_FIX: cells[seq_id] is the per-sequence STATE cell only for
            // pure-recurrent caches (kv.recurrent == true, one cell per sequence - see
            // llama_kv_cache_find_slot). For HYBRID arches (qwen35moe/qwen3next) kv.recurrent is
            // false and the unified cache is one-cell-per-TOKEN, so cells[seq_id] (= ring cell 0 at
            // np=1) is an unrelated attention-KV cell - usually the FIRST PROMPT TOKEN (the
            // attention sink). Stamping its pos on every rejected draft mispositions it, and the
            // next prompt-cache trim (llama_kv_cache_seq_rm from a divergence point) then evicts it
            // permanently -> attention-sink loss -> repetition collapse in long chats. For hybrids,
            // per_step_restore() above already restored the recurrent state and the seq_rm below
        ...
        case LLAMA_SPEC_CKPT_CPU: {
            // PXA_LLAMA_MTP_NP3B_FIX: restore THIS seq's own saved checkpoint (not the shared buffer that
            // the last-saving slot would have overwritten at np>1).
            auto it = kv.ckpt.cpu_state_by_seq.find(seq_id);
            if (it != kv.ckpt.cpu_state_by_seq.end() && !it->second.empty()) {
                llama_state_seq_set_data(ctx, it->second.data(),
                                         it->second.size(), seq_id,
...
void llama_spec_ckpt_discard(struct llama_context * ctx) {
    ...
    kv.ckpt.selected_spec_mode = LLAMA_SPEC_CKPT_NONE;
    kv.ckpt.cpu_state_data.clear();
    // PXA_LLAMA_MTP_NP3B_FIX: do NOT clear the whole cpu_state_by_seq map here. discard runs per-slot in
    // the accept loop; wiping the map would drop a sibling slot's checkpoint before it restores. Stale
    // per-seq entries are harmless (replaced on that seq's next save) and bounded by n_seq_max.
}
...
struct llama_data_write_buffer : llama_data_write {
    ...
    void write_tensor_data(const struct ggml_tensor * tensor, size_t offset, size_t size, int il) override {
        ...
        // PXA_CKPT_PROF: env-gated per-tensor D2H timing for checkpoint serialization
        static const bool pxa_ckpt_prof = getenv("PXA_CKPT_PROF") != nullptr;
        const int64_t pxa_t0 = pxa_ckpt_prof ? ggml_time_us() : 0;
        if (tensor->extra) {
        ...
        if (pxa_ckpt_prof) {
            const int64_t el = ggml_time_us() - pxa_t0;
            if (el > 500) fprintf(stderr, "PXA_CKPT_PROF %-24s il=%3d size=%9zu extra=%d us=%lld\n",
                tensor->name, il, size, tensor->extra ? 1 : 0, (long long) el);
        }
        ptr += size;
        size_written += size;
        buf_size -= size;
...
static size_t llama_state_seq_get_data_internal(struct llama_context * ctx, llama_data_write & data_ctx, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    if (pxa_ckpt_prof) {
        const int64_t el = ggml_time_us() - pxa_t0;
        if (el > 500) fprintf(stderr, "PXA_CKPT_PROF llama_synchronize us=%lld\n", (long long) el);
    }
...
void llama_synchronize(struct llama_context * ctx) {
    // PXA_SYNC_BISECT: per-backend timed sync (env-gated)
    static const bool pxa_sync_bisect = getenv("PXA_SYNC_BISECT") != nullptr;
    if (pxa_sync_bisect) {
        const int nb = ggml_backend_sched_get_n_backends(ctx->sched);
        for (int i = 0; i < nb; ++i) {
            ggml_backend_t b = ggml_backend_sched_get_backend(ctx->sched, i);
