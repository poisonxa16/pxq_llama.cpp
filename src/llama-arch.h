...
enum llm_kv {
    ...
    // DeepSeek-V4 hyper-connections / hash routing (upstream llama-arch.h @ 82dbc4f01)
    ...
    // DeepSeek-V4 o-LoRA + per-layer compressed attention
...
enum llm_tensor {
    ...
    // DeepSeek-V4. Names mirror upstream src/llama-arch.h @ 82dbc4f01.
    // NOTE: DS4's "attn_kv_a_norm" reuses the existing LLM_TENSOR_ATTN_KV_A_NORM
    // (byte-identical GGUF name), and its indexer q_b/proj reuse the existing
    // LLM_TENSOR_INDEXER_{ATTN_Q_B,PROJ} - do not duplicate those here.
    LLM_TENSOR_ATTN_KV,
