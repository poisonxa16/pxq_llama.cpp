...
void llama_sample_repetition_penalties_impl(
    ...
    // PXA_SAMPLER_O_WINDOW: penalties only touch tokens present in the penalty window (<= penalty_last_n
    // distinct ids). When the candidates array is still identity-ordered (data[i].id == i -- the state the
    // common sampler calls this in: freshly built from logits, pre-sort), apply by DIRECT INDEX: O(window)
    // instead of an O(n_vocab) scan with a hash lookup per candidate (a 248K-vocab model pays ~2ms/token
    // for that scan). The guard only checks the ids we would touch; any mismatch falls back to the
    // original full scan, so other call sites keep exact behavior.
    bool pxa_identity = true;
    for (const auto & kv : token_count) {
        const llama_token id = kv.first;
        if (id < 0 || (size_t) id >= candidates->size || candidates->data[id].id != id) { pxa_identity = false; break; }
    }
    if (pxa_identity) {
        for (const auto & kv : token_count) {
            auto & cd = candidates->data[kv.first];
            const int count = kv.second;
            if (cd.logit <= 0) {
                cd.logit *= penalty_repeat;
            } else {
                cd.logit /= penalty_repeat;
            }
            cd.logit -= float(count) * penalty_freq + float(count > 0) * penalty_present;
        }
        candidates->sorted = false;
        if (smpl) {
            smpl->t_sample_us += ggml_time_us() - t_start_sample_us;
...
void llama_sampler_dry_apply(struct llama_sampler_dry* smpl, llama_token_data_array* cur_p) {
    ...
    // PXA_SAMPLER_O_WINDOW: dry_max_token_repeat holds only the (few) tokens that would extend an
    // observed repeat. When cur_p is still identity-ordered (DRY is the first chain stage, running on
    // the freshly built candidates), apply by direct index: O(|map|) instead of an O(n_vocab) scan
    // with a hash lookup per candidate. Falls back to the original scan on any id mismatch.
    bool pxa_identity = true;
    for (const auto & kv : smpl->dry_max_token_repeat) {
        const llama_token id = kv.first;
        if (id < 0 || (size_t) id >= cur_p->size || cur_p->data[id].id != id) { pxa_identity = false; break; }
    }
    if (pxa_identity) {
        for (const auto & kv : smpl->dry_max_token_repeat) {
            auto range = smpl->dry_processed_breakers.equal_range(kv.first);
            ...
            if (!is_single_token_breaker) {
                int repeat_exp = kv.second - smpl->dry_allowed_length;
                if (max_exponent > 0 && repeat_exp > max_exponent) {
                    repeat_exp = max_exponent;
                }
                float penalty = smpl->dry_multiplier * std::pow(smpl->dry_base, repeat_exp);
                cur_p->data[kv.first].logit -= penalty;
            }
        }
    } else {
