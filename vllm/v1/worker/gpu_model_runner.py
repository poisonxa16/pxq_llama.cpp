...
class GPUModelRunner(
    ...
    def _determine_batch_execution_and_padding(
        ...
        # PXA short-prefill fix (2026-08-24): a prompt of exactly
        # uniform_decode_query_len tokens (ptok=1 raw completion) satisfies the
        # shape test above and would replay the FULL decode CUDA graph with zero
        # computed context over stale graph-input state -> garbage from char 0
        # (content is a per-boot lottery, the defect is not; observed live on both
        # the DGX and the local sm_70 seats). True decode always has computed
        # context; require it. Capture paths pass force_uniform_decode and are
        # unaffected.
        if (
            force_uniform_decode is None
            and uniform_decode
            and num_reqs > 0
            and int(self.input_batch.num_computed_tokens_cpu[:num_reqs].min()) == 0
        ):
    ...
    def _dummy_run(
    ...
    ) -> tuple[torch.Tensor | IntermediateTensors, torch.Tensor | IntermediateTensors]:
        """
        ...
        # PP: on every rank except the last, the model returns
        # IntermediateTensors, not a Tensor. Everything below assumes a
        # real Tensor: tuple(hidden_states.shape) is evaluated eagerly as
        # a call argument (the VLLM_SM70_PROFILE_TRACE gate is inside the
        # callee at :675, too late), and IntermediateTensors.__getitem__
        # accepts only str|slice - a Tensor key returns None SILENTLY.
        # Bail out here exactly as execute_model already does at :8391.
        if not get_pp_group().is_last_rank:
            assert isinstance(hidden_states, IntermediateTensors)
