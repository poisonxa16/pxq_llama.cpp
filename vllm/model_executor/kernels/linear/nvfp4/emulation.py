...
class EmulationNvFp4LinearKernel(NvFp4LinearKernel):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if not _supports_triton_nvfp4_emulation(layer.weight.device):
            ...
            layer.register_parameter(
                "weight_dequant", Parameter(weight_dequant, requires_grad=False)
            )
    ...
    def apply_weights(
        ...
        out = run_nvfp4_emulations(
            ...
            weight_dequant=getattr(layer, "weight_dequant", None),
            swizzle=False,
        )
        if bias is not None:
            out = out + bias
