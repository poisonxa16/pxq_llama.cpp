...
# NOTE: PEP 563 lazy annotations are REQUIRED here, not cosmetic.
# torch 2.7.1 dynamo cannot trace a nested `def` whose annotations use PEP 604
# unions (`torch.Tensor | None`): building the function object materialises a
# `types.UnionType`, and 2.7.1s SourcelessBuilder has no handler for it:
#   Unsupported: Unexpected type in sourceless builder types.UnionType
# torch 2.10 dynamo handles it; this image is pinned to 2.7.1 because 2.10
# dropped sm_60. Making annotations lazy strings keeps the UnionType from ever
# being constructed at trace time. Verified safe for the 12
# direct_register_custom_op() calls below: torch 2.7.1 infer_schema() resolves
# string annotations via get_type_hints and emits an identical schema.
...
def _resolve_gdn_prefill_backend(
    ...
    * Blackwell (SM10.x) with ``head_k_dim == 128``;
    ...
    # PASCAL PORT: token-sequential recurrent prefill. The chunked FLA kernels
    # (and FlashQLA/FlashInfer/CuteDSL) all bottom out in tl.dot or tensor
    # cores, neither of which exists below sm_70. The fused_recurrent kernel is
    # elementwise-only and compiles for sm_60, so it becomes the prefill core
    # there: O(T) sequential per sequence, slow but exact.
    if backend == "recurrent":
        return backend, "recurrent"
    if current_platform.is_cuda():
        _cap = current_platform.get_device_capability()
        if backend == "auto" and _cap is not None and _cap.major < 7:
            return backend, "recurrent"
...
def _log_gdn_backend_decision(
    ...
    chosen = {
        ...
        "cutedsl": "CuteDSL",
        "triton": "Triton/FLA",
        "recurrent": "Recurrent/FLA (Pascal sequential prefill)",
    }[active_backend]
    logger.info_once(
...
class ChunkGatedDeltaRule(CustomOp):
    def __init__(self) -> None:
        ...
        elif active_backend == "recurrent":
            self._forward_method = self.forward_recurrent
        else:
    ...
    def forward_recurrent(
        ...
        """PASCAL PORT: prefill through the token-sequential recurrent kernel.

        Same recurrence as the chunked kernels, evaluated one token at a time
        (fp32 state throughout), so it needs no tl.dot and runs on sm_60.

        The call site hands us a GATHERED per-sequence initial_state copy
        [N, HV, V, K] (rows without prior state already zeroed) and scatters
        the returned final state back into the ssm_state cache itself. We run
        the kernel in its continuous-batching mode against that gathered copy:
        ssm_state_indices = arange(N) expanded to [N, max_len] with a zero
        token-stride, so every token of sequence n reads and writes row n of
        the copy in place. After the kernel, row n holds the state after
        sequence n's last token - exactly the final state the caller scatters.
        """
        if gate_is_exp:
            g = torch.log(g)
        assert cu_seqlens is not None, "recurrent prefill requires cu_seqlens"
        n_seqs = int(cu_seqlens.numel() - 1)
        if os.getenv("PXA_GDN_RECURRENT_DEBUG") and _SM70_GDN_DUMP_COUNTS.get(
            "pxa_r_in", 0
        ) < 8:
            _SM70_GDN_DUMP_COUNTS["pxa_r_in"] = _SM70_GDN_DUMP_COUNTS.get("pxa_r_in", 0) + 1
            logger.info(
                "PXA recurrent dbg: q%s%s k%s v%s g%s%s beta%s%s cu=%s init%s%s "
                "contig q=%s v=%s",
                tuple(q.shape), q.dtype, tuple(k.shape), tuple(v.shape),
                tuple(g.shape), g.dtype, tuple(beta.shape), beta.dtype,
                cu_seqlens.tolist()[:8], tuple(initial_state.shape),
                initial_state.dtype, q.is_contiguous(), v.is_contiguous(),
            )
        idx = torch.arange(
            n_seqs, device=q.device, dtype=torch.int32
        ).unsqueeze(1).expand(n_seqs, 2)
        o, final_state = fused_recurrent_gated_delta_rule(
            ...
            # ALWAYS the True variant: triton 3.3 miscompiles the
            # USE_QK_L2NORM_IN_KERNEL=False specialization of this kernel into
            # a zero-writer (o and the final state come back all-zero; verified
            # on sm_60 and sm_70). l2norm is idempotent, so re-normalizing the
            # already-normalized prefill q/k changes nothing beyond one ulp.
            use_qk_l2norm_in_kernel=True,
        )
        if os.getenv("PXA_GDN_RECURRENT_DEBUG") and _SM70_GDN_DUMP_COUNTS.get(
            "pxa_r_out", 0
        ) < 8:
            _SM70_GDN_DUMP_COUNTS["pxa_r_out"] = _SM70_GDN_DUMP_COUNTS.get("pxa_r_out", 0) + 1
            logger.info(
                "PXA recurrent dbg out: o%s absmean=%.6f state_absmean=%.6f",
                tuple(o.shape), float(o.float().abs().mean()),
                float(final_state.float().abs().mean()),
            )
        if core_attn_out is not None:
            o_flat = o.squeeze(0).reshape(-1)
            co_flat = core_attn_out.reshape(-1)
            co_flat[: o_flat.numel()].copy_(o_flat)
        if os.getenv("PXA_GDN_RECURRENT_DEBUG") and not getattr(
            ChunkGatedDeltaRule, "_pxa_dbg2_done", False
        ):
            ChunkGatedDeltaRule._pxa_dbg2_done = True
            logger.info(
                "PXA recurrent dbg2: g_mean=%.4f g_max=%.4f beta_mean=%.4f "
                "core_kwarg=%s core_absmean_after_copy=%s",
                float(g.float().mean()), float(g.float().max()),
                float(beta.float().mean()),
                core_attn_out is not None,
                (float(core_attn_out.float().abs().mean())
                 if core_attn_out is not None else "n/a"),
            )
...
def qwen_gdn_attention_core_standard(
    ...
    if os.getenv("PXA_GDN_RECURRENT_DEBUG"):
        n = _SM70_GDN_DUMP_COUNTS.get("pxa_dbg3", 0)
        if n < 6 and attn_metadata is not None and attn_metadata.num_prefills >= 1:
            _SM70_GDN_DUMP_COUNTS["pxa_dbg3"] = n + 1
            np_ = attn_metadata.num_prefills if attn_metadata is not None else -1
            logger.info(
                "PXA dbg3 core_standard exit: layer=%s prefills=%s tokens=%s "
                "core_absmean=%.6f",
                layer_name, np_, mixed_qkv.shape[0],
                float(core_attn_out.float().abs().mean()),
            )
