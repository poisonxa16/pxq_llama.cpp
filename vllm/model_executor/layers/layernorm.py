...
# Probed once, then constant for the process (assume_constant_result requires that).
_SM70_LONG_PREFILL_OP: bool | None = None


@torch.compiler.assume_constant_result
def _sm70_gemma_long_prefill_available() -> bool:
    """Is the sm_70 fused long-prefill RMSNorm op ACTUALLY callable here?

    This used to return `current_platform.is_device_capability(70)` — it checked the
    GPU and never checked that the kernel was compiled in. Those are different
    questions whenever the build omits it, and one build does exactly that:
    `VLLM_SKIP_C_STABLE=1` (required to build against torch 2.7.1 for Pascal support)
    drops `csrc/libtorch_stable/`, which is where this op lives. The result was a
    V100 boot dying in `forward` with

        AttributeError: '_OpNamespace' '_C' object has no attribute
                        'sm70_gemma_long_prefill_fused_add_rms_norm'

    and `custom_ops:["none"]` could not save it, because the call is reached from
    `forward_native` as well as `forward_cuda`. Probe the op instead of the device;
    when it is absent we fall back to the unfused path, which is slower and correct.
    """
    global _SM70_LONG_PREFILL_OP
    if not current_platform.is_device_capability(70):
        return False
    if _SM70_LONG_PREFILL_OP is None:
        try:
            _SM70_LONG_PREFILL_OP = hasattr(
                torch.ops._C, "sm70_gemma_long_prefill_fused_add_rms_norm"
            )
        except Exception:
            _SM70_LONG_PREFILL_OP = False
        if not _SM70_LONG_PREFILL_OP:
            logger.warning(
                "sm_70 fused long-prefill RMSNorm kernel is not present in this "
                "build (expected when VLLM_SKIP_C_STABLE=1 was used to target "
                "torch<2.10). Falling back to the unfused path: correct, slower."
            )
    return _SM70_LONG_PREFILL_OP
