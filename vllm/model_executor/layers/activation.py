...
class FatreluAndMul(CustomOp):
    ...
    def __init__(self, threshold: float = 0.0):
        ...
        if current_platform.is_cuda_alike():
            # PASCAL PORT: basic activation ops live in _C_stable_libtorch,
            # which torch 2.7 cannot build; fall back to forward_native.
...
class GELU(CustomOp):
    ...
    def __init__(self):
        ...
        if current_platform.get_cpu_architecture() == CpuArchEnum.ARM and hasattr(
            ...
            # PASCAL PORT: basic activation ops live in _C_stable_libtorch,
            # which torch 2.7 cannot build; fall back to forward_native.
            self.op = getattr(torch.ops._C, "activation_lut_bf16", None)
            if self.op is None:
                self._forward_method = self.forward_native
        else:
            self.op = None
