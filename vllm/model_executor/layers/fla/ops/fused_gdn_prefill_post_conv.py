...
def _fused_post_conv_kernel(
    ...
    if i_head < H:
        ...
        mask_k = offs_k < K
        qk_mask_2d = mask_t[:, None] & mask_k[None, :]  # [BLOCK_T, BK]
        ...
        k_offsets = (
            offs_t[:, None] * stride_x_tok
            + (HK + i_h * K + offs_k[None, :]) * stride_x_feat
        )
        k_f32 = tl.load(mixed_qkv_ptr + k_offsets, mask=qk_mask_2d, other=0).to(
            tl.float32
        )
        ...
        mask_v = offs_v < V
        v_mask_2d = mask_t[:, None] & mask_v[None, :]  # [BLOCK_T, BV]
        ...
        v_offsets = (
            offs_t[:, None] * stride_x_tok
            + (V_OFFSET + i_hv * V + offs_v[None, :]) * stride_x_feat
        )
        v_vals = tl.load(mixed_qkv_ptr + v_offsets, mask=v_mask_2d, other=0)

        # Store V
        v_out = offs_t[:, None] * stride_v_tok + i_hv * V + offs_v[None, :]
        tl.store(v_ptr + v_out, v_vals, mask=v_mask_2d)
...
def fused_post_conv_prep(
    ...
    _fused_post_conv_kernel[grid](
        ...
        stride_x_feat=conv_output.stride(1),
