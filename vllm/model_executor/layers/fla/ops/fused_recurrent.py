...
def fused_recurrent_gated_delta_rule_fwd_kernel(
    ...
    for i_t in range(0, T):
        ...
        if INPLACE_FINAL_STATE:
            # PASCAL PORT bugfix: honor stride_indices_tok instead of a
            # hard-coded token stride of 1. The old code read
            # ssm_state_indices[i_n*stride_seq + i_t], which is only valid
            # when the caller supplies a full [N, >=T] per-token slot table;
            # any other layout made i_t >= row_len read GARBAGE indices and
            # store the state at wild addresses (observed: illegal memory
            # access, and silent state corruption when the address happened
            # to be mapped). With a zero token stride (all tokens of a
            # sequence share one slot) the per-token stores are redundant, so
            # only the last token stores -- stride_indices_tok is constexpr,
            # making this specialization free for the decode/spec paths.
            if (stride_indices_tok != 0) or (i_t == T - 1):
                # Load state index and check for invalid entries.
