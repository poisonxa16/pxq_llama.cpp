...
def _patch_torch_config_keys_if_needed():
    """PASCAL PORT: tolerate torch config keys that torch 2.7 does not define.

    torch 2.7 is the last release that ships sm_60 (Pascal) kernels, so it is
    the floor for this build.  vLLM -- written against torch 2.10 -- toggles a
    number of inductor and AOTAutograd settings that only exist in newer torch:
    ``alignment_asserts``, ``size_asserts``, ``scalar_asserts``,
    ``bundled_autograd_cache``, ``autograd_cache_normalize_inputs``, and so on.

    ``ConfigModule.patch()`` reads each key's current value before overriding
    it, so an unknown name raises AttributeError.  Every one of these calls sits
    inside the compile path, which means that AttributeError comes back as a
    BackendCompilerFailed and drops the whole engine into eager mode -- where
    ~90% of a decode step is python dispatch rather than GPU work.

    The settings in question are debug assertions and compile-artifact caching
    policy.  They do not change what is compiled or what it computes, so
    dropping the ones this torch has never heard of is safe; the wrapper logs
    each name once at debug level.

    Wrapped on ConfigModule itself so it covers torch._inductor.config,
    torch._functorch.config and torch._dynamo.config alike.  Note that a patch
    whose keys are *all* unknown degrades to a plain context manager and can no
    longer be used as a decorator; vLLM only ever uses it as a context manager.
    """
    ...
    if getattr(ConfigModule.patch, "_pxa_key_tolerant", False):
        return

    _orig_patch = ConfigModule.patch
    _warned: set[str] = set()

    def _keep(cfg, key):
        if key in cfg._config:
            return True
        name = f"{cfg.__name__}.{key}"
        if name not in _warned:
            _warned.add(name)
            logger.debug(
                "torch %s has no config key %s; ignoring it (assertions and "
                "compile-cache policy only).",
                torch.__version__,
                name,
            )
        return False

    @functools.wraps(_orig_patch)
    def patch(self, arg1=None, arg2=None, **kwargs):
        if isinstance(arg1, str) and arg2 is not None:
            if not _keep(self, arg1):
                arg1, arg2 = None, None
        elif isinstance(arg1, dict):
            arg1 = {k: v for k, v in arg1.items() if _keep(self, k)}
            if not arg1:
                arg1 = None
        kwargs = {k: v for k, v in kwargs.items() if _keep(self, k)}
        if arg1 is None and not kwargs:
            return contextlib.nullcontext()
        return _orig_patch(self, arg1, arg2, **kwargs)

    patch._pxa_key_tolerant = True  # type: ignore[attr-defined]
    ConfigModule.patch = patch  # type: ignore[method-assign]


_patch_torch_config_keys_if_needed()
