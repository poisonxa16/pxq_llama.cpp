...
class AnthropicServingMessages(OpenAIServingChat):
    ...
    def _convert_messages(
        ...
        for msg in messages:
            role = msg.role
            # Harnesses (e.g. the Claude Code / 1Cat harness) may inject
            # system messages inside `messages`; keep their role so the chat
            # template treats them as system instructions.
            openai_msg: dict[str, Any] = {"role": role or "user"}  # type: ignore
            ...
            if not (role == "user" and "content" not in openai_msg):
