    ...
    def __post_init__(self):
        ...
        if sm70_flash_0dot3_compile_graph:
            if sm70_compile_disabled_by_user:
                ...
                # Respect an EXPLICIT cudagraph_mode instead of overwriting it. This
                # assignment was unconditional, so a user passing
                # --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' watched it
                # parse into the config dump, get silently replaced here, and then read
                # FULL_AND_PIECEWISE back out of the startup log. That is worse than
                # refusing the flag: it looks like it worked. On sm_60 FULL_DECODE_ONLY is
                # a correctness requirement, so a silent downgrade there is a data-quality
                # bug, not a perf note.
                _requested = self.compilation_config.cudagraph_mode
                if _requested is None:
                    self.compilation_config.cudagraph_mode = (
                        CUDAGraphMode.FULL_AND_PIECEWISE
                    )
                elif _requested != CUDAGraphMode.FULL_AND_PIECEWISE:
                    logger.warning_once(
                        "SM70 Flash-V100 default cudagraph_mode is FULL_AND_PIECEWISE; "
                        "honouring the explicitly requested %s instead. If this seat "
                        "misbehaves, drop the override before reporting it.",
                        _requested,
                    )
                ...
                if "VLLM_USE_AOT_COMPILE" not in os.environ:
                    # aot_compile does not exist before torch 2.10. This image is built
                    # against torch 2.7.1 on purpose (2.10 dropped sm_60, and Pascal
                    # support is the reason this fork exists), so auto-setting it here
                    # produced a hard boot failure on every V100 start:
                    #   RuntimeError: aot_compile is not supported ... (torch: 2.7.1+cu126)
                    # Probe for the symbol rather than assuming the platform default's
                    # torch version.
                    # NOTE: check torch._dynamo ONLY. torch._inductor.aot_compile exists in
                    # 2.7.1 as the older AOT Inductor entry point and is NOT what vLLM calls;
                    # accepting it here let the auto-set fire on 2.7.1 and reintroduced the
                    # exact boot failure this guard exists to prevent. Verified on the image:
                    #   torch 2.7.1+cu126 -> _dynamo.aot_compile False, _inductor.aot_compile True
                    _has_aot = hasattr(getattr(torch, "_dynamo", None), "aot_compile")
                    if _has_aot:
                        ...
                        logger.warning_once(
                            "SM70 Flash-V100 0.0.3 wants VLLM_USE_AOT_COMPILE=1, but "
                            "this torch (%s) has no aot_compile. Leaving it off. Note "
                            "the upstream warning that regular torch.compile reproduced "
                            "deterministic greedy token drift - verify decode quality "
                            "on this build before trusting it.",
                            torch.__version__,
                        )
                ...
                # Inductor combo (horizontal-fusion) kernels. Escape hatch:
                # VLLM_SM70_COMBO_KERNELS=0 disables them. Kept ON by default so
                # this stays a no-op for the intended torch-2.10 build.
                if os.environ.get("VLLM_SM70_COMBO_KERNELS", "1") != "0":
                    ...
                    logger.warning_once(
                        "VLLM_SM70_COMBO_KERNELS=0: Inductor combo kernels disabled."
                    )
                logger.info_once(
                    "Using SM70 Flash-V100 0.0.3 compile CUDA graph policy: "
                    "mode=%s, cudagraph_mode=%s, capture_sizes=%s.",
