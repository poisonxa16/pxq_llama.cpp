...
def set_functorch_config() -> None:
    for k, v in _get_vllm_functorch_config().items():
        # PASCAL PORT: torch 2.7 -- the last release shipping sm_60 kernels --
        # does not define every AOTAutograd-cache knob vLLM sets here, and
        # torch._functorch.config rejects an unknown name outright.  Because
        # this runs inside the compile path, that AttributeError surfaces as a
        # BackendCompilerFailed and drops the engine back to eager.  These
        # knobs only govern caching of compiled artifacts, so skipping the ones
        # this torch has never heard of costs compile time and nothing else.
        if not hasattr(torch._functorch.config, k):
            continue
