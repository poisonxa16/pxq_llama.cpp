    ...
    def __init__(
        ...
        if mode != CompilationMode.STOCK_TORCH_COMPILE:
            ...
            if self.evaluate_guards:
                ...
                elif is_torch_equal_or_newer("2.8"):
                    # Equivalent fallback for older PyTorch: skip all guards
                    options["guard_filter_fn"] = lambda x: [False for _ in x]
                else:
                    # PASCAL PORT: torch 2.7 -- the last release that ships
                    # sm_60 kernels -- has no guard_filter_fn at all, and its
                    # torch.compile forwards any unrecognised `options` entry
                    # straight through to the backend, so VllmBackend dies on
                    # "unexpected keyword argument 'options'".  Guard filtering
                    # only suppresses recompiles; dropping it costs a little
                    # compile time and changes nothing about the result.
                    logger.debug_once(
                        "torch %s has no guard_filter_fn; compiling with "
                        "dynamo's normal guards.",
                        torch.__version__,
                    )
