...
extern "C" {
    ...
    // true once any gguf loaded by this process contained a tensor of ANY PXQ tier
    ...
    // PXA: switch the applied control vector into directional-ABLATION mode (project the
    // direction out of the residual instead of adding it). alpha scales the projection
    // (1.0 = full orthogonal projection). Directions should be unit-normalized by the caller.
    LLAMA_API void llama_control_vector_set_ablation(
            struct llama_context * lctx,
                            bool   ablation,
                           float   alpha);
