...
def bundle_flash_attn_v100(build_lib: str) -> None:
    ...
    if os.environ.get("VLLM_SKIP_FLASH_V100", "") in ("1", "ON", "on", "true"):
        logger.info("Skipping Flash-V100 bundle (VLLM_SKIP_FLASH_V100 set)")
        return
...
if _build_custom_ops():
    ext_modules.append(CMakeExtension(name="vllm._C"))
    if _is_cuda() or _is_hip():
        if os.environ.get("VLLM_SKIP_C_STABLE", "") not in ("1", "ON", "on", "true"):
            ext_modules.append(CMakeExtension(name="vllm._C_stable_libtorch"))
        else:
            logger.info("Skipping _C_stable_libtorch (VLLM_SKIP_C_STABLE set)")
