# The flake interface to pxq_llama's Nix expressions. The flake is used as a
...
#
# pxq_llama is a fork of llama.cpp by way of ik_llama.cpp; see NOTICE and
# LICENSING.md for the upstream attribution.
...
#
# ```nix
# pkgs.callPackage ${pxq-llama-root}/.devops/nix/scope.nix { }`
# ```

# Cf. https://jade.fyi/blog/flakes-arent-real/ for a more detailed exposition
# of the relation between Nix and the Nix Flakes.
{
  description = "pxq_llama: llama.cpp fork with the PXQ quantization family, by PXA Network";
  ...
  # For inspection, use `nix flake show <path-to-this-checkout>` or the nix repl:
  #
  # ```bash
  # ❯ nix repl
  # nix-repl> :lf .
