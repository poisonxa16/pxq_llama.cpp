...
If you only want the two-line version, it is in the README, and the guided walk-through from
clone to first server run is in [`docs/QUICKSTART.md`](docs/QUICKSTART.md). This file is the
one to follow when those do not work.
...
Every other `PXA_*` variable is a lab knob — see the README's Run section.
