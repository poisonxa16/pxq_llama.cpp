...
Measured 2026-08-27 on a pair of Tesla P100-PCIE-16GB cards (sm_60, TP=2).
...
Name the two P100s explicitly with `CARDS=<i>,<j>`; the TP size is however many
you list. On a host where some cards belong to another workload, put their
indices in `PXA_RESERVED_CARDS` and the launcher refuses to start if `CARDS`
intersects that list. The container boundary then excludes everything not named
in `NVIDIA_VISIBLE_DEVICES`.
