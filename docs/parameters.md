...
Overview of the most common command-line parameters in `pxq_llama` and some info how to use them.
...
| GGUF | The file format used by pxq_llama and llama.cpp |
...
| `--dry-run` | Prints the tensor types and resulting tensor sizes, but does not run the quantization, so it is very fast. | - | Useful for experimenting with --custom-q before running the actual quantization. [PR 1309](https://github.com/ikawrakow/ik_llama.cpp/pull/1309). **⚠ Fixed 2026-08-25 — treat any dry-run output from before that date as wrong for PXQ targets.** The native PXQ dispatch lives in the `else` arm of `if (params->dry_run)`, so a dry run used to account every PXQ-eligible tensor (above all the routed expert stacks) as its MXFP4 backbone default. On a MoE model that projected a few percent of PXQ-family bytes and tripped the end-of-run composition assertion — a failure the equivalent real run does not have. Measured on a 128-expert 30B: dry run reported `mxfp4 144 tensors / 92.9%` and aborted at "2.6% PXQ-family bytes", while the real run of the same command produced `pxq4 240 tensors / 95.5%`, rc=0. The dry-run branch now projects the same landing the real run takes and reproduces that composition and `quant size` exactly. |
