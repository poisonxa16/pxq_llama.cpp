...
⚠ **On Volta it requires a vLLM build with sm_70 support, not upstream vLLM.**
Upstream vLLM does not target compute capability 7.0. A Volta-capable build has to carry the
sm_70 work this backend depends on entirely: a TurboMind sm_70 W4A16 GEMM, the
`FLASH_ATTN_V100` attention backend, and the Qwen Gated-DeltaNet kernels. On sm_80+ hardware
you do not need any of that, but the sm_70 path is what this sidecar was built and measured
against. See `tools/vllm-pxq4/LICENSE-NOTICE.md` for the attribution.

⚠ **One optional PyTorch patch, for Pascal only.** `tools/vllm-pxq4/tools/patch_sm60_compile.py` lifts a
...
itself does not depend on it. Disclosed in `tools/vllm-pxq4/LICENSE-NOTICE.md`.

### Build

`tools/vllm-pxq4/src/build.sh` compiles the kernels inside the same image the server runs, so torch/nvcc/gcc are
...
Both configurations are gated before a release cut: a no-ik build and a no-AVX2 build are
configured, compiled and run against a real model, in addition to the default build.
