...
The CPU quantized GEMM path (the dominant hot path for quantized prompt
processing) is gated on the AVX-512 feature set upstream ik_llama.cpp called
"fancy SIMD". This fork writes the same five feature macros out inline in
[`ggml/src/pxq-cpu.c`](../ggml/src/pxq-cpu.c) (`pxa_has_fancy_simd`):
...
- VNNI alone (`vpdpbusd`) is responsible for most of the speedup on
  quantized matmul, so `GGML_AVX512_VNNI=ON` is the single most valuable
  option in the list above. (Upstream ik_llama.cpp also carried a separate
  256-bit `HAVE_VNNI256` path for AVX2-only CPUs with VNNI; it was removed
  from this fork along with the rest of the upstream CPU GEMM kernels.)
...
# Zero means the quantized CPU path fell back to AVX2.
```
