...
| `PXA_PXQ3_PAIRLUT` | off | K2b3: P3 (PXQ3 bit-plane tier) 64-entry float2 pair-LUT — replaces the per-pair bit-plane code reassembly (~10 int ops + 2 smem gathers) with BFE+BFE+BFI+LDS.64 keyed on the pair's 4 low-plane + 2 high-plane bits (bit-exact sourcing; distinct from K2a, which only fuses the two gathers on 4-bit tiers) | **staged 2026-08-04 — measurement pending; fill this cell from the A/B before any publish** | pending A/B |
| `PXA_PXQ6_SHFL` | off | K2d: register-book __shfl decode (decode mmv family, every tier) — lane l holds book[l] in one register; each per-pair book gather becomes SHFL.IDX, moving the codebook traffic (~63% of decode LSU bytes on the fired gateup loop, which is LSU-pipe bound at ~83% of the structural ceiling while DRAM sits at 50%) onto the idle shuffle pipe at identical instruction count (bit-exact sourcing: same float from the same index, same FMA order; no fp16 round-trip, unlike K2c PRMT) | **staged 2026-08-04 — measurement pending; fill this cell from the A/B before any publish** | pending A/B |
| `PXA_PXQ6_QPF` | off | Q1: one-slab-lookahead register prefetch twins of the fired decode kernels (1 = mmv family, 2 = gateup family, 3 = both) — the canonical chunk walk issues slab i+1's code words + scale bytes before slab i's unpack/FMA chain, overlapping the exposed per-warp DRAM latency (the residual after K2d: every issue-side lever pays ~0.2:1 and the code LDG was consumed by the next instruction with zero lookahead) with compute that was being issued anyway; walk order, fold boundaries, chains, ws slots identical — pure load reordering of read-only data, bit-exact; yields to an engaged R2 twin per site | **staged 2026-08-04 — measurement pending; fill this cell from the A/B before any publish** | pending A/B |
