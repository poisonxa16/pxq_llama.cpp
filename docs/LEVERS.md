...
> **Why K9/K10 have no code here.** Both landed on `sm70-codec` as commit `0227577`, against the *pre-canon* `k_pxq6_mmv` and its `pxq6_pick_mmv_ksplit_gen_x` variant selector. `PXQ_CANON_v2` since rewrote that kernel and its dispatch (which now carries the `rowx2`/`qpf` variants), so the 187-line probe body no longer applies and porting it would mean writing new, unmeasured kernel code for two levers that are **already known to be slower**. The measurements above are the part worth keeping — that was the commit's own stated purpose. The kernel bodies remain retrievable at `git show 0227577` on branch `sm70-codec`, which is deliberately left unmerged for exactly that reason. Also in that commit and likewise not ported: folding the PXQ4-HQ per-row sub-scale pair into one 16-bit load, and a compile-time `pxq6_srcpair()` operand-sourcing route.
