...
of WMMA. **+6.9% prefill**, decode unchanged. Measured with the protocol in `bench/fair-battle.md`.
...
Every gated flag in this release is documented with its mechanism, its default, and -- where a
throughput claim is made for it -- the measurement and the exact configuration that produced it.
Levers that ship without a throughput claim say so rather than implying one.
