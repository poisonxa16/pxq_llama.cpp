...
__global__ void delta_net_recurrent_f32(
    ...
    float * state_dst = (state_ext ? state_ext : dst + output_offset) + batch_idx * state_batch_stride + state_head_offset;
    ...
    if (n_tokens <= 8) {
        ...
        if (head_dim == 64) {
            delta_net_recurrent_f32<64, threads_per_block><<<num_blocks, threads_per_block, smem_size, stream>>>(
                    q, k, v, g, beta, state_in, dst, saved_states, state_ext, n_heads, gqa_ratio, repeat_type, n_tokens, n_seqs, output_offset, vnb1, vnb2, vnb3);
        } else {
...
void ggml_cuda_op_delta_net(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ggml_cuda_op_delta_net_ex(ctx, dst, nullptr);
}
