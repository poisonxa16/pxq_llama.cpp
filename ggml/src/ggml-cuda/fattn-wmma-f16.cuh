...
#pragma unroll
    ...
    // PXA_FA_MASK_SKIP_v1 (2026-07-15): scratch for the fully-masked-tile skip below.
    __shared__ float pxa_skip_smax[nwarps];

    // Iterate over ne11 == previous tokens:
    for (int k_VKQ_0 = ip*FATTN_KQ_STRIDE; k_VKQ_0 < ne11; k_VKQ_0 += parallel_blocks*FATTN_KQ_STRIDE) {
        // PXA_FA_MASK_SKIP_v1 (2026-07-15): skip KV tiles whose mask is entirely -inf. Such tiles
        // contribute exactly nothing (exp(-inf - max) == 0, running max unchanged, VKQ scale == 1),
        ...
        if (mask) {
            ...
            const int pxa_stride2  = (nb31/sizeof(half))/2; // mask row stride in half2
            float pxa_max = -INFINITY;
            for (int idx = threadIdx.y*WARP_SIZE + threadIdx.x; idx < ncols*(FATTN_KQ_STRIDE/2); idx += nwarps*WARP_SIZE) {
                const int j = idx / (FATTN_KQ_STRIDE/2);
                const int k = idx % (FATTN_KQ_STRIDE/2);
