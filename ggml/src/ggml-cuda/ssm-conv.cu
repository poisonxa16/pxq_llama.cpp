...
static __global__ void ssm_conv_f32_kernel(
    ...
    // PXA_MULTISEQ_CKPT: within-seq step run counter (tokens of a seq are contiguous in batch
    // order -- enforced by the delta_net capture gate before a capture tensor is wired in).
    int pxa_prev_seq = -1;
    int pxa_step = -1;
...
static __global__ void ssm_conv_f32_kernel_nc4(
    ...
    // PXA_MULTISEQ_CKPT: within-seq step run counter (see ssm_conv_f32_kernel).
    int pxa_prev_seq = -1;
    int pxa_step = -1;
...
void ggml_cuda_op_ssm_conv(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src4) {
        ...
        // (see ggml_ssm_conv in ggml.c). n_kv == 1 no longer required.
