...
// PXA_MXFP4_DEQ_V2 (2026-07-15): fast MXFP4->f16 dequant for the Pascal/P100 prompt path.
// The stock kernel indexes kvalues_mxfp4 (device-global table) with DIVERGENT per-thread indices --
// measured 150 GB/s effective on GP100 (the lookup serializes), 30%+ of gpt-oss prefill GPU time.
// This version stages the table in shared memory and emits coalesced 16B uint4 stores (thread-pair
// per block_mxfp4): measured 397 GB/s (at the write ceiling; 2.6x). Bit-identical output (same
// float d * table product, same float->half rounding).
static __global__ void dequantize_block_mxfp4_f16_v2(const void * __restrict__ vx, half * __restrict__ yy, const int64_t nblk32) {
    __shared__ float tab[16];
    if (threadIdx.x < 16) tab[threadIdx.x] = (float)kvalues_mxfp4[threadIdx.x];
    __syncthreads();
    constexpr uint32_t uval[2] = { 0x00200000, 0x00400000 };
    const int64_t b = (int64_t)blockIdx.x*(blockDim.x/2) + threadIdx.x/2;
    if (b >= nblk32) return;
    const int il = threadIdx.x & 1;                 // 0: qs[0..7] -> y[0..7]/y[16..23]; 1: qs[8..15] -> y[8..15]/y[24..31]
...
static void dequantize_row_mxfp4_cuda(const void * vx, dst_t * y, const int64_t nrows, const int64_t n_per_row, cudaStream_t stream) {
    ...
    if constexpr (std::is_same<dst_t, half>::value) {
        // PXA_MXFP4_DEQ_V2: fast coalesced smem-table kernel for the f16 path (env =0 rolls back)
        static const bool v2 = [](){ const char * e = getenv("PXA_MXFP4_DEQ_V2"); return !(e && atoi(e) == 0); }();
        if (v2 && k % 32 == 0) {
            const int64_t nblk32 = k/32;
            const int64_t nb = (nblk32 + 63) / 64;
            dequantize_block_mxfp4_f16_v2<<<nb, 128, 0, stream>>>(vx, y, nblk32);
            return;
        }
    }
    const int nb = (k + QK_K - 1) / QK_K;
