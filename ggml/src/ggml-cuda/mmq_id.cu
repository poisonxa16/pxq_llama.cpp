...
bool ggml_cuda_can_use_mmq_id(enum ggml_type type, int cc, int64_t ne11) {
    ...
    if (GGML_CUDA_CC_IS_NVIDIA(cc)) {
        ...
        // PXA_VOLTA_CUBLAS_ID_NE11 (sm_70 prefill, 2026-07-15): Volta has no int8 MMA —
        // grouped expert MMQ runs on DP4A (~1/2 the fp16 tensor-core rate). When set (>0),
        // batches with ne11 >= threshold route to the per-expert dequant + fp16 cuBLAS
        // GemmEx fallback (volta_h884gemm tensor cores; same path the P100 fp16-GEMM fix
        // rides, incl. the MOE_FUSED_UP_GATE precision-alias fix + MXFP4 V2 dequant).
