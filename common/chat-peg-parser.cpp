...
common_peg_parser common_chat_peg_builder::standard_constructed_tools(
    ...
    // PXA tool-tag fix (2026-06-30): Qwen3-Next/Qwen3.5 intermittently emit the tool-DEFINITIONS
    // tag <tools>...</tools> instead of the tool-CALL tag <tool_call>...</tool_call> (~50% of calls),
    // which then leaks as plain text instead of a parsed tool_call. Accept <tools> as an alternate
    ...
    auto section =
        parallel_tool_calls ?
            trigger_rule("tool-call", pxa_open + space() + one_or_more(tool_choices + space()) +
                                          pxa_close) :
            trigger_rule("tool-call", pxa_open + space() + tool_choices + space() + pxa_close);
