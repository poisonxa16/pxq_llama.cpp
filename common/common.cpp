...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--control-vector-ablation") {
        params.control_vector_ablation = true;
        return true;
    }
    if (arg == "--mmproj") {
        CHECK_ARG
...
void gpt_params_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    options.push_back({ "*",           "--ctx-checkpoints-eviction NAME","checkpoint eviction strategy: fifo, variance, or auto (auto=variance). Variance preserves coverage with uniform spacing. (default: variance)" });
...
bool string_parse_kv_override(const char * data, std::vector<llama_model_kv_override> & overrides) {
    ...
    if (strncmp(sep, "int:", 4) == 0) {
        ...
        // distinct from int: so llama-quantize can BAKE a UINT32 key. The loader
        // type-checks GGUF metadata strictly, so a key read as uint32_t must be
        // written as UINT32, not INT32.
        sep += 5;
        kvo.tag = LLAMA_KV_OVERRIDE_TYPE_UINT;
        kvo.val_i64 = (int64_t) std::strtoull(sep, nullptr, 10);
...
struct llama_init_result llama_init_from_gpt_params(gpt_params & params) {
    ...
    if (!params.control_vectors.empty()) {
        ...
        auto cvec = llama_control_vector_load(params.control_vectors);
        if (cvec.n_embd == -1) {
            llama_free(lctx);
            llama_free_model(model);
            return iparams;
        ...
        // PXA: ablation mode — unit-normalize each per-layer direction so the projector
        // cur -= alpha*dir(dir^T cur) is exact; the additive strength becomes alpha.
        float cvec_ablation_alpha = 1.0f;
        if (params.control_vector_ablation) {
            if (!params.control_vectors.empty()) cvec_ablation_alpha = params.control_vectors[0].strength;
            for (size_t off = 0; off + (size_t) cvec.n_embd <= cvec.data.size(); off += (size_t) cvec.n_embd) {
                double nrm = 0.0;
                for (int j = 0; j < cvec.n_embd; j++) nrm += (double) cvec.data[off+j] * cvec.data[off+j];
                nrm = std::sqrt(nrm);
                if (nrm > 1e-8) for (int j = 0; j < cvec.n_embd; j++) cvec.data[off+j] = (float)(cvec.data[off+j] / nrm);
            }
        }
        ...
        if (params.control_vector_ablation) {
            llama_control_vector_set_ablation(lctx, true, cvec_ablation_alpha);
        }
    }
