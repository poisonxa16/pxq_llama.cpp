...
static common_chat_params common_chat_params_init_deepseek_v3_2(const common_chat_template &    tmpl,
    ...
    const std::string THINK_START  = "<think>";
    const std::string THINK_END    = "</think>";
    const std::string FC_NAME      = tmpl.source().find("function_calls") != std::string::npos ? "function_calls" : "tool_calls";
...
// Muse Glimmer: recipient-addressed harmony. An assistant turn is one or more messages, each
// "<|start|>assistant to=<recipient><|message|>{content}{END}" where END is <|eom|> (more messages
// follow) or <|eot|> (end of turn):
//   - chain-of-thought: to=self, terminated by <|eom|>
//   - final answer:     to=user, terminated by <|eot|>
// The generation prompt is just "<|start|>assistant"; the model emits its own " to=...<|message|>".
//
// WHY THIS EXISTS: this template carries no <|channel|>, so the GPT-OSS branch never matched it and it
// fell through to no specialized parser at all. The entire raw stream -- reasoning, recipients and
// markers -- landed in content, with nothing in reasoning_content. Measured before this handler:
// content = " to=self<|message|>...<|eom|><|start|>assistant to=user<|message|>banana"; after: "banana".
//
// Ported from mainline. Deltas, because this fork predates parts of that surface:
//   - no data.message_delimiters   (field does not exist here)
//   - no assistant-prefill continuation (has_continuation/continue_final_message do not exist here)
//   - tool-arg string values are not wrapped in p.ac() (no such helper here) -- parsing is unaffected,
//     only the lazy-grammar stop hint is lost
static common_chat_params common_chat_params_init_muse_glimmer(const common_chat_template &          tmpl,
    ...
    auto parser = build_chat_peg_parser([&](common_chat_peg_builder & p) {
        ...
        if (has_tools && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE) {
            auto string_value = p.tool_arg_string_value(p.until("</atem:parameter>")) +
...
std::optional<common_chat_params> common_chat_try_specialized_template(
    ...
    // Muse Glimmer - recipient-based harmony (" to=self"/" to=user" with <|eom|>/<|eot|> terminators).
    // Carries no <|channel|>, so the GPT-OSS branch above can never match it.
...
common_chat_msg common_chat_peg_parse(const common_peg_arena &          src_parser,
    ...
    common_peg_parse_context ctx(effective_input2, flags);
