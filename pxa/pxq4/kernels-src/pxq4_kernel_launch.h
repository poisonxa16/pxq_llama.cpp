...
// sync; capture-safe). slabs [E, panels, kslabs, 1088], anchor [E, panels, 64]. Rows whose id
// is outside [0, E) produce zeros. Values per row are bit-identical to pxq4_launch_mmv_f16 on
// that expert's 2-D slice.
void pxq4_launch_moe_mmv_f16(const uint8_t * slabs, const void * anchor, const void * x,
                             ...
                             int kslabs, bool vecx, cudaStream_t stream);
// Fused-split twin (grid.x = nfix); part = S*panels*nfix*256 floats, ctr = S*panels unsigned,
// zero on entry and exit. Same barrier contract as pxq4_launch_mmv_fused_f16.
...
// Plain-fp16 dense decode GEMV / multi-token GEMM: out[M,N] = x[M,K] @ w[N,K]^T,
// M 1..8, K % 8 == 0. fp32 accumulation, warp-per-row, fully coalesced weight reads.
void pxa_launch_f16_mmv_mt(const void * w, const void * x, void * out,
                           int M, int N, int K, cudaStream_t stream);

// v10 smem-staged fp16 mmv, M 4..16 (x tiled through shared memory; fixes the M>=4
// bandwidth sag of pxa_launch_f16_mmv_mt).
void pxa_launch_f16_mmv_smem(const void * w, const void * x, void * out,
                             int M, int N, int K, cudaStream_t stream);

// Fused 4-bit prefill GEMM, RAG+PIPE arm: out[M,N] = x[M,K] @ W^T with the dequant folded
// into the tile (fp16 accumulation — quality-gate before enabling; sm_60-only by policy).
