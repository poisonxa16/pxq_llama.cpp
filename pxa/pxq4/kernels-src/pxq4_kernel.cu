...
#include "pxq4_moe_kernel.cuh"
#include "pxq4_f16_kernel.cuh"
#include "pxq4_gemm2d_kernel.cuh"
...
// ---------------------------------------------------------------------------------------------
// MoE expert-indexed mmv launchers (see pxq4_moe_kernel.cuh for the capture-safety and
// bit-exactness arguments). S = rows = tokens * topk; ids[s] selects the expert.
// ---------------------------------------------------------------------------------------------
void pxq4_launch_moe_mmv_f16(const uint8_t * slabs, const void * anchor, const void * x,
                             const int32_t * ids, void * out, int S, int E, int panels,
                             int kslabs, bool vecx, cudaStream_t stream) {
    const int    R    = panels * PXQ4_BM;
    const int    K    = kslabs * PXQ4_QK;
    const size_t smem = (size_t)pxq4_mmv_smem_bytes(kslabs);
    if (S < 1 || S > 65535 || panels < 1 || panels > 2147483647 / (E > 0 ? E : 1)) {
        fprintf(stderr, "pxq4: moe mmv grid invalid: S=%d panels=%d E=%d\n", S, panels, E);
        abort();
    }
    const dim3 grid((unsigned)panels, (unsigned)S, 1u);
    if (vecx) {
        k_pxq4_moe_mmv<true><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, ids, (__half *)out,
            R, K, E, panels);
    } else {
        k_pxq4_moe_mmv<false><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, ids, (__half *)out,
            R, K, E, panels);
    }
    PXQ4_CUDA_CHECK(cudaGetLastError());
...
void pxq4_launch_moe_mmv_fused_f16(const uint8_t * slabs, const void * anchor, const void * x,
                                   const int32_t * ids, float * part, unsigned * ctr,
                                   void * out, int S, int E, int panels, int kslabs,
                                   bool vecx, cudaStream_t stream) {
    const int    R    = panels * PXQ4_BM;
    const int    K    = kslabs * PXQ4_QK;
    const int    nfix = pxq4_mmv_nfix(kslabs);
    const size_t smem = (size_t)pxq4_mmv_smem_bytes(kslabs);
    if (nfix < 2) {
        fprintf(stderr, "pxq4: fused moe mmv requires nfix >= 2 (got %d)\n", nfix);
        abort();
    }
    if (nfix != pxq4_canon_nfix(kslabs, PXQ4_CANON_CMAX) || panels < 1) {
        fprintf(stderr, "pxq4: fused moe mmv nfix/panels inconsistent (nfix=%d panels=%d "
                        "kslabs=%d)\n", nfix, panels, kslabs);
        abort();
    }
    if (panels > 65535 || S > 65535 || S < 1) {
        fprintf(stderr, "pxq4: fused moe mmv grid limit exceeded: panels=%d S=%d\n", panels, S);
        abort();
    }
    const dim3 grid((unsigned)nfix, (unsigned)panels, (unsigned)S);   // c fastest
    if (vecx) {
        k_pxq4_moe_mmv_fused<true><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, ids, part, ctr,
            (__half *)out, R, K, E, nfix, panels);
    } else {
        k_pxq4_moe_mmv_fused<false><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, ids, part, ctr,
            (__half *)out, R, K, E, nfix, panels);
    }
    PXQ4_CUDA_CHECK(cudaGetLastError());
...
// Plain-fp16 dense decode GEMV/MT-GEMM (see pxq4_f16_kernel.cuh). M 1..8.
void pxa_launch_f16_mmv_mt(const void * w, const void * x, void * out,
                           int M, int N, int K, cudaStream_t stream) {
    const unsigned blocks = (unsigned)((N + PXA_F16_ROWS_PER_BLOCK - 1) / PXA_F16_ROWS_PER_BLOCK);
    switch (M) {
#define PXA_F16_CASE(MTV)                                                                     \
        case MTV:                                                                             \
            k_pxa_f16_mmv_mt<MTV><<<blocks, 256, 0, stream>>>(                                \
                (const __half *)w, (const __half *)x, (__half *)out, N, K);                   \
            break;
        PXA_F16_CASE(1) PXA_F16_CASE(2) PXA_F16_CASE(3) PXA_F16_CASE(4)
        PXA_F16_CASE(5) PXA_F16_CASE(6) PXA_F16_CASE(7) PXA_F16_CASE(8)
#undef PXA_F16_CASE
        default:
            fprintf(stderr, "pxq4: f16 mmv M out of range: %d\n", M);
            abort();
    }
    PXQ4_CUDA_CHECK(cudaGetLastError());
}

// v10 smem-staged f16 mmv for M 4..16 (see pxq4_f16_kernel.cuh).
void pxa_launch_f16_mmv_smem(const void * w, const void * x, void * out,
                             int M, int N, int K, cudaStream_t stream) {
    const unsigned blocks = (unsigned)((N + PXA_F16_ROWS_PER_BLOCK - 1) / PXA_F16_ROWS_PER_BLOCK);
    switch (M) {
#define PXA_F16S_CASE(MTV)                                                                    \
        case MTV:                                                                             \
            k_pxa_f16_mmv_smem<MTV><<<blocks, 256, 0, stream>>>(                              \
                (const __half *)w, (const __half *)x, (__half *)out, N, K);                   \
            break;
        PXA_F16S_CASE(4) PXA_F16S_CASE(5) PXA_F16S_CASE(6) PXA_F16S_CASE(7)
        PXA_F16S_CASE(8) PXA_F16S_CASE(9) PXA_F16S_CASE(10) PXA_F16S_CASE(11)
        PXA_F16S_CASE(12) PXA_F16S_CASE(13) PXA_F16S_CASE(14) PXA_F16S_CASE(15)
        PXA_F16S_CASE(16)
#undef PXA_F16S_CASE
        default:
            fprintf(stderr, "pxq4: f16 smem mmv M out of range: %d\n", M);
            abort();
    }
    PXQ4_CUDA_CHECK(cudaGetLastError());
}

// Fused 4-bit prefill GEMM (sm_60 only by policy; see pxq4_gemm2d_kernel.cuh).
void pxq4_launch_gemm2d_f16(const uint8_t * slabs, const void * anchor, const void * x,
                            void * out, int M, int panels, int kslabs, cudaStream_t stream) {
    const int N = panels * PXQ4_BM;
    const int K = kslabs * PXQ4_QK;
    const unsigned gy = (unsigned)((M + PXQ4_G2D_BN - 1) / PXQ4_G2D_BN);
    if (gy > 65535u || panels > 2147483647) {
        fprintf(stderr, "pxq4: gemm2d grid limit exceeded (M=%d panels=%d)\n", M, panels);
        abort();
    }
    const dim3 grid((unsigned)panels, gy, 1u);
    k_pxq4_gemm2d<true, true><<<grid, 64, 0, stream>>>(
        slabs, (const __half *)anchor, (const __half *)x, (__half *)out, M, N, K);
    PXQ4_CUDA_CHECK(cudaGetLastError());
}
