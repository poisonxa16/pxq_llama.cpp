...
Component B ("runtime"), design note ``09-chosen-design.md`` §6.7.  The frozen op ABI is the design note §7.1:
...
------------------------------------------------------------------------------
DIVERGENCE FROM THE DESIGN NOTE §7.3, RESOLVED IN FAVOUR OF THE FROZEN ABI
------------------------------------------------------------------------------
Design note §7.3 anticipated that ``k_pxq6_mmv`` consumes fp32 activations and
...
arguments are checked ``at::kHalf``), i.e. it matches the FROZEN §7.1 ABI
...
that the §6.7 signature is honoured, but it allocates nothing.
...
def _register_fakes() -> None:
    ...
    if hasattr(torch.ops.pxq4, "f16_mmv_out"):
        @torch.library.register_fake("pxq4::f16_mmv_out")
        def _f16_mmv_out_meta(out, x, w):  # noqa: ANN001, ANN202
            torch._check(x.dim() == 2 and out.dim() == 2 and w.dim() == 2)
            torch._check(x.shape[1] == w.shape[1])
            torch._check(out.shape[0] == x.shape[0] and out.shape[1] == w.shape[0])
            return None

    if hasattr(torch.ops.pxq4, "moe_mmv_out"):
        @torch.library.register_fake("pxq4::moe_mmv_out")
        def _moe_mmv_out_meta(out, x, ids, slabs, anchor):  # noqa: ANN001, ANN202
            torch._check(slabs.dim() == 4 and anchor.dim() == 3)
            torch._check(x.dim() == 2 and out.dim() == 2 and ids.dim() == 1)
            torch._check(x.shape[1] == slabs.shape[2] * 32)
            torch._check(out.shape[0] == x.shape[0])
            torch._check(ids.shape[0] == x.shape[0])
            torch._check(out.shape[1] == slabs.shape[1] * 64)
            return None

    if hasattr(torch.ops.pxq4, "moe_mmv_out_mono"):
        @torch.library.register_fake("pxq4::moe_mmv_out_mono")
        def _moe_mmv_out_mono_meta(out, x, ids, slabs, anchor):  # noqa: ANN001, ANN202
            torch._check(slabs.dim() == 4 and anchor.dim() == 3)
            torch._check(x.dim() == 2 and out.dim() == 2 and ids.dim() == 1)
            return None
...
class PXQ4Workspace:
    ...
    def _resolve_device(cls, device: "torch.device | None") -> torch.device:
        # Design note §6.6 calls dequant_view(N, K) with no device; a TP worker owns
