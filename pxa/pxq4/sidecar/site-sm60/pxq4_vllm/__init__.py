...
How the hook works (verified against the vLLM source tree this plugin targets):
  ...
  * ``vllm/plugins/__init__.py:14`` declares the group name
    ...
    ``pxq4_vllm-0.1.0.dist-info/entry_points.txt`` inside the sidecar site
    directory (``sidecar/site-sm60`` in this package) plus ``PYTHONPATH`` is
    enough -- nothing needs to be pip-installed into the container image, whose
    root filesystem may be read-only or full.
