...
--------------------------------------------------------------------------
Verified against the vLLM source tree this plugin targets.
...
# The P1 allow-list from the design note (``09-chosen-design.md``) §3.  Kept here as documentation and as the
...
# asks for.  This list is authoritative for P2b: the LM head is fp16 in
...
#
# Why it cannot be served, verified in the vLLM source -- TWO independent
...
# That is P3 work, not a decorator tweak.  ``embed_tokens`` is listed
...
class PXQ4Config(QuantizationConfig):
    ...
    Design note §3.1 invariant: a module in ``pxq4_modules`` is *uniformly* PXQ4
    ...
    def validate_fused_uniformity(self) -> None:
        """Enforce the config half of the design note §3.1 invariant.
        ...
        for fused_name, shard_names in mapping.items():
            ...
            if bad:
                raise ValueError(
                    ...
                    "be uniformly PXQ4 (the design note §3.1) -- there is no "
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            ...
            # Ignore is checked FIRST (the design note §6.5). The two lists overlap
        ...
        # VRAM. PXQ4MoEMethod is the fix; see ``moe.py``.
        if type(layer).__name__ in ("FusedMoE", "RoutedExperts") or hasattr(layer, "moe_config"):
            if _matches(prefix, self.pxq4_modules):
                ...
                logger.info("pxq4 dispatch: %-64s -> moe", prefix)
                return PXQ4MoEMethod(self, moe_cfg)
            return None
