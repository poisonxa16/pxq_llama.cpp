...
Component B ("runtime"), design note ``09-chosen-design.md`` §6.6.  This is where TP correctness lives.
...
The chosen design (the design note §5.3) resolves that OFFLINE: the converter emits
...
model (the design note §3.1).  P2c dissolves that case by re-encoding k/v instead.
...
==============================================================================
VERIFIED FACTS THIS FILE DEPENDS ON (all read in the vLLM source tree this plugin targets)
==============================================================================
...
class PXQ4LinearMethod(LinearMethodBase):
    ...
    Design note §3.1 invariant: uniformly PXQ4 across ALL of
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        (the design note §5.3) -- so unlike the TurboMind template there is nothing to
        ...
        if sentinel_slabs:
            ...
            raise ValueError(
                ...
                "MXFP4, ...), i.e. the design note §3.1 uniformity invariant "
        ...
        if _SENTINEL_ENABLED and bool(torch.isnan(anchor).any()):
            ...
            raise ValueError(
                ...
                "module that is not uniformly PXQ4 (the design note §3.1) -- e.g. "
        ...
        # params). Standard torch behaviour; not exercised by the host-side
        # gates, which run on a machine with no torch installed.
    ...
    def apply(
        ...
        if getattr(layer, "pxq4_linear_op", False):
            ...
            # tested on a GPU by the host-side gates -- that is gate G8. Fallback:
