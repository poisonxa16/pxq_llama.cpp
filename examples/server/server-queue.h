...
struct server_queue {
    ...
    // PXA_DEFER_PUMP_v1: returns true when at least one slot can take work. Lets the loop
    // re-dispatch parked (deferred) tasks instead of relying solely on notify_slot_changed(),
    // which only fires from release_slots() — if a task is ever deferred while slots are
    // already idle (observed live 2026-06-12: completion tasks parked forever, health fine,
    // "all slots are idle" logging every tick), nothing revives it. With the pump, the loop
    // heartbeats every 500ms while tasks are parked and re-dispatches as soon as a slot is
    // free — starvation becomes structurally impossible (worst case 500ms added latency).
    std::function<bool(void)>                 callback_slot_available;
    ...
    // PXA_DEFER_PUMP_v1: register the "any slot can take work" probe
    void on_slot_available(std::function<bool(void)> callback) {
        callback_slot_available = std::move(callback);
    }
