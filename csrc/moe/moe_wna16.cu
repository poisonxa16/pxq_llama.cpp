...
#pragma unroll
    ...
    for (int m = 0; m < num_valid_tokens; ++m) {
      ...
      // PASCAL PORT: fp16/bf16 atomicAdd needs sm_70+. This grouped-quant MoE
      // kernel is never dispatched on Pascal (dense models only); trap so a
      // future misuse is loud instead of silently wrong.
      __trap();
#else
