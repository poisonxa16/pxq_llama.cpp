...
// Keep the second K16 V fragment above the independent P matrix-A loads. The
// empty compiler fence must lower to no SASS instruction.
__device__ __forceinline__ void update_phase_reuse_pv_panel_load_fence(
    const __half* __restrict__ probability_top,
    const __half* __restrict__ probability_bottom,
    const __half* __restrict__ value_panel, int d_offset,
    AccumulatorFragment& accumulator_top,
...
void sm70_native_bm32_allp_pv_load_fence_baseline(
...
extern "C" __global__ __launch_bounds__(kCandidateThreads, 2)
void sm70_native_bm32_allp_pv_load_fence_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
  // This path differs from the accepted pair-scratch baseline only by the
  // second-K16 PV compiler fence.
  __shared__ __align__(16) CandidateShared shared;
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      ...
      update_phase_reuse_pv_panel_load_fence(
          shared.probability_top[panel], shared.probability_bottom[panel],
          value_group +
              (shared.block_index * kBlockN + panel * kSoftmaxPanelN) * kD,
          d_offset, accumulator_top, accumulator_bottom);
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; same accepted pair-scratch path with a second-K16 PV compiler fence only\",\n";
...
int run(const Args& args) {
  ...
  const KernelResources baseline_resources = query_resources(
      sm70_native_bm32_allp_pv_load_fence_baseline, kBaselineThreads);
  const KernelResources candidate_resources = query_resources(
      sm70_native_bm32_allp_pv_load_fence_candidate, kCandidateThreads);
  ...
  auto launch_baseline = [&] {
    sm70_native_bm32_allp_pv_load_fence_baseline<<<baseline_grid,
                                                kBaselineThreads>>>(
  ...
  auto launch_candidate = [&] {
    sm70_native_bm32_allp_pv_load_fence_candidate<<<candidate_grid,
                                                 kCandidateThreads>>>(
