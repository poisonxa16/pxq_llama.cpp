...
// 41,744-byte all-P layout without a shared loop counter.
struct alignas(16) Shared {
  ...
  int layout_reserved[4];
};

static_assert(sizeof(Shared) == 41744,
              "all-P shared layout changed unexpectedly");
static_assert(sizeof(Shared) <= 48 * 1024,
              "all-P shared storage exceeds the 48 KiB gate");
...
// This is the accepted pair-slab mapping. Each two-QK-warp pair owns a
// disjoint 32-column slab and uses the score allocation as temporary storage.
...
__device__ __forceinline__ void qk_pair_scratch_and_store(
    Shared* shared, const __half* __restrict__ key_group, int block_n,
    AccumulatorFragment& pv_accumulator_top,
    AccumulatorFragment& pv_accumulator_bottom) {
  const int qk_warp = threadIdx.x >> 5;
  if (qk_warp >= kQKWarps) {
    return;
  }
  const int n_offset = qk_warp * kPanelN;
  spill_qk_warp_pv_accumulators(shared->score, pv_accumulator_top,
                                pv_accumulator_bottom);
  asm volatile("" ::: "memory");
  AccumulatorFragment qk_top;
  AccumulatorFragment qk_bottom;
  qk_pair_accumulate(shared->query, shared->query + kQPanelElements,
                     key_group + (block_n * kBlockN + n_offset) * kD, qk_top,
                     qk_bottom);
  asm volatile("" ::: "memory");
  reload_qk_warp_pv_accumulators(shared->score, pv_accumulator_top,
                                 pv_accumulator_bottom);
...
__device__ __forceinline__ void update_pv_block(
    const Shared* shared, const __half* __restrict__ value_group, int block_n,
    AccumulatorFragment& accumulator_top,
    AccumulatorFragment& accumulator_bottom) {
...
__device__ __forceinline__ void initialize_shared(
    const __half* __restrict__ query, int group, Shared* shared) {
  const __half* query_group = query + static_cast<int64_t>(group) * kQElements;
  stage_swizzled_q_panel(query_group, shared->query, threadIdx.x, kThreads);
...
void sm70_native_bm32_allp_crossblock_baseline(
  ...
  initialize_shared(query, group, &shared);
  __syncthreads();
  ...
  // Matching local-loop baseline: QK, all-P, then all sixteen PV warps.
  for (int block_n = 0; block_n < nblocks; ++block_n) {
    qk_pair_scratch_and_store(&shared, key_group, block_n, accumulator_top,
                              accumulator_bottom);
    __syncthreads();
    make_all_probability_panels(&shared);
    __syncthreads();
    update_pv_block(&shared, value_group, block_n, accumulator_top,
                    accumulator_bottom);
    __syncthreads();
  }
...
void sm70_native_bm32_allp_crossblock_candidate(
  ...
  // This local timeline loop has one QK call site. Its first iteration is the
  // QK(0) prologue; later iterations are steady QK(n)+PV(n-1) steps. There is
  // no runtime readiness flag or shared loop state.
  for (int qk_block = 0; qk_block < nblocks; ++qk_block) {
    const int warp = threadIdx.x >> 5;
    if (qk_block != 0) {
      make_all_probability_panels(&shared);
      __syncthreads();
    ...
    if (warp < kQKWarps) {
      const __half* key_group =
          key + static_cast<int64_t>(group) * nblocks * kBlockN * kD;
      qk_pair_scratch_and_store(&shared, key_group, qk_block, accumulator_top,
                                accumulator_bottom);
    }
    if (qk_block == 0) {
      __syncthreads();
    ...
    // score(qk_block) is written while warps 8..15 update D[128:256] for
    // PV(qk_block-1). The first CTA barrier publishes that score and the
    // second closes PV(qk_block-1) after warps 0..7 update D[0:128].
    const int pv_block = qk_block - 1;
    if (warp >= kQKWarps) {
      const __half* value_group =
          value + static_cast<int64_t>(group) * nblocks * kBlockN * kD;
      update_pv_block(&shared, value_group, pv_block, accumulator_top,
                      accumulator_bottom);
    }
    __syncthreads();
    if (warp < kQKWarps) {
  ...
  // Drain: the final block has no next QK and all sixteen PV warps run.
  const int last_block = nblocks - 1;
  make_all_probability_panels(&shared);
  __syncthreads();
  const __half* value_group =
      value + static_cast<int64_t>(group) * nblocks * kBlockN * kD;
  update_pv_block(&shared, value_group, last_block, accumulator_top,
                  accumulator_bottom);
  __syncthreads();
...
KernelResources query_resources(Kernel kernel) {
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "  \"hmma_contract\": {\"unit\": \"wmma::mma_sync operations\", "
            ...
            << ", \"total_per_block\": " << kWmmaPerBlock
            << ", \"baseline_total_per_group\": "
            << args.nblocks * kWmmaPerBlock
            << ", \"candidate_total_per_group\": "
            << args.nblocks * kWmmaPerBlock
            << ", \"per_group_equal\": true},\n";
...
int run(const Args& args) {
  ...
  CUDA_CHECK(cudaDeviceGetAttribute(&sm_count, cudaDevAttrMultiProcessorCount,
                                    args.device));
  const KernelResources baseline_resources =
      query_resources(sm70_native_bm32_allp_crossblock_baseline);
  const KernelResources candidate_resources =
      query_resources(sm70_native_bm32_allp_crossblock_candidate);
  ...
  auto launch_baseline = [&] {
    sm70_native_bm32_allp_crossblock_baseline<<<grid, kThreads>>>(
        device_query, device_key, device_value, device_baseline, args.groups,
        args.nblocks);
    CUDA_CHECK(cudaGetLastError());
  };
  auto launch_candidate = [&] {
    sm70_native_bm32_allp_crossblock_candidate<<<grid, kThreads>>>(
