...
// P keeps the same four 16x8 groups, but rotates each group's row slot so
// concurrent softmax stores do not begin in the same shared-memory bank.
__device__ __forceinline__ int group_skewed_probability_matrix_a_offset(
    int row, int column) {
  const int group = column / kProbabilityGroupN;
  const int inner = column & 7;
  const int slot = (swizzled_row_slot(row) + group) & (kPanelM - 1);
  return group * kPanelM * 8 + slot * 8 + inner;
}
...
// LDS.128 operations, one for each adjacent 8-column P group.
__device__ __forceinline__ void load_group_skewed_probability_matrix_a_fragment(
    MatrixAFragment& fragment, const __half* __restrict__ probability,
    int k_offset) {
  const int lane = threadIdx.x & 31;
  const int row = (lane & 3) + ((lane >> 4) & 1) * 4 +
                  ((lane >> 2) & 1) * 8;
  const int base_slot = swizzled_row_slot(row);
  const int first_group = k_offset / kProbabilityGroupN;
  const int second_group = first_group + 1;
  uint32_t address = static_cast<uint32_t>(__cvta_generic_to_shared(
      probability + first_group * kPanelM * 8 +
      ((base_slot + first_group) & (kPanelM - 1)) * 8));
  uint32_t* words = reinterpret_cast<uint32_t*>(&fragment);
  asm volatile("ld.shared.v4.u32 {%0, %1, %2, %3}, [%4];"
               : "=r"(words[0]), "=r"(words[1]), "=r"(words[2]),
                 "=r"(words[3])
               : "r"(address)
               : "memory");
  address = static_cast<uint32_t>(__cvta_generic_to_shared(
      probability + second_group * kPanelM * 8 +
      ((base_slot + second_group) & (kPanelM - 1)) * 8));
  asm volatile("ld.shared.v4.u32 {%0, %1, %2, %3}, [%4];"
               : "=r"(words[4]), "=r"(words[5]), "=r"(words[6]),
                 "=r"(words[7])
               : "r"(address)
...
__device__ __forceinline__ float make_group_skewed_probability_row(
    const float* __restrict__ score_row, __half* __restrict__ probability,
    int probability_row, float* __restrict__ row_max,
    float* __restrict__ row_sum, int state_row, int panel) {
  const int lane = threadIdx.x & 31;
...
#pragma unroll
  ...
  if (lane < kSoftmaxPanelN / 4) {
    ...
    const int column = lane * 4;
    __half2* first_pair = reinterpret_cast<__half2*>(
        probability + group_skewed_probability_matrix_a_offset(
...
__device__ __forceinline__ void update_group_skewed_probability_pv_panel(
    const __half* __restrict__ probability_top,
    const __half* __restrict__ probability_bottom,
    const __half* __restrict__ value_panel, int d_offset,
    AccumulatorFragment& accumulator_top,
    AccumulatorFragment& accumulator_bottom) {
  {
    ...
    PVMatrixBFragment b_fragment;
    nvcuda::wmma::load_matrix_sync(b_fragment, value_panel + d_offset, kD);
    load_group_skewed_probability_matrix_a_fragment(a_fragment,
                                                     probability_top, 0);
    nvcuda::wmma::mma_sync(accumulator_top, a_fragment, b_fragment,
                           accumulator_top);
    load_group_skewed_probability_matrix_a_fragment(a_fragment,
  ...
  }
  {
    ...
    nvcuda::wmma::mma_sync(accumulator_top, a_fragment, b_fragment,
                           accumulator_top);
    load_group_skewed_probability_matrix_a_fragment(
...
void sm70_native_bm32_allp_p_group_skew_baseline(
...
extern "C" __global__ __launch_bounds__(kCandidateThreads, 2)
void sm70_native_bm32_allp_p_group_skew_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      const int phase_warp = threadIdx.x >> 5;
      const float top_exp_diff = make_group_skewed_probability_row(
          shared.score + phase_warp * kBlockN,
          shared.probability_top[panel], phase_warp, shared.row_max,
          shared.row_sum, phase_warp, panel);
      const float bottom_exp_diff = make_group_skewed_probability_row(
          shared.score + (kPanelM + phase_warp) * kBlockN,
          shared.probability_bottom[panel], phase_warp, shared.row_max,
          shared.row_sum, kPanelM + phase_warp, panel);
      if ((threadIdx.x & 31) == 0) {
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      ...
      update_group_skewed_probability_pv_panel(
          shared.probability_top[panel], shared.probability_bottom[panel],
          value_group +
              (shared.block_index * kBlockN + panel * kSoftmaxPanelN) * kD,
          d_offset, accumulator_top, accumulator_bottom);
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "  \"probability_layout\": {"
               ...
               "\"panel_fp16\": 512, \"top_bottom_panels\": 8, "
               "\"top_bottom_bytes\": 8192, "
               "\"baseline_offset\": \"group*128 + swizzled_slot(row)*8 + inner; group=column/8\", "
               "\"candidate_offset\": \"group*128 + ((swizzled_slot(row)+group)&15)*8 + inner; group=column/8\", "
               "\"bank_proof\": \"32-bit store bank=(4*swizzled_slot+4*group+inner/2) mod 32; groups 0..3 map to distinct banks, reducing the static four-way same-bank group collision to one-way\", "
               "\"pv_matrix_a_loader\": \"P-only; two ld.shared.v4.u32 (LDS.128) per M16xK16 fragment; Q loader unchanged\"},\n";
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"1 CTA/group; BM32, 512 threads; accepted all-P pair-scratch QK, softmax, PV\",\n";
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; same accepted pair-scratch path with P group-skew layout only\",\n";
...
int run(const Args& args) {
  ...
  const KernelResources baseline_resources = query_resources(
      sm70_native_bm32_allp_p_group_skew_baseline, kBaselineThreads);
  const KernelResources candidate_resources = query_resources(
      sm70_native_bm32_allp_p_group_skew_candidate, kCandidateThreads);
  ...
  auto launch_baseline = [&] {
    sm70_native_bm32_allp_p_group_skew_baseline<<<baseline_grid,
                                                kBaselineThreads>>>(
  ...
  auto launch_candidate = [&] {
    sm70_native_bm32_allp_p_group_skew_candidate<<<candidate_grid,
                                                 kCandidateThreads>>>(
