...
// One score buffer and one 32-column probability panel replace the async
// path's next-block score buffers and producer/consumer counter state.
struct alignas(16) CandidateShared {
  __half query[kQElements];
  float score[kM * kBlockN];
  __half probability_top[kPPanelElements];
  __half probability_bottom[kPPanelElements];
  float row_max[kM];
  float row_sum[kM];
  float row_exp_diff[kM];
  int block_index;
};

static_assert(sizeof(CandidateShared) == 35216,
              "phase-reuse shared layout changed unexpectedly");
...
void sm70_native_bm32_full_phase_baseline(
  ...
  const __half* value_group = value + static_cast<int64_t>(group) * nblocks *
                                          kBlockN * kD;
  __half* output_group = output + static_cast<int64_t>(group) *
                                       kOutputElements +
                         m_panel * kPanelM * kD;
...
void sm70_native_bm32_full_phase_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      ...
      const float top_exp_diff = make_probability_row(
          shared.score + phase_warp * kBlockN, shared.probability_top,
      ...
      const float bottom_exp_diff = make_probability_row(
          ...
          shared.probability_bottom, phase_warp, shared.row_max,
          shared.row_sum, kPanelM + phase_warp, panel);
      if ((threadIdx.x & 31) == 0) {
        shared.row_exp_diff[phase_warp] = top_exp_diff;
        shared.row_exp_diff[kPanelM + phase_warp] = bottom_exp_diff;
      }
      __syncthreads();
      ...
      update_phase_reuse_pv_panel(
          shared.probability_top, shared.probability_bottom,
          value_group +
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  const bool resource_pass = candidate_resource_gate(candidate_resources);
  ...
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"2 CTA/group; BM16, 512 threads; serial QK-softmax-PV with shared output\",\n";
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; phase-reuse QK, softmax, PV\",\n";
  std::cout << "    \"candidate_qk\": \"warps 0..7; one K fragment then top/bottom M16\",\n";
  std::cout << "    \"candidate_softmax\": \"16 warps; each handles matching top/bottom rows with 32-lane reductions\",\n";
  std::cout << "    \"candidate_pv\": \"16 warps; one D16 and two FP32 accumulators, one V fragment then top/bottom M16\",\n";
  std::cout << "    \"candidate_cross_block_overlap\": \"disabled\",\n";
  std::cout << "    \"candidate_score_buffers\": 1,\n";
  std::cout << "    \"candidate_probability_panels\": 1,\n";
  std::cout << "    \"candidate_shared_layout_bytes\": "
...
int run(const Args& args) {
  ...
  const KernelResources baseline_resources = query_resources(
      sm70_native_bm32_full_phase_baseline, kBaselineThreads);
  const KernelResources candidate_resources = query_resources(
      sm70_native_bm32_full_phase_candidate, kCandidateThreads);
  const Exactness no_exactness;
  const TimingSummary no_timing;
  const PairSummary no_pairs;
  if (!candidate_resource_gate(candidate_resources)) {
  ...
  auto launch_baseline = [&] {
    sm70_native_bm32_full_phase_baseline<<<baseline_grid, kBaselineThreads>>>(
        device_query, device_key, device_value, device_baseline, args.groups,
        args.nblocks);
    CUDA_CHECK(cudaGetLastError());
  };
  auto launch_candidate = [&] {
    sm70_native_bm32_full_phase_candidate<<<candidate_grid,
                                             kCandidateThreads>>>(
