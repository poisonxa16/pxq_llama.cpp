...
constexpr int kProbabilityGroupN = 8;
constexpr int kProbabilityGroups = kSoftmaxPanelN / kProbabilityGroupN;
...
static_assert(kPPanelElements == kProbabilityGroups * kPanelM * kProbabilityGroupN);
...
// Four 16x8 groups use a 136-half pitch: 3 * 136 + 128 = 536 fp16.
struct alignas(16) CandidateShared {
  __half query[kQElements];
  float score[kM * kBlockN];
  __half probability_top[kSoftmaxPanelsPerBlock]
                         [3 * 136 + kPanelM * kProbabilityGroupN];
  __half probability_bottom[kSoftmaxPanelsPerBlock]
                            [3 * 136 + kPanelM * kProbabilityGroupN];
  float row_max[kM];
  float row_sum[kM];
  float row_exp_diff[kSoftmaxPanelsPerBlock][kM];
...
static_assert(sizeof(BaselineShared) == 41744,
              "all-P pair-scratch baseline shared layout changed unexpectedly");
static_assert(sizeof(CandidateShared) == 42128,
              "P group-pad shared layout changed unexpectedly");
static_assert(sizeof(CandidateShared) - sizeof(BaselineShared) == 384,
              "P group-pad must add exactly 384 bytes");
...
// Each P group advances by 136 fp16 (272 bytes, 68 banks), so its base
// advances four banks modulo 32 without changing the swizzled row mapping.
__device__ __forceinline__ int group_padded_probability_matrix_a_offset(
    int row, int column) {
  const int group = column / kProbabilityGroupN;
  const int inner = column & 7;
  return group * 136 + swizzled_row_slot(row) * 8 + inner;
}

// This is intentionally P-specific: Q retains the production swizzled
// layout and loader above. Each 16-column matrix-A fragment still uses two
// LDS.128 operations; the second group address is the first plus 272 bytes.
__device__ __forceinline__ void load_group_padded_probability_matrix_a_fragment(
    MatrixAFragment& fragment, const __half* __restrict__ probability,
    int k_offset) {
  const int lane = threadIdx.x & 31;
  const int row = (lane & 3) + ((lane >> 4) & 1) * 4 +
                  ((lane >> 2) & 1) * 8;
  const int slot = swizzled_row_slot(row);
  const int first_group = k_offset / kProbabilityGroupN;
  uint32_t address = static_cast<uint32_t>(__cvta_generic_to_shared(
      probability + first_group * 136 + slot * 8));
  uint32_t* words = reinterpret_cast<uint32_t*>(&fragment);
  asm volatile("ld.shared.v4.u32 {%0, %1, %2, %3}, [%4];"
               : "=r"(words[0]), "=r"(words[1]), "=r"(words[2]),
                 "=r"(words[3])
...
__device__ __forceinline__ float make_group_padded_probability_row(
    const float* __restrict__ score_row, __half* __restrict__ probability,
    int probability_row, float* __restrict__ row_max,
    float* __restrict__ row_sum, int state_row, int panel) {
  const int lane = threadIdx.x & 31;
...
#pragma unroll
  ...
  if (lane < kSoftmaxPanelN / 4) {
    ...
    const int column = lane * 4;
    __half2* first_pair = reinterpret_cast<__half2*>(
        probability + group_padded_probability_matrix_a_offset(
...
__device__ __forceinline__ void update_group_padded_probability_pv_panel(
    const __half* __restrict__ probability_top,
    const __half* __restrict__ probability_bottom,
    const __half* __restrict__ value_panel, int d_offset,
    AccumulatorFragment& accumulator_top,
    AccumulatorFragment& accumulator_bottom) {
  {
    ...
    PVMatrixBFragment b_fragment;
    nvcuda::wmma::load_matrix_sync(b_fragment, value_panel + d_offset, kD);
    load_group_padded_probability_matrix_a_fragment(a_fragment,
                                                     probability_top, 0);
    nvcuda::wmma::mma_sync(accumulator_top, a_fragment, b_fragment,
                           accumulator_top);
    load_group_padded_probability_matrix_a_fragment(a_fragment,
                                                     probability_bottom, 0);
    nvcuda::wmma::mma_sync(accumulator_bottom, a_fragment, b_fragment,
                           accumulator_bottom);
  }
  {
    ...
    nvcuda::wmma::load_matrix_sync(
        b_fragment, value_panel + kPanelK * kD + d_offset, kD);
    load_group_padded_probability_matrix_a_fragment(a_fragment,
                                                     probability_top, kPanelK);
    nvcuda::wmma::mma_sync(accumulator_top, a_fragment, b_fragment,
                           accumulator_top);
    load_group_padded_probability_matrix_a_fragment(
        a_fragment, probability_bottom, kPanelK);
    nvcuda::wmma::mma_sync(accumulator_bottom, a_fragment, b_fragment,
                           accumulator_bottom);
  }
}
...
void sm70_native_bm32_allp_p_group_pad_baseline(
...
extern "C" __global__ __launch_bounds__(kCandidateThreads, 2)
void sm70_native_bm32_allp_p_group_pad_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
  // This path differs from the accepted pair-scratch baseline only in P.
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      const int phase_warp = threadIdx.x >> 5;
      const float top_exp_diff = make_group_padded_probability_row(
          shared.score + phase_warp * kBlockN,
          shared.probability_top[panel], phase_warp, shared.row_max,
          shared.row_sum, phase_warp, panel);
      const float bottom_exp_diff = make_group_padded_probability_row(
          shared.score + (kPanelM + phase_warp) * kBlockN,
          shared.probability_bottom[panel], phase_warp, shared.row_max,
          shared.row_sum, kPanelM + phase_warp, panel);
      if ((threadIdx.x & 31) == 0) {
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      ...
      update_group_padded_probability_pv_panel(
          shared.probability_top[panel], shared.probability_bottom[panel],
          value_group +
              (shared.block_index * kBlockN + panel * kSoftmaxPanelN) * kD,
          d_offset, accumulator_top, accumulator_bottom);
...
bool resource_gate(const KernelResources& resources, size_t shared_bytes) {
  return resources.registers_per_thread <= 64 &&
         resources.local_bytes_per_thread == 0 &&
         resources.static_shared_bytes == shared_bytes &&
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  const bool resource_pass =
      resource_gate(baseline_resources, sizeof(BaselineShared)) &&
      resource_gate(candidate_resources, sizeof(CandidateShared));
  ...
  std::cout << "  \"probability_layout\": {"
               "\"panel\": \"M16 x N32 = four 16x8 groups, 128 fp16/group\", "
               "\"panel_fp16\": 512, \"physical_panel_fp16\": 536, "
               "\"top_bottom_panels\": 8, \"baseline_top_bottom_bytes\": 8192, "
               "\"candidate_top_bottom_bytes\": 8576, "
               "\"baseline_offset\": \"group*128 + swizzled_slot(row)*8 + inner; group=column/8\", "
               "\"candidate_offset\": \"group*136 + swizzled_slot(row)*8 + inner; group=column/8\", "
               "\"bank_proof\": \"group base is 136 fp16 = 272B = 68 banks, or +4 mod 32; 32-bit store bank=(4*swizzled_slot+4*group+inner/2) mod 32, so groups 0..3 use four distinct banks and reduce the static four-way collision to one-way\", "
               "\"pv_matrix_a_loader\": \"P-only; two ld.shared.v4.u32 (LDS.128) per M16xK16 fragment; second address is first +272B with no second generic offset/cvta; Q loader unchanged\"},\n";
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"1 CTA/group; BM32, 512 threads; accepted all-P pair-scratch QK, softmax, PV\",\n";
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; same accepted pair-scratch path with P 136-half group-pad layout only\",\n";
  ...
  std::cout << "    \"baseline_shared_layout_bytes\": " << sizeof(BaselineShared)
            << ",\n"
            << "    \"candidate_shared_layout_bytes\": " << sizeof(CandidateShared)
            << "\n";
  std::cout << "  },\n";
  ...
  std::cout << "  \"resource_gate\": {\"max_registers_per_thread\": 64, "
               ...
               "\"baseline_required_static_shared_bytes\": "
            << sizeof(BaselineShared)
            << ", \"candidate_required_static_shared_bytes\": "
            << sizeof(CandidateShared)
            << ", \"min_active_ctas_per_sm\": 2, "
               "\"runtime_pass\": "
            << (resource_pass ? "true" : "false")
            << ", \"ptxas_validation\": \"compile with -O3 and --ptxas-options=-v; require 0 stack frame, 0 spill stores, 0 spill loads\"},\n";
  std::cout << "  \"exactness\": ";
  ...
  std::cout << ",\n";
  std::cout << "  \"tradeoff\": {\"candidate_extra_shared_bytes\": 384, "
               "\"candidate_extra_fp16_per_panel\": 24, "
               "\"baseline_shared_bytes\": 41744, "
               "\"candidate_shared_bytes\": 42128, "
               "\"qk_k_fragment_reuse\": 2, "
  ...
  std::cout << "  \"measurement\": {\"warmup_pairs\": " << args.warmup
            ...
            << ", \"interleaving\": \"baseline/candidate order alternates each round\", "
               "\"formal_gate\": \"100 paired rounds x 8 launches; at least 95 candidate wins and at least 2 percent wall-time win\"}\n";
  std::cout << "}\n";
}
...
int run(const Args& args) {
  ...
  const KernelResources baseline_resources = query_resources(
      sm70_native_bm32_allp_p_group_pad_baseline, kBaselineThreads);
  const KernelResources candidate_resources = query_resources(
      sm70_native_bm32_allp_p_group_pad_candidate, kCandidateThreads);
  const Exactness no_exactness;
  const TimingSummary no_timing;
  const PairSummary no_pairs;
  if (!resource_gate(baseline_resources, sizeof(BaselineShared)) ||
      !resource_gate(candidate_resources, sizeof(CandidateShared))) {
  ...
  auto launch_baseline = [&] {
    sm70_native_bm32_allp_p_group_pad_baseline<<<baseline_grid,
                                                kBaselineThreads>>>(
  ...
  auto launch_candidate = [&] {
    sm70_native_bm32_allp_p_group_pad_candidate<<<candidate_grid,
                                                 kCandidateThreads>>>(
