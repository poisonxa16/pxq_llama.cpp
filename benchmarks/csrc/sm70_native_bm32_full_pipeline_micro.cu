...
constexpr int kPVWarps = kD / (2 * kPanelN);
...
struct alignas(16) CandidateShared {
  ...
  float score_top[2][kScoreElements];
  float score_bottom[kScoreElements];
  __half probability[2][2][kPPanelElements];
  float row_max[kM];
  float row_sum[kM];
  float row_exp_diff[2][kM];
  alignas(16) int p_ready[2];
  int p_consumed[2];
  int qk_boot_ready;
  int next_score_ready;
  uint64_t protocol_error_address;
};
...
__device__ __forceinline__ void scale_candidate_accumulators(
    AccumulatorFragment& accumulator_d0_top,
    AccumulatorFragment& accumulator_d0_bottom,
    AccumulatorFragment& accumulator_d1_top,
    AccumulatorFragment& accumulator_d1_bottom,
    const float* __restrict__ row_exp_diff) {
  const int lane = threadIdx.x & 31;
  const int row = accumulator_fragment_row(lane, 0);
  float first_row_scale = row_exp_diff[row];
  float second_row_scale = row_exp_diff[row + 2];
  scale_accumulator_two_rows(accumulator_d0_top, first_row_scale,
                             second_row_scale);
  scale_accumulator_two_rows(accumulator_d1_top, first_row_scale,
                             second_row_scale);
  first_row_scale = row_exp_diff[kPanelM + row];
  second_row_scale = row_exp_diff[kPanelM + row + 2];
  scale_accumulator_two_rows(accumulator_d0_bottom, first_row_scale,
                             second_row_scale);
  scale_accumulator_two_rows(accumulator_d1_bottom, first_row_scale,
                             second_row_scale);
}
...
__device__ __forceinline__ void update_candidate_pv_panel(
    ...
    const __half* __restrict__ value_panel, int d0,
    AccumulatorFragment& accumulator_d0_top,
    AccumulatorFragment& accumulator_d0_bottom,
    AccumulatorFragment& accumulator_d1_top,
    AccumulatorFragment& accumulator_d1_bottom) {
  const int d1 = d0 + kPanelN;

  // Keep the K16 loop rolled so ptxas cannot software-pipeline both K16
  // fragments against the four long-lived output accumulators.
#pragma unroll 1
  for (int k_offset = 0; k_offset < kSoftmaxPanelN; k_offset += kPanelK) {
    {
      MatrixAFragment a_fragment;
      PVMatrixBFragment b_fragment;
      nvcuda::wmma::load_matrix_sync(b_fragment,
                                     value_panel + k_offset * kD + d0, kD);
      load_swizzled_matrix_a_fragment(a_fragment, probability_top, k_offset);
      nvcuda::wmma::mma_sync(accumulator_d0_top, a_fragment, b_fragment,
                             accumulator_d0_top);
      load_swizzled_matrix_a_fragment(a_fragment, probability_bottom,
                                      k_offset);
...
__device__ __forceinline__ void produce_candidate_probability_panel(
    CandidateShared* shared, const float* __restrict__ score_top,
    int qk_warp, int panel, int probability_slot) {
#pragma unroll 1
  for (int row_step = 0; row_step < 4; ++row_step) {
    const int row = qk_warp + row_step * kPVWarps;
    const int local_row = row & (kPanelM - 1);
    const bool top = row < kPanelM;
    const float exp_diff = make_probability_row(
        (top ? score_top : shared->score_bottom) + local_row * kBlockN,
...
__device__ __forceinline__ bool wait_for_counter(
    int* counter, int expected, CandidateShared* shared, uint32_t phase) {
  const int lane = threadIdx.x & 31;
  int observed = 0;
#pragma unroll 1
  ...
  if (lane == 0) {
    uint32_t* const protocol_error = reinterpret_cast<uint32_t*>(
        static_cast<uintptr_t>(shared->protocol_error_address));
    atomicCAS(protocol_error + blockIdx.x, 0u,
              phase | static_cast<uint32_t>(threadIdx.x >> 5));
  }
  return false;
}
...
void sm70_native_bm32_full_pipeline_baseline(
  ...
  const __half* query_group =
      query + static_cast<int64_t>(group) * kQElements + m_panel * kQPanelElements;
  const __half* key_group = key + static_cast<int64_t>(group) * nblocks *
                                       kBlockN * kD;
  const __half* value_group = value + static_cast<int64_t>(group) * nblocks *
                                           kBlockN * kD;
  __half* output_group =
...
void sm70_native_bm32_full_pipeline_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
  ...
  if (thread == 0) {
    ...
    shared.qk_boot_ready = 0;
    shared.next_score_ready = 0;
    shared.protocol_error_address = static_cast<uint64_t>(
        reinterpret_cast<uintptr_t>(protocol_error));
  }
  // The only full-CTA synchronization: query/row-state initialization ends
  // before the PV accumulator fragments are created below.
  __syncthreads();
  ...
  if (warp < kQKWarps) {
    {
      ...
      qk_pair_accumulate(shared.query, shared.query + kQPanelElements,
                         key_group + n_offset * kD, top_accumulator,
                         bottom_accumulator);
      nvcuda::wmma::store_matrix_sync(shared.score_top[0] + n_offset,
    ...
    publish_warp_counter(&shared.qk_boot_ready);
    if (!wait_for_counter(&shared.qk_boot_ready, kQKWarps, &shared,
                          kProtocolTimeoutBoot)) {
      return;
    }

    for (int block_n = 0; block_n < nblocks; ++block_n) {
      const int generation0 = block_n * kSoftmaxPanelsPerBlock;
      const bool has_next = block_n + 1 < nblocks;
      if (generation0 >= 2 &&
          !wait_for_counter(&shared.p_consumed[0],
                            kPVWarps * (generation0 >> 1), &shared,
                            kProtocolTimeoutPConsumed)) {
        return;
      }
      produce_candidate_probability_panel(&shared,
                                           shared.score_top[block_n & 1],
                                           warp, 0, 0);
      publish_warp_counter(&shared.p_ready[0]);
      if (!wait_for_counter(&shared.p_ready[0],
                            kQKWarps * ((generation0 >> 1) + 1), &shared,
                            kProtocolTimeoutPReady)) {
        return;
      }

      if (!has_next) {
#pragma unroll 1
        for (int panel = 1; panel < kSoftmaxPanelsPerBlock; ++panel) {
          const int generation = generation0 + panel;
          const int probability_slot = panel & 1;
          if (generation >= 2 &&
              !wait_for_counter(&shared.p_consumed[probability_slot],
                                kPVWarps * (generation >> 1), &shared,
                                kProtocolTimeoutPConsumed)) {
            return;
          }
          produce_candidate_probability_panel(
              &shared, shared.score_top[block_n & 1], warp, panel,
              probability_slot);
          publish_warp_counter(&shared.p_ready[probability_slot]);
          if (!wait_for_counter(
                  &shared.p_ready[probability_slot],
                  kQKWarps * ((generation >> 1) + 1), &shared,
                  kProtocolTimeoutPReady)) {
            return;
          }
        }
      ...
      AccumulatorFragment next_bottom_accumulator;
      {
        const int n_offset = warp * kPanelN;
        const __half* next_key =
            key + static_cast<int64_t>(group) * nblocks * kBlockN * kD +
            ((block_n + 1) * kBlockN + n_offset) * kD;
        AccumulatorFragment next_top_accumulator;
        qk_pair_accumulate(shared.query, shared.query + kQPanelElements,
                           next_key, next_top_accumulator,
                           next_bottom_accumulator);
        nvcuda::wmma::store_matrix_sync(
            shared.score_top[(block_n + 1) & 1] + n_offset,
            next_top_accumulator, kBlockN, nvcuda::wmma::mem_row_major);
      }
      if ((warp >> 1) == 0) {
        const int n_offset = warp * kPanelN;
        nvcuda::wmma::store_matrix_sync(shared.score_bottom + n_offset,
                                        next_bottom_accumulator, kBlockN,
                                        nvcuda::wmma::mem_row_major);
        publish_warp_counter(&shared.next_score_ready);
      }
...
#pragma unroll 1
      for (int panel = 1; panel < kSoftmaxPanelsPerBlock; ++panel) {
        ...
        produce_candidate_probability_panel(
            ...
            probability_slot);
        publish_warp_counter(&shared.p_ready[probability_slot]);
        if (!wait_for_counter(&shared.p_ready[probability_slot],
                              kQKWarps * ((generation >> 1) + 1), &shared,
                              kProtocolTimeoutPReady)) {
          return;
        }
      ...
      if (!wait_for_counter(&shared.next_score_ready,
                            kQKWarps * (block_n + 1), &shared,
                            kProtocolTimeoutNextScore)) {
        return;
      }
    }
    return;
    ...
    const int d0 = pv_warp * (2 * kPanelN);
    AccumulatorFragment accumulator_d0_top;
    AccumulatorFragment accumulator_d0_bottom;
    AccumulatorFragment accumulator_d1_top;
    AccumulatorFragment accumulator_d1_bottom;
...
#pragma unroll 1
      for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
        ...
        const int probability_slot = panel & 1;
        const int ready_epoch = kQKWarps * ((generation >> 1) + 1);
        if (!wait_for_counter(&shared.p_ready[probability_slot], ready_epoch,
                              &shared, kProtocolTimeoutPReady)) {
          return;
        }
        if (generation != 0) {
          scale_candidate_accumulators(
              accumulator_d0_top, accumulator_d0_bottom, accumulator_d1_top,
              accumulator_d1_bottom, shared.row_exp_diff[probability_slot]);
        }
        update_candidate_pv_panel(
            shared.probability[probability_slot][0],
            shared.probability[probability_slot][1],
            value_group + (block_n * kBlockN + panel * kSoftmaxPanelN) * kD,
            d0, accumulator_d0_top, accumulator_d0_bottom,
            accumulator_d1_top, accumulator_d1_bottom);
    ...
    __half* output_group =
        output + static_cast<int64_t>(group) * kOutputElements;
    store_accumulator_output(accumulator_d0_top, output_group, shared.row_sum,
                             0, d0);
    store_accumulator_output(accumulator_d0_bottom, output_group,
                             shared.row_sum, kPanelM, d0);
    store_accumulator_output(accumulator_d1_top, output_group, shared.row_sum,
                             0, d0 + kPanelN);
    store_accumulator_output(accumulator_d1_bottom, output_group,
                             shared.row_sum, kPanelM, d0 + kPanelN);
}
...
Exactness compare_outputs(const std::vector<__half>& baseline,
  ...
  for (size_t word = 0; word < word_count; ++word) {
    const uint32_t word_xor =
        half_word_bits(baseline.data(), word) ^ half_word_bits(candidate.data(), word);
...
ProtocolDiagnostics read_protocol_diagnostics(const uint32_t* device_errors,
                                              int groups) {
  std::vector<uint32_t> host_errors(groups);
  CUDA_CHECK(cudaMemcpy(host_errors.data(), device_errors,
                        host_errors.size() * sizeof(uint32_t),
                        cudaMemcpyDeviceToHost));
  ProtocolDiagnostics result;
  for (int group = 0; group < groups; ++group) {
    if (host_errors[group] == 0) {
      continue;
    }
    if (!result.timed_out) {
      result.first_group = group;
      result.first_code = host_errors[group];
    }
    result.timed_out = true;
    ++result.timeout_groups;
  }
  return result;
}
...
void report_protocol_timeout(const ProtocolDiagnostics& diagnostics) {
  ...
  std::cerr << "Candidate counter protocol timeout: groups="
            << diagnostics.timeout_groups << ", first_group="
            << diagnostics.first_group << ", code=0x" << std::hex
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  const bool candidate_resource_gate =
      ...
      candidate_resources.local_bytes_per_thread == 0 &&
      candidate_resources.static_shared_bytes <= 48 * 1024 &&
      candidate_resources.active_ctas_per_sm >= 2;
  ...
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"2 CTA/group; BM16, 512 threads; "
               "serial QK-softmax-PV with shared output\",\n";
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; "
               "8 QK producer and 8 PV consumer warps\",\n";
  std::cout << "    \"candidate_qk_order\": \"one K fragment, top then "
               "bottom M16\",\n";
  std::cout << "    \"candidate_pv_order\": \"one V fragment, top then "
               "bottom M16 for each D16\",\n";
  std::cout << "    \"candidate_score_buffers\": 3,\n";
  std::cout << "    \"candidate_probability_slots\": 2,\n";
  std::cout << "    \"candidate_shared_output\": false,\n";
  std::cout << "    \"candidate_pv_sync\": \"monotonic shared ready/consumed epochs; warp-local fences and syncwarp\",\n";
  std::cout << "    \"candidate_counter_overhead\": {\"p_ready_publishes_per_block\": "
            << kQKWarps * kSoftmaxPanelsPerBlock
            << ", \"p_consumed_publishes_per_block\": "
            << kPVWarps * kSoftmaxPanelsPerBlock
            << ", \"next_score_publishes_per_transition\": "
            << kQKWarps << ", \"spin_limit\": " << kCounterSpinLimit
            << "}\n";
  std::cout << "  },\n";
  ...
  std::cout << "  \"measurement\": {\"warmup_pairs\": " << args.warmup
            ...
            << ", \"interleaving\": \"baseline/candidate alternates each "
               "round\"},\n";
  ...
  std::cout << "  \"protocol_diagnostics\": {\"timed_out\": "
            ...
            << ", \"timeout_groups\": "
            << protocol_diagnostics.timeout_groups
            << ", \"first_group\": " << protocol_diagnostics.first_group
            << ", \"first_code\": " << protocol_diagnostics.first_code
            << "},\n";
  std::cout << "  \"resource_gate\": {\"candidate_runtime_pass\": "
            << (candidate_resource_gate ? "true" : "false")
            << ", \"max_registers_per_thread\": 64, "
               "\"max_static_shared_bytes\": 49152, "
               "\"min_active_ctas_per_sm\": 2, "
               "\"requires_external_sass_ldl_stl_check\": true}\n";
  std::cout << "}\n";
}
...
int run(const Args& args) {
  ...
  if (args.groups < 1 || args.nblocks < 1 || args.warmup < 0 ||
    ...
    std::cerr << "groups, nblocks, rounds, and launches-per-sample must be "
                 "positive; warmup cannot be negative\n";
    return EXIT_FAILURE;
  }
  ...
  CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&device_protocol_error),
                        static_cast<size_t>(args.groups) * sizeof(uint32_t)));
  ...
  auto launch_baseline = [&] {
    sm70_native_bm32_full_pipeline_baseline<<<baseline_grid, kBaselineThreads>>>(
        device_query, device_key, device_value, device_baseline, args.groups,
        args.nblocks);
    CUDA_CHECK(cudaGetLastError());
  };
  auto launch_candidate = [&] {
    sm70_native_bm32_full_pipeline_candidate<<<candidate_grid,
                                                kCandidateThreads>>>(
  ...
  if (args.profile_only) {
    ...
    const ProtocolDiagnostics diagnostics =
        read_protocol_diagnostics(device_protocol_error, args.groups);
    free_device_buffers();
  ...
  const KernelResources baseline_resources = query_resources(
      sm70_native_bm32_full_pipeline_baseline, kBaselineThreads);
  const KernelResources candidate_resources = query_resources(
      sm70_native_bm32_full_pipeline_candidate, kCandidateThreads);
    ...
    if (argument == "--device") {
      ...
      std::cerr << "Usage: " << argv[0]
                << " [--device N] [--groups N] [--nblocks N] [--warmup N]"
                   " [--rounds N] [--launches-per-sample N]"
