...
- Model:   any OpenAI-compatible endpoint. Point SWE_ENDPOINT at it and SWE_MODEL at
           the served model name (defaults: http://127.0.0.1:8080/v1/chat/completions,
           model "pxq"). Our published figure was measured against pxq_llama's
           llama-server holding the PXA Fusion2-35B PXQ4 tier with MTP enabled.
...
Layout under the working root (SWE_ROOT, default ./swe-work):
...
# Working root, endpoint and served-model name are all environment-overridable so this
# harness is not tied to any one machine. Defaults are a local llama-server on :8080 and
# a scratch directory beside wherever you run it.
ROOT = Path(os.environ.get("SWE_ROOT", "swe-work")).expanduser().resolve()
...
MODEL_URL = os.environ.get("SWE_ENDPOINT", "http://127.0.0.1:8080/v1/chat/completions")
MODEL_NAME = os.environ.get("SWE_MODEL", "pxq")
MAX_TURNS = 16
