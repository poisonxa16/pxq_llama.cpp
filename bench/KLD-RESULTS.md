...
Raw logs: `kld-{PXQ6,PXQ3,PXQ2}-pxqu.log` + the `base.log` bf16 dump from the `kld.sh` run (too large to commit; regenerate with `kld.sh`).
...
`bench/kld.sh` in this repository is the exact procedure (bf16 base dump, then per-tier scoring).
