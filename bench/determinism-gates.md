...
battery), `pxqu_golden.py`, `pxqu_wrel.py`, `pxqu_ref.cpp`, and `pxq1_selftest.cpp` (the PXQ1
byte-parity gate). Build notes are at the top of each file. Run them against any PXQ GGUF tier;
a failure of any bit-exact gate is a release-blocking bug — report it.

One more harness sits outside that directory: `.tie-harness/` is a self-contained A/B build
(`./run.sh`, needs only `g++`) that proves the PXQ6 (`pxq6r`) tie-breaking semantics are
quality-neutral — it asserts the legacy and shipped selectors produce bit-identical total
weighted SSE, and that forced-tie blocks resolve exactly per the deterministic selector.
