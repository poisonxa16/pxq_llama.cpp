#!/usr/bin/env python3
"""Discriminating probe for the repeated-token runaway observed on a 122B MoE.
...
BUILD = (os.environ.get("PXQ_SRC", ".") + "/build-unified")
...
PUB = (os.environ.get("PXQ_MODELS", "./models") + "/PXA-Fusion-122B-A5B-PXQ4.gguf")
FIX = (os.environ.get("PXQ_WORK", "./work") + "/PXA-Fusion-122B-A5B-PXQ4.gguf")
...
def serve(model, env):
    ...
    cmd = ("docker run -d --name %s --runtime=nvidia -e NVIDIA_VISIBLE_DEVICES=%s %s "
           ...
           "-c 4096 -np 1 -ngl 99 -sm layer -fa on -ctk f16 -ctv f16 "
...
PROMPT = None
for f in [(os.environ.get("PXQ_WORK", "./work") + "/scripts/codec_ab_prompts.json")]:
    for p in json.load(open(f)):
        if "runaway" in p["qid"]:
            PROMPT = p["prompt"]
if PROMPT is None:
    print("runaway prompt not found")
    sys.exit(2)
print("PROMPT: %s\n" % PROMPT[:160])
